---
title: 执行测试
description: 执行测试
ms.assetid: 421FD558-01D0-484d-B4F6-274A5C1BC62E
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 15f0b6ed1c194bb020e43d527a43b074f7b3e013
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/31/2019
ms.locfileid: "56575363"
---
# <a name="executing-tests"></a>执行测试


以下主题提供有关在 TAEF 中执行测试的详细信息：

-   [执行测试的概述](overview-of-executing-tests.md)
-   [Te.exe 命令行参数](te-exe-command-line-parameters.md)
-   [TAEF 的退出代码](exit-codes-for-taef.md)
-   [所选内容](selection.md)
-   [测试隔离](test-isolation.md)
-   [测试模式](test-modes.md)
-   [Parallel](parallel.md)
-   [RunAs](runas.md)
-   [跨计算机执行](cross-machine-execution.md)
-   [跨体系结构执行](cross-architecture-execution.md)
-   [TAEF 超时](taef-timeouts.md)
-   [用户模式监视器](user-mode-monitor.md)
-   [堆栈和转储日志记录](stack-and-dump-logging.md)
-   [调试 TAEF 测试](debugging-taef-tests.md)
-   [激活上下文](activation-context.md)
-   [TAEF 错误代码](taef-error-codes.md)

 

 





