...
当 SCO 配置文件驱动程序发出**BRB\_SCO\_打开\_通道**BRB 时，它将指定一个指针，该指针指向 BRB 的对应\_BRB\_SCO 的**回调**成员中的[*sco 回调函数*](https://docs.microsoft.com/windows-hardware/drivers/ddi/bthddi/nc-bthddi-pfnsco_indication_callback) [ **\_打开\_通道**](https://docs.microsoft.com/windows-hardware/drivers/ddi/bthddi/ns-bthddi-_brb_sco_open_channel)结构。 如果远程设备接受 SCO 连接请求，则当 SCO 连接发生变化时，蓝牙驱动程序堆栈可以通过回调函数向配置文件驱动程序发送通知。
...
当 L2CAP 配置文件驱动程序发出**BRB\_L2CA\_打开\_通道**BRB 时，它将在 L2CAP 的相应\_BRB\_BRB 的**回调**成员中指定指向其[*L2CA 回调函数*](https://docs.microsoft.com/windows-hardware/drivers/ddi/bthddi/nc-bthddi-pfnbthport_indication_callback)的指针。 [ **\_打开\_渠道**](https://docs.microsoft.com/windows-hardware/drivers/ddi/bthddi/ns-bthddi-_brb_l2ca_open_channel)结构。 如果远程设备接受 L2CAP 连接请求，则在发生对 L2CAP 连接的更改时，蓝牙驱动程序堆栈可以通过回调函数向配置文件驱动程序发送通知。
...
-   为 L2CAP 连接定义的上下文。 对于 BRB\_L2CA\_注册\_服务器请求，此上下文为在 \_BRB 的**IndicationCallbackContext**成员中传递的值\_L2CA\_注册\_服务器结构需要. 对于**BRB\_L2CA\_打开\_通道**或**BRB\_L2CA\_打开\_通道\_响应**请求，此上下文是在 \_的**CallbackContext**成员中传递的值。BRB\_L2CA\_打开随请求传入\_通道结构。
...
<td align="left"><p><strong>与</strong></p></td>
</tr>
<tr class="even">
...
-   为 SCO 连接定义的上下文。 对于**BRB\_SCO\_注册\_服务器**请求时，此上下文为在 \_BRB\_SCO\_的**IndicationCallbackContext**成员中传递的值。请求。 对于**BRB\_sco\_打开\_通道**或**BRB\_SCO\_打开\_通道\_响应**请求，此上下文是在 \_BRB 的**CallbackContext**成员中传递的值。\_SCO\_打开随请求传入\_通道结构。
...
当蓝牙驱动程序堆栈收到意外删除 IRP 后，它会将*ScoIndicationRemoteDisconnect*传递给配置文件驱动程序在配置文件驱动程序生成并发送 BRB\_sco 时由配置文件驱动程序指定的[*sco 回调函数*](https://docs.microsoft.com/windows-hardware/drivers/ddi/bthddi/nc-bthddi-pfnsco_indication_callback) [ **\_打开\_频道**](https://docs.microsoft.com/previous-versions/ff536626(v=vs.85))或[**BRB\_SCO\_打开\_通道\_响应**](https://docs.microsoft.com/previous-versions/ff536627(v=vs.85))请求，以关闭当前打开的所有 SCO 通道。 同样，在配置文件驱动程序生成并发送 BRB\_L2CA\_打开时，蓝牙驱动程序堆栈会将*IndicationRemoteDisconnect*传递到配置文件驱动程序指定的[*L2CAP 回调函数*](https://docs.microsoft.com/windows-hardware/drivers/ddi/bthddi/nc-bthddi-pfnbthport_indication_callback) [ **\_通道**](https://docs.microsoft.com/previous-versions/ff536615(v=vs.85))或[**BRB\_L2CA\_打开\_通道\_响应**](https://docs.microsoft.com/previous-versions/ff536616(v=vs.85))请求，以关闭当前打开的所有 L2CAP 通道。
