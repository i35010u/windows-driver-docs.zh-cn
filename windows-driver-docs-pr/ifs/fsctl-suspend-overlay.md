...
指向输入缓冲区，必须包含[ **WOF\_外部\_信息**](https://msdn.microsoft.com/library/windows/hardware/dn632452)结构。 在需要时，其他提供程序特定的数据是包含后立即**WOF\_外部\_信息**。 如果提供程序为 WIM 文件， [ **WIM\_提供程序\_挂起\_覆盖\_输入**](https://msdn.microsoft.com/library/windows/hardware/mt426740)结构是在**WOF\_外部\_信息**。
...
如果要删除的备份源 Windows 映像格式 (WIM) 文件，将包含输入的缓冲区[ **WOF\_外部\_信息**](https://msdn.microsoft.com/library/windows/hardware/dn632452)结构跟[ **WIM\_提供程序\_挂起\_覆盖\_输入**](https://msdn.microsoft.com/library/windows/hardware/mt426740)结构。 *InputBufferLength*在这种情况下将为**sizeof**(WOF\_外部\_INFO) + **sizeof**([**WIM\_提供程序\_删除\_覆盖\_输入**](https://msdn.microsoft.com/library/windows/hardware/dn632450))。 **DataSourceId**中的值**WIM\_提供程序\_挂起\_覆盖\_输入**之前在中添加的WIM文件必须是[ **FSCTL\_添加\_覆盖**](fsctl-add-overlay.md)请求。
