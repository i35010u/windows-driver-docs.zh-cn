...
由于它们可以在 DISPATCH_LEVEL IRQL 的情况下调用，因此完成例程无法调用必须在较低的 IRQL （如[**IoDeleteDevice**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iodeletedevice)）中调用的内核模式例程。 出于同一原因，必须从非分页池分配完成例程中使用的任何数据结构。
