...
-   分页 i/o 路径中的调度例程绝不应在 APC\_级别之上的任何 IRQL 上调用[**IoCallDriver**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocalldriver) 。 如果调度例程引发 IRQL，则必须在调用**IoCallDriver**之前将其降低。

-   寻呼路径中的调度例程（如读取和写入）无法安全调用任何需要调用方在 IRQL 被动\_级别运行的内核模式例程。

-   页面文件 i/o 路径中的调度例程不能安全地调用任何要求调用方以 IRQL 运行的内核模式例程，&lt; 调度\_级别。

-   不在分页 i/o 路径中的调度例程绝不应在被动\_级别以上的任何 IRQL 上调用[**IoCallDriver**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocalldriver) 。 如果调度例程引发 IRQL，则必须在调用**IoCallDriver**之前将其降低。
...
-   此外，如果 IRP 包含从32位平台发送到64位平台的 IOCTL 或 FSCTL 缓冲区，则可能需要 thunked 缓冲区内容。 有关详细信息，请参阅[64 位驱动程序中的支持32位 i/o](https://docs.microsoft.com/windows-hardware/drivers/kernel/supporting-32-bit-i-o-in-your-64-bit-driver)。

-   与文件系统不同，文件系统筛选器驱动程序绝不应调用[**FsRtlEnterFileSystem**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsrtlenterfilesystem)或[**FsRtlExitFileSystem**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsrtlexitfilesystem) ，除非调用[**ExAcquireFastMutexUnsafe**](https://docs.microsoft.com/previous-versions/windows/hardware/drivers/ff544340(v=vs.85))或[**ExAcquireResourceExclusiveLite**](https://msdn.microsoft.com/library/windows/hardware/ff544351)。 **FsRtlEnterFileSystem**和**FsRtlExitFileSystem**禁用正常内核 apc，这是大多数文件系统所需的。
...
-   尽管状态\_"挂起" 是成功的 NTSTATUS 值，但它是一个编程错误，用于完成\_状态为 "挂起" 的 IRP。

-   调度例程调用[**IoCompleteRequest**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocompleterequest)后，IRP 指针将不再有效，且不能安全地取消引用。
...
-   当调度例程调用[**IoSetCompletionRoutine**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iosetcompletionroutine)时，它可以选择将*上下文*指针传递到结构，以便完成例程在处理给定的 IRP 时使用。 此结构必须从非分页池分配，因为完成例程可以被称为 IRQL 调度\_级别。

-   如果派单例程设置的完成例程可能返回状态\_\_需要更多的\_处理，则必须执行以下操作之一来阻止 i/o 管理器提前完成 IRP：
    -   将 IRP 标记为 "挂起"，调用[**IoCallDriver**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocalldriver)，并返回\_"挂起" 状态。
    -   调用[**KeWaitForSingleObject**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-kewaitforsingleobject)以等待完成例程执行，然后调用[**IOCOMPLETEREQUEST**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocompleterequest)来完成 IRP。
...
-   在调度例程调用[**IoCallDriver**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocalldriver)后，IRP 指针不再有效且不能安全地取消引用，除非调度例程等待完成例程通知它已被调用。

-   从文件系统筛选器驱动程序调用[**PoCallDriver**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntifs/nf-ntifs-pocalldriver)的编程错误。 （**PoCallDriver**用于将 IRP\_MJ\_请求传递到较低级别的驱动程序。 文件系统筛选器驱动程序绝不会接收 IRP\_MJ\_的 POWER 请求。）
...
-   当[**IoCallDriver**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocalldriver)返回状态\_"挂起" 时，调度例程还应返回状态\_"挂起"，除非它等待完成例程向事件发出信号。

-   将 IRP 发送到辅助角色队列以便以后处理时，调度例程应将 IRP 标记为 "正在挂起" 并返回状态\_"挂起"。

-   在将 IRP 设置为将 IRP 发送到辅助角色队列以便以后处理时，调度例程应将 IRP 标记为 "正在挂起" 并返回状态\_"挂起"。

-   标记 IRP "挂起" 的调度例程必须返回状态\_"挂起"。

-   Oplock 操作不能挂起（发布到辅助角色队列），调度例程无法返回状态\_挂起。
...
-   如果调度例程将 Irp 发送到工作队列，则必须先调用[**也**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iomarkirppending)，然后才能将每个 irp 发送到工作队列。 否则，IRP 可以取消排队，由其他驱动程序例程完成，并在调用**也**之前由系统释放，从而导致崩溃。
