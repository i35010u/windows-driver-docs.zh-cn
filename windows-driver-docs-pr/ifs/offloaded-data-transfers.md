...
此方案涉及在两个不同文件服务器上的两个位置之间复制文件，每个位置都具有通过智能存储阵列（ISA）公开的其自己的虚拟磁盘。 初始系统首先需要将源虚拟磁盘中的数据读入本地缓冲区。 然后，它将通过某些传输和协议（如 SMB over 1GbE）将数据打包并传输到第二个系统，后者随后接收数据并将数据输出到本地缓冲区。 然后，目标系统将数据写入目标虚拟磁盘。 此方案描述了一种非常典型的数据传输读取/写入方法，这些方法每天由许多不同的应用程序执行多次。
...
<span id="FSCTL_OFFLOAD_READ"></span><span id="fsctl_offload_read"></span>[**FSCTL\_卸载\_读取**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-read)  
此控制请求在要读取的文件中采用偏移量，并在[**FSCTL\_卸载\_读取\_输入**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_read_input)结构中有一个所需的长度。 如果支持，则承载该文件的存储子系统将接收关联的卸载读取存储命令，并生成一个令牌，该令牌是在卸载读取命令时要读取的数据的逻辑表示形式。 此标记字符串返回到[**FSCTL\_卸载\_读取\_输出**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_read_output)结构中的调用方。

<span id="FSCTL_OFFLOAD_WRITE"></span><span id="fsctl_offload_write"></span>[**FSCTL\_卸载\_写入**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-write)  
...
1.  复制引擎会在源文件上发出[**FSCTL\_卸载\_读取**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-read)，以获取读取令牌。
2.  如果检索读取令牌时出现故障，复制引擎将回退到传统的读取和写入（传统的复制文件代码路径）。 如果故障表明源卷不支持卸载，则复制引擎还会在每个进程缓存中标记该卷。 对于每个进程的缓存中的卷，复制引擎将不尝试重新卸载。
3.  如果已成功检索到令牌，则复制引擎会尝试发出[**FSCTL\_卸载\_** ](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-write)在大块区中的目标文件上写入命令，直到该令牌以逻辑方式表示的所有数据已被卸载。
4.  执行卸载读取或写入操作时出现的任何错误都会导致复制引擎恢复到读写的传统代码路径，从卸载代码路径结束的位置开始（读取或写入被截断）。 如果故障表明目标卷不支持卸载，或源卷无法访问目标卷，则复制引擎会更新相同的每个进程缓存，以便不会在这些卷上尝试卸载。 此每进程缓存将定期重置。
...
Hyper-v 存储堆栈和 Windows SMB 文件服务器中提供了对卸载操作的支持。 当后备物理存储支持 ODX 操作时，调用方可以发出[**FSCTL\_卸载\_读取**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-read)和[**FSCTL\_卸载\_写入**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-write)位于 vhd 或远程文件共享上的文件，不管是从虚拟计算机或物理硬件上。 下图说明了卸载数据传输的最基本的受支持源和目标目标。
...
筛选器必须通过位于驱动程序服务定义中的一个名为**SupportedFeatures**的注册表**DWORD**值来指示它支持[**FSCTL\_卸载\_读取**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-read)和[**FSCTL\_卸载\_写入**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-write)在注册表中的 HKEY\_本地\_计算机\\System\\CurrentControlSet\\Services\\&lt;筛选器驱动程序名称&gt;\\。 此值包含位域，其中位确定选择加入的功能，并且应在筛选器安装过程中设置。

目前，定义的位是：

| 旗帜                                               | 含义                               |
|----------------------------------------------------|---------------------------------------|
| 支持\_FS\_功能\_卸载\_读取0x00000001  | 筛选器支持 FSCTL\_卸载\_读取  |
| 支持\_FS\_功能\_卸载\_写入0x00000002 | 筛选器支持 FSCTL\_卸载\_写入 |

 

可以基于 HKEY\_本地\_计算机\\系统\\CurrentControlSet\\控件中提供的值，启用或禁用筛选器选择模式\\FilterSupportedFeaturesMode 注册表密钥，它具有以下值：

| FilterSupportedFeaturesMode 值 | 含义                                                                             |
|-----------------------------------|-------------------------------------------------------------------------------------|
| 0（默认值）                       | 执行正常的选择正在处理。                                                        |
| 1                                 | 从不选择加入（等效于在所有附加的筛选器上将 SupportedFeatures 设置为0） |
...
作为 IRP 处理的一部分， [**FsRtlGetSupportedFeatures**](https://msdn.microsoft.com/library/windows/hardware/hh920378)例程检索附加到给定卷堆栈的所有筛选器的聚合**SupportedFeatures**状态。 组件（如 i/o 管理器和 SRV （SMB））调用此例程来验证堆栈上所有筛选器的**SupportedFeatures**状态。 滚动自己的卸载 Irp 的组件应调用此函数来验证对该操作的选择支持。
...
-   更新筛选器安装程序，以便将**SupportedFeatures**的 REG\_DWORD 值添加到 HKLM\\System\\CurrentControlSet\\Services\\\[filter\] 子项。 将其初始化以指定卸载功能。
-   对于想要对卸载操作执行操作的筛选器，请将注册操作更新为**IRP\_MJ\_FILE\_SYSTEM\_控制**来处理[**FSCTL\_** ](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-read) [ **\_\_写**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-write)。
-   对于需要阻止卸载操作的筛选器，从筛选器内返回状态代码状态\_不\_支持。 不要依赖于注册表值来强制阻止卸载操作，因为最终用户可以对其进行更改。 筛选器应显式允许或禁止卸载操作。
...
使用卸载操作时，i/o 堆栈不会显示文件数据。 相反，它被视为数据的逻辑代理作为512字节的标记。 此令牌是存储子系统生成的特定于供应商的格式的不透明且唯一的字符串，也可以是众所周知的类型来表示数据模式（如逻辑上等于零的数据范围）。 对标记是其代理的数据的修改将导致令牌无效，或对存储子系统进行修改以使其按某些供应商特定的方式（例如通过快照机制）保存原始数据。 对于文件中指定范围的后续卸载读取请求将生成唯一标记。

有类标记表示定义良好的数据模式。 最常见的已知标记是与零等效的零标记。 当令牌定义为已知令牌时，**存储\_卸载\_令牌**结构中的**TokenType**成员将设置为存储\_卸载\_令牌\_类型\_已知。 设置此字段时， **WellKnownPattern**成员确定标记的数据模式。

-   将**WellKnownPattern**字段设置为存储时\_卸载\_模式\_零或存储\_卸载\_\_\_\_\_，它指示零标记。 当[**FSCTL\_卸载\_读取**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-read)操作返回此标记时，它表示所需文件范围内包含的数据在逻辑上等于零。 将此令牌提供给[**FSCTL\_卸载\_写入**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-write)操作时，它指示要写入的文件的所需范围应在逻辑上归零。
...
Windows 与之通信的基础存储子系统可以处理卸载操作所需的更少数据。 这称为 "截断"。 使用 "卸载" 读取时，这意味着返回的令牌表示比请求的数据范围少的数据。 这由[**FSCTL\_卸载\_读取\_输出**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_read_output)结构中的**TransferLength**成员指示，该结构是从要读取的文件范围的开头开始的字节计数。 对于卸载写入，截断表明写入的数据量小于所需的数据量。 这由[**FSCTL\_卸载\_写入\_输出**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_output)结构中的**LengthWritten**成员指示，该结构是从要写入的文件范围的开头开始的字节计数。 命令处理中的错误或大型范围内堆栈限制会导致截断。

在以下两种情况下，NTFS 会截断要读取或写入的范围：

1.  如果 VDL 在文件结尾（EOF）之前，则复制范围将截断为有效数据长度（VDL）。 这假定 VDL 与逻辑扇区边界对齐，否则请参阅方案。

    ![eof 之前发生的 vdl](images/odx-vdl-1.png)

    在[**FSCTL\_卸载\_读取**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-read)操作过程中，标志卸载\_读取\_标志\_超出\_当前\_范围\_\_\_\_中[**设置读取\_输出**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_read_output)结构，指示文件的其余部分包含零， **TransferLength**成员被截断为 VDL。
...
-   仅 NTFS 卷支持卸载操作。
-   如果远程共享是 NTFS 卷并且服务器正在运行 Windows Server 2012 （假设远程堆栈还支持卸载操作），则通过远程文件服务器支持卸载操作。
-   NTFS 不支持在用 Bitlocker 或 NTFS 加密（EFS）加密的文件上执行的卸载 FSCTLs、包含消除重复的文件、压缩文件、常驻文件、稀疏文件或参与了 TxF 事务处理的文件。
...
-   在[**FSCTL\_卸载\_写入**](https://docs.microsoft.com/windows-hardware/drivers/ifs/fsctl-offload-write)之前，必须预先分配目标文件（**SetEndOfFile** ，而不是**SetAllocation**）。
-   在处理卸载读取和卸载写入时，NTFS 首先调用[**CcCoherencyFlushAndPurgeCache**](https://msdn.microsoft.com/library/windows/hardware/ff539032)以提交系统缓存中任何已修改的数据。 这与非缓存 IO 的语义相同。
