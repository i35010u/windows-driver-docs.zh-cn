...
文件系统在系统启动过程中进行初始化;具体而言，在 i/o 系统初始化过程中。 I/o 管理器会创建全局文件系统队列，并初始化操作系统（OS）加载程序和 PnP 管理器加载的文件系统和筛选器驱动程序。

下面是对文件系统和筛选器驱动程序开发人员感兴趣的系统引导过程中选定部分的摘要。

1.  在系统启动过程中，操作系统加载程序将加载启动文件系统、原始文件系统以及类型为 SERVICE\_BOOT\_的所有驱动程序，然后在加载程序将控制权移交给内核之前开始。 当内核获得控制时，这些驱动程序将在内存中。
...
3.  PnP 管理器调用原始文件系统的[**DriverEntry**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nc-wdm-driver_initialize)例程和所有服务\_启动\_启动驱动程序。

    如果服务\_启动\_启动驱动程序依赖于其他驱动程序，则也会加载并启动这些驱动程序。
...
4.  PnP 管理器遍历 PnP 设备树，查找并加载与每个 devnode 相关联但尚未运行的驱动程序。

    （有关 PnP 设备树的详细信息，请参阅[设备树](https://docs.microsoft.com/windows-hardware/drivers/kernel/device-tree)。）每个 PnP 设备启动时，PnP 管理器会枚举设备的子节点（如果有）。 PnP 管理器配置子设备，加载其设备驱动程序，并启动设备。
...
5.  PnP 管理器加载并初始化尚未加载的 SERVICE\_系统\_启动类型的驱动程序。

    此时将加载文件系统识别器（FsRec）。 请注意，尽管它位于 "启动文件系统" 加载顺序组，但 FsRec 不是启动文件系统。 实际启动文件系统（即装载启动卷的文件系统）是在启动过程开始时加载的。

    稍后在服务\_系统\_开始阶段，将加载 "文件系统" 加载顺序组中的文件系统。 这包括命名管道文件系统（NPFS）和 Mailslot 文件系统（MSFS）。 它不包括基于媒体的文件系统，例如 NTFS、FAT、CDFS 或 UDF。
...
7.  服务控制管理器加载类型为 SERVICE\_自动\_启动的、尚未加载的驱动程序。

### <a name="span-idddk_file_system_recognizer_ifspanspan-idddk_file_system_recognizer_ifspanfile-system-recognizer"></a><span id="ddk_file_system_recognizer_if"></span><span id="DDK_FILE_SYSTEM_RECOGNIZER_IF"></span>文件系统识别器

系统启动后，将加载并启动附加到系统的所有卷的存储设备驱动程序。 但是，并非所有内置文件系统都已加载，且并非所有文件系统卷都已装入。 文件系统识别器（FsRec）根据需要执行这些任务，以处理[**IRP\_MJ\_创建**](https://docs.microsoft.com/windows-hardware/drivers/ifs/irp-mj-create)请求。

在系统启动\_系统\_启动阶段，会在服务中加载 FsRec。 请注意，尽管它位于 "启动文件系统" 加载顺序组，但 FsRec 不是启动文件系统。 实际启动文件系统（即装载启动卷的文件系统）是在启动过程开始时加载的。
