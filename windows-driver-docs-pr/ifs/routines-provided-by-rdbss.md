...
<td align="left"><p>此资源获取例程以独占模式获取文件控制块（FCB）资源。 此例程将等待 FCB 资源空闲，因此在获取资源之前，此例程不会返回控制。 即使与此 FCB 关联的 RX_CONTEXT 已被取消，此例程也会获取 FCB 资源。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此资源获取例程获取共享模式下的 FCB 资源。 如果以前以独占方式获取 FCB 资源，此例程将等待该资源空闲，因此在获取资源之前，此例程不会返回控制。 即使与此 FCB 关联的 RX_CONTEXT 已被取消，此例程也会获取 FCB 资源。</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>此资源获取例程获取共享模式下的 FCB 资源。 如果以前以独占方式获取了 FCB 资源，此例程将等待该资源释放。 在获取资源之前，此例程不会返回控制。 即使与此 FCB 关联的 RX_CONTEXT 已被取消，此例程也会获取 FCB 资源。</p>
...
<td align="left"><p>此例程将提供的不透明上下文与 MID_ATLAS 数据结构中可用的多路复用 ID （MID）相关联。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程沿虚拟线路上的指定连接发送传输服务数据单位（TSDU）。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程分配 MID_ATLAS 数据结构的新实例，并对其进行初始化。 RDBSS 使用在此数据结构中定义的多路复用 ID （MID），网络客户端（微型重定向程序）和服务器可以区分任何连接上的并发活动请求。</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>此例程为此 FCB 打开的 NET_ROOT 结构分配、初始化新的 FCB 结构并将其插入到内存中数据结构中。 所分配的结构具有 SRV_OPEN 和 FOBX 结构的空间。 此例程由 RDBSS 在内部使用，不应由网络微型重定向程序驱动程序使用。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程生成表示 NET_ROOT 结构的节点，并将该名称插入到相关设备对象上的网络名称表中。 此例程由 RDBSS 在内部使用，不应由网络微型重定向程序使用。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程生成一个节点，该节点表示 V_NET_ROOT 结构，并将该名称插入网络名称表。 此例程由 RDBSS 在内部使用，不应由网络微型重定向程序驱动程序使用。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程取消引用 RX_CONTEXT 结构，如果引用计数变为零，则从内存中数据结构中释放并删除指定的 RX_CONTEXT 结构。</p></td>
</tr>
<tr class="odd">
...
<p>对于非单片驱动程序，此初始化例程等效于<em>rdbss</em>设备驱动程序的<strong>DriverEntry</strong>例程。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程将完成给定的 NET_ROOT 结构。 调用方应对与此 NET_ROOT 结构关联的设备对象的网络名称表（通过 SRV_CALL 结构）具有独占访问权限。 此例程由 RDBSS 在内部使用，不应由网络微型重定向程序驱动程序使用。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程将完成给定的 V_NET_ROOT 结构。 调用方必须具有对与此 V_NET_ROOT 结构关联的设备对象的网络名称表上的锁的独占访问权限。 此例程由 RDBSS 在内部使用，不应由网络微型重定向程序使用。</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>此例程强制最终完成与给定 NET_ROOT 结构关联的所有 V_NET_ROOT 结构。 调用方必须具有对与此 V_NET_ROOT 结构关联的设备对象的网络名称表上的锁的独占访问权限。 此例程由 RDBSS 在内部使用，不应由网络微型重定向程序使用。</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>此例程将由 RX_CONTEXT 结构指定的 i/o 请求数据包（IRP）排队到辅助队列中，以便通过文件系统进程（FSP）进行处理。</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>此例程尝试从 RX_CONTEXT 结构中的<strong>CreateOptions</strong> （ <strong>NtCreateParameters</strong>）字段推断文件类型（目录或非目录）。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程在 MID_ATLAS 数据结构中将 MID 映射到其关联的上下文，并将中间与上下文解除关联。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程将 MID 映射到 MID_ATLAS 数据结构中与其关联的上下文。</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>例程在前缀表中查找用于对 SRV_CALL 和 NET_ROOT 名称进行分类的名称，并将基础指针转换为包含结构。</p>
...
<td align="left"><p>此例程用于跟踪在已检查生成中引用 SRV_CALL、NET_ROOT、V_NET_ROOT、FOBX、FCB 和 SRV_OPEN 结构的请求。 日志记录系统和 WMI 可访问这些引用请求的日志。</p>
<p>对于零售版本，此例程不执行任何操作。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程释放使用 RxAcquireSharedFcbResourceInMRxEx 获取的 FCB <a href="https://docs.microsoft.com/windows-hardware/drivers/ddi/mrxfcb/nf-mrxfcb-rxacquiresharedfcbresourceinmrxex" data-raw-source="[&lt;strong&gt;RxAcquireSharedFcbResourceInMRxEx&lt;/strong&gt;](https://docs.microsoft.com/windows-hardware/drivers/ddi/mrxfcb/nf-mrxfcb-rxacquiresharedfcbresourceinmrxex)">资源</a></p>
<p>此例程仅在 Windows Server 2003 Service Pack 1 （SP1）和更高版本上可用。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程为 RX_CONTEXT 结构设置网络小型重定向器取消例程。</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>此例程设置与任何给定服务器（SRV_CALL 结构）关联的域名。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>此例程启动一个网络微型重定向程序，调用它来注册自身。 如果驱动程序指示支持 UNC 名称，RDBSS 还会将网络小型重定向程序驱动程序注册为具有多 UNC 提供程序（MUP）的通用命名约定（UNC）提供程序。</p></td>
</tr>
<tr class="even">
...
<p>建议使用<a href="https://docs.microsoft.com/windows-hardware/drivers/ddi/rxlog/nf-rxlog-_rxlog" data-raw-source="[&lt;strong&gt;RxLog&lt;/strong&gt;](https://docs.microsoft.com/windows-hardware/drivers/ddi/rxlog/nf-rxlog-_rxlog)"><strong>RxLog</strong></a>宏，而不是直接调用此例程。</p>
