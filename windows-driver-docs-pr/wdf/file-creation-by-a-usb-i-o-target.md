...
驱动程序可以在格式函数中使用此堆栈内文件对象（例如，当驱动程序必须在此文件对象的默认会话上发送 i/o 时，驱动程序可以通过调用[**IWDFIoTarget：： FormatRequestForRead**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wudfddi/nf-wudfddi-iwdfiotarget-formatrequestforread)方法，将此文件对象的指针传递到 *.pfile*参数）。 为了获取堆栈内文件对象，驱动程序可以调用[**IWDFIoTarget：： GetTargetFile**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wudfddi/nf-wudfddi-iwdfiotarget-gettargetfile)方法。
