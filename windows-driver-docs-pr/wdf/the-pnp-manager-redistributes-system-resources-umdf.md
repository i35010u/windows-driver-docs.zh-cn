...
在重新分发的资源，即插即用管理器不会更改设备的资源分配如果其中一个设备的基于 UMDF 驱动程序提供[ **IPnpCallback::OnQueryStop** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wudfddi/nf-wudfddi-ipnpcallback-onquerystop)回调函数和回调函数已禁止重新分配。
...
3.  框架将调用的驱动程序[ **IPnpCallback::OnD0Exit** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wudfddi/nf-wudfddi-ipnpcallback-ond0exit)回调函数 （如果存在）。

4.  框架将调用的驱动程序[ **IPnpCallbackHardware::OnReleaseHardware** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wudfddi/nf-wudfddi-ipnpcallbackhardware-onreleasehardware)传递的即插即用的管理员分配给设备的硬件资源的列表的回调函数 （如果存在）。
...
1.  框架将调用的驱动程序[ **IPnpCallbackHardware::OnPrepareHardware** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wudfddi/nf-wudfddi-ipnpcallbackhardware-onpreparehardware)回调函数 （如果存在），同时传入的即插即用的管理员分配给设备的硬件资源的列表。

2.  框架将调用的驱动程序[ **IPnpCallback::OnD0Entry** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wudfddi/nf-wudfddi-ipnpcallback-ond0entry)回调函数 （如果存在）。
