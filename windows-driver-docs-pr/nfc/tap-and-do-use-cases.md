...
3. 如果他们接受邀请，他们可以获取应用。 应用不会自动启动。 为此，他们需要直接运行它，然后再次点击其朋友（*点击和使用*用例），或者只需再次点击其朋友即可启动应用（*点击和启动*用例）。
...
1. 用户希望与朋友玩游戏，并启动应用程序。 应用可能会提示你将其计算机点击另一台计算机。
2. 第二个用户未安装应用。 它们被邀请获取应用。 如果他们拒绝邀请，则不会发生任何事情。
3. 如果他们接受邀请，他们可以获取应用。 应用不会自动启动。 为此，他们需要直接运行它，然后再次点击其朋友（*点击和使用*用例），或者只需再次点击其朋友即可启动应用（*点击和启动*用例）。
...
*点击和接收*是用户接收内容的交互。 利用这种体验，用户可将其计算机点击为其他设备（如计算机或电话）或共享内容的对象（如标记）。 Windows 将启动用户确认的接收体验，以帮助用户获取和使用收到的内容。
