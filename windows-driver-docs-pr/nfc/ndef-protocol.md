...
若要侦听这些 NDEF 消息，客户端需要订阅其中一种受支持的类型，如 "NDEF： wkt"。Sp "。 只要提供程序检测到与类型匹配的 NDEF 消息，就会将整个 NDEF 消息（仍以 NDEF 编码）传递给订阅客户端。 根据 \[NDEF\]中的约定，要与 NDEF 消息匹配的 "type" 是 NDEF 消息的第一个 NDEF 记录中指定的类型字段。 同样，若要传输 NDEF 消息，客户端将发布一个完整的 NDEF 消息，并指定 "NDEF" 协议。
...
-   驱动程序必须根据 \[NDEF\]中指定的 NDEF 消息中第一个 NDEF 记录的 TNF 和 TYPE 字段，将收到的 NDEF 消息与订阅匹配。
...
-   如果一个或多个 "\*：WriteTag "已启用发布，并且驱动程序检测到一个可写标记，驱动程序必须首先向标记写入，遵循其他地方所述的常见"\*： WriteTag "要求，然后将 NDEF 读/写标记转换为只读。
...
使用状态\_无效\_参数的邻近性提供程序驱动程序必须拒绝具有此类型的订阅或发布。
...
该驱动程序必须与 "NDEF： ext.&lt;SomeExternalType&gt;" 的订阅匹配，只使用收到的 NDEF 消息，其 TNF 字段值为0x04，并且具有与 "&lt;SomeExternalType&gt;" 匹配的类型字段基于等效性\[NFC RTD\]中指定的规则。
...
驱动程序必须与 "NDEF： MIME" 的订阅匹配。仅&lt;SomeMimeType&gt;"接收的 NDEF 消息，该消息的 TNF 字段值为0x02，并且具有与在 \[NDEF\]中指定的等效规则匹配"&lt;SomeMimeType&gt;"的类型字段。
...
-   驱动程序必须与 "NDEF： wkt" 的订阅匹配。仅&lt;SomeWellKnownType&gt;"接收的 NDEF 消息，该消息的 TNF 字段值为0x01，并且具有与"&lt;SomeWellKnownType&gt;"匹配的类型字段，该字段基于在 \[NDEF\]中指定的等效规则。
