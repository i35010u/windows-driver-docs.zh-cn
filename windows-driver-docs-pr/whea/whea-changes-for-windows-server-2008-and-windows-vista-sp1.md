...
-   每当发生硬件错误时，WHEA 将引发 Windows 事件跟踪（ETW）事件。 从 Windows Server 2008 开始，WHEA 硬件错误事件和描述这些硬件错误事件的数据模板不同于 Windows Vista 上支持的事件和模板。

    有关 WHEA 内 ETW 支持的详细信息，请参阅[硬件错误事件](https://docs.microsoft.com/windows-hardware/drivers/ddi/_whea/)。
