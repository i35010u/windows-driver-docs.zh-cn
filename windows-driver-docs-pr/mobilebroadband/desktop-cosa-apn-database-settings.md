...
| 设置名 | 说明 | 可选或必需 | 说明 | COSA 支持的最低 Windows 版本 |
| --- | --- | --- | --- | --- |
| UpdateType | 描述 APN 数据库条目是新增还是修改。 | 必须 | 可能的值： <ul><li>**添加** –新条目</li><li>**更改** –更新现有条目</li><li>**保留** -不更改条目</li><li>**删除** –删除条目</li></ul> | Windows 10 版本 1703 |
| 国家/地区 | APN 条目的国家或地区。 | 必须 | 此项可能会更改以匹配 Windows 如何引用特定的国家或地区。 |  Windows 10 版本 1703 |
| 运算符 | 操作员的名称。 不需要在此字段中包含国家或地区。 | 必须 | 确保每次提交 APN 项的更新时均使用相同的拼写和大小写。 | Windows 10 版本 1703 |
...
| ICCID 范围-开始 |  以89开头的19或20位数字 (ICCID 颁发者标识符编号) 。 | 可选 | 该数字应以00结束。  | Windows 10 版本 1703 |
| ICCID 范围-结束  |  以89开头的19或20位数字 (ICCID 颁发者标识符编号) 。 | 可选 | 该数字应以99结束。  | Windows 10 版本 1703 |
...
| AccessString | 对于 GSM 网络，这是一个接入点，例如 data.contoso.com。 对于 CDMA 网络，此访问字符串包含特殊的拨号代码，如 #777 或网络访问标识符（如） example@contoso.com 。 | 必须 |  | Windows 10 版本 1703 |
| UserName | 用于连接到 APN 的用户名。 此设置区分大小写。  | 可选 |  | Windows 10 版本 1703 |
| Password |  用于连接到 APN 的密码。 此设置区分大小写。  | 可选 |  | Windows 10 版本 1703 |
| AuthProtocol | 指定用于 (PDP) 上下文激活数据包数据协议的身份验证协议。 | 可选 | 可能的值： <ul><li>**无** -无需身份验证协议</li><li>**Pap** –要求执行 pap 身份验证。</li><li>需要**chap** -chap 身份验证。</li><li>**Eap-mschapv2** –需要进行身份验证。</li></ul> <p>此设置仅支持 windows 10 版本1703之前的 Windows 10 Windows 8.1 和版本。</p> | Windows 10 版本 1703 |
...
| 设置名 | 说明 | 可选或必需 | 说明 | 支持的最低 Windows 版本 |
| --- | --- | --- | --- | --- |
...
| SPN | 服务提供程序名称的标识符字符串 (SPN)  | 可选 | 有助于识别 MO/MVNO 的网络。 如果留空，则默认为空字符串，并且不执行任何操作。 | Windows 10 版本 1703 |
| AlwaysOn | 描述连接是否始终打开。 | 必须 | 如果留空，默认值为 "true"。 | Windows 10 版本 1703 |
| PurposeGroups | 一个字符串，该字符串通过以逗号分隔的表示目的值的 Guid 列表指定连接目的。 | 必须 | 可用值如下： <ul><li>**Internet**</li><li>**彩信**</li><li>**IMS**</li><li>**SUPL**</li><li>**购买**</li><li>**管理**</li><li>**应用程序**</li><li>**eSIM 预配**</li></ul> <p> 如果留空，默认值为 "Internet"。</p> | Windows 10 版本 1703 |
...
| 漫游 | 指定应在其下激活连接的漫游条件。 | 可选 | 可能的值： <ul><li>0：仅限家庭网络</li><li>1： (home and 漫游) 所有漫游条件</li><li>2：仅限家庭和国内漫游</li><li>3：仅限国内漫游</li><li>4：仅限非国内漫游</li><li>5：仅漫游</li></ul> 如果留空，则默认为 1 (所有漫游条件) 。 | Windows 10 版本 1709 |
...
| UIName | 如果目标条件值 (SPN、MCC、MNC、IMSI 范围、ICCID 范围等，则 ) 不能指示唯一项，然后 COSA 会将此名称显示给最终用户，以启用手动选择要使用的 MO 连接值。 | 可选 | 例如， **ContosoMVNO** 为 MVNO。 它没有任何唯一值可以用于将自身与主 MO 网络 **Contoso**区分开来。 在这种情况下，可以使用 **UIName** 并将在 WINDOWS 设置 UI 中显示，使用户能够手动选择要使用的 MO 设置设置。 | Windows 10 版本 1709 |
