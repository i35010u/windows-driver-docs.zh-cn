...
无论采用哪种计划购买选项，都可以根据 SIM 或 CDMA 移动宽带设备的状态进行各种子状态。 冷 Sim （无关联计划）、温 Sim （已准备好接受计划）和热 Sim （已与计划一起使用）可能会根据你想要如何构建购买流程来提供不同的体验。
...
1. 移动宽带服务读取国际移动用户标识（IMSI）、用于 GSM 网络的集成卡 ID （ICCID）、CDMA 网络的提供程序 ID （SID）或 CDMA 网络的提供程序名称，并生成一组硬件 Id （Hwid）。

    **注意**   只有 OEM 尚未插入 SIM 并预加载移动宽带应用和服务元数据时，才需要执行此步骤。

2. 当电脑连接到 Internet 时，Hwid 将发送到 Windows 元数据和 Internet 服务（WMIS）。 WMIS 标识运算符并返回相应的服务元数据包。
...
6. 当移动宽带应用打开时，你应该确保编写应用以建立与后端计费基础结构的安全连接，以便用户可以购买订阅。 此过程仅适用于每个操作员，而 Microsoft 不涉及购买过程。 此应用通过有限的移动宽带连接（需要启用操作员网络）或通过备用 Internet 连接（如 Wi-fi）建立此连接。
...
对于最终用户不应查看的操作员消息，移动宽带应用程序可以处理这些消息并进行相应操作。 Windows 通知服务提供最有效的直接到应用通知通道，但 Windows 还支持从移动宽带网络使用传入短信和非结构化补充服务数据（USSD）通知。
 ...
 **注意**   若要在收到文本消息时向最终用户显示通知 UI，则需要构建具有 SMS 支持的移动宽带应用程序，这可能需要满足特定市场的法规要求或最佳做法。
 ...
 SMS 功能适用于移动宽带应用、有权访问移动网络操作员的 UWP 应用、有权访问移动网络操作员的 UWP 应用（如果移动宽带设备嵌入在 PC 中）或移动宽带设备 IHV （如果移动宽带设备是可移动宽带设备）。 移动网络操作员和 PC OEM （或移动宽带设备 IHV）通过服务元数据指定特权应用。 有关服务元数据的详细信息，请参阅[使用元数据配置移动宽带体验](using-metadata-to-configure-mobile-broadband-experiences.md)。
...
3. 系统事件代理调用移动宽带应用程序来处理后台事件。 （其他触发器，如传入通知，可能导致此情况发生。）移动宽带应用可以选择在为此目的调用时要执行的操作。
...
7. 计算机上的 Windows 组件和第三方应用程序可以使用[**ConnectionProfile**](https://docs.microsoft.com/uwp/api/Windows.Networking.Connectivity.ConnectionProfile)类访问此使用情况信息。 应用可以相应地调整其行为。 例如，应用可以在按流量计费的网络上使用较低质量的视频流。
...
1. Wi-fi 热点中的固定门户网站包括无线 Internet 服务提供商漫游（WISPr）协议中的质询。
...
如果移动宽带应用（动态 WISPr 凭据）提供凭据，或在预配过程中静态定义凭据（静态 WISPr 凭据、EAP 凭据），Windows 会将这些凭据传递到 Wi-fi 热点。
...
当连接到与 Windows 8、Windows 8.1 或 Windows 10 电脑相同的 Wi-fi 或 LAN 网络时，设备（如 DVRs、网关路由器、移动热点和手机）可以使用 Pnp-x 来使 Windows 8、Windows 8.1 和 Windows 10 知道它们的存在。 系统会基于设备的设备属性下载设备元数据，并会自动下载设备元数据。 你可以为这些设备引用此应用程序，以便单个移动宽带应用可以管理移动宽带以及这些其他设备。
...
- 网站在 Windows 8、Windows 8.1 或 Windows 10 计算机上提供的已签名的 XML 文件[**，该文件**](https://docs.microsoft.com/previous-versions/windows/internet-explorer/ie-developer/platform-apis/dn529170(v=vs.85))至少运行 Internet Explorer 10 （或另一个支持的浏览器）。

- 应用程序提供给[**windows.networking.networkoperators. ProvisioningAgent. ProvisionFromXmlDocumentAsync**](https://docs.microsoft.com/uwp/api/windows.networking.networkoperators.provisioningagent.provisionfromxmldocumentasync?view=winrt-19041#Windows_Networking_NetworkOperators_ProvisioningAgent_ProvisionFromXmlDocumentAsync_System_String_)函数的 XML 文件（有符号或无符号）。
...
移动宽带类驱动程序降低了设备制造商为其特定移动宽带设备提供自定义驱动程序的负担。 任何清单为 USB 设备并符合 USB 实现者论坛（USB IF）网络控制模型（NCM）2.0 规范的移动宽带接口都将由移动宽带类驱动程序进行管理，并且不需要下载或安装其他驱动程序。
...
Windows 元数据和 Internet 服务（WMIS）是一种基于云的 Windows 服务，它从参与 Windows 设备生态系统的第三方向 Windows 提供自定义项。 对于移动宽带设备，WMIS 将提供服务元数据包。 这提供了 Windows 从 Microsoft Store 检索移动宽带应用程序所需的基本信息，允许首次连接到网络，并在 Windows 连接管理器中显示相应的品牌元素。
...
有关 UWP 设备应用的详细信息，请参阅[uwp 设备应用](https://docs.microsoft.com/windows-hardware/drivers/devapps/index)。
