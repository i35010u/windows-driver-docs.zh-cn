...
| JSON 元素 | 字段名称 | 描述 | 示例 |
| --- | --- | --- | --- |
| 根对象 | promotionTemplates | 要用于 "增强网关" 页的模板列表。 可以为每个移动运营商定义一个或多个模板。 | 不可用 |
| PromotionTemplate | id | 每个模板的唯一字符串标识符。 仅定义一个模板时的默认值应为 "0" | 123 |
|   | backgroundColor | 网关页面背景的颜色。 此字段是格式为的`0xRRGGBBAA`十六进制字符串。 如果未定义, 将使用白色作为默认值。 | 0x000000FF |
|   | bodyFontColor | 正文文本的颜色。 这是格式为的`0xRRGGBBAA`十六进制字符串。 如果未定义, 将使用黑色作为默认值。 | 0xFFFFFFFF |
|   | bodyText | 客户端语言的本地化正文文本。 请注意, 不能更改免责声明文本。| 随时随地保持连接状态。 |
|   | buttonColor | 启动移动运营商 web 门户的 "继续" 按钮的颜色。 此字段是格式为的`0xRRGGBBAA`十六进制字符串。 如果未定义, 则使用用户选择的系统突出显示颜色作为默认值。 | 0x00B0F0FF |
|   | buttonFontColor | "继续" 按钮上的文本颜色。 此字段是格式为的`0xRRGGBBAA`十六进制字符串。 如果未定义, 将使用白色作为默认值。 | 0xFFFFFFFF |
|   | buttonText | "继续" 按钮的本地化文本。 | 立即开始行动 |
|   | hyperlinkFontColor | 超链接的颜色。 此字段是格式为的`0xRRGGBBAA`十六进制字符串。 如果未定义, 则使用用户选择的系统突出显示颜色作为默认值。 | 0x00B0F0FF |
