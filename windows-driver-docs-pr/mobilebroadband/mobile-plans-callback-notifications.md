...
若要侦听网络注册更改, `MobilePlansInlineProfileDownload.registrationChangedScript`必须将设置为采用字符串`registrationArgs`的 Javascript 函数的名称字符串。
...
| 属性名 | type | 描述 |
| --- | --- | -- |
| networkRegistrationState | String | 一个表示当前网络注册状态的字符串。 此属性的值可在中`MobilePlansNetworkRegistrationState`查看。 |
...
若要侦听配置文件激活事件, `MobilePlansInlineProfileDownload.profileActivationCompleteScript`必须将设置为采用字符串`activationArgs`的 Javascript 函数的名称字符串。
...
| 属性名 | type | 描述 |
| --- | --- | -- |
| activationResult | String | 激活的结果。 此属性的值可在中`MobilePlansActivationError`查看。 |
...
如果配置文件的 ICCID 已知, 还可以将余额添加到非活动配置文件。 `MobilePlansInlineOperations.notifyBalanceAddition`使用 with ICCID 将告知应用余额增加, 并将活动配置文件切换到与所提供的 ICCID 对应的配置文件。
...
| 属性名 | type | 描述 | 示例 |
| --- | --- | --- | --- |
| 用户帐户 | String | 可能值： <ul><li>新建：指示用户创建了一个新的用户帐户。</li><li>现存指示用户使用现有的用户帐户登录。</li><li>帮指示用户在此步骤结束了购买流程。</li><li>None:指示用户没有达到此步骤。</li></ul> | "用户帐户": "New" |
| purchaseInstrument | String | 可能值： <ul><li>新建：指示用户创建了一个新的用户帐户。</li><li>现存指示用户使用现有的用户帐户登录。</li><li>帮指示用户在此步骤结束了购买流程。</li><li>None:指示用户没有达到此步骤。</li></ul> | "purchaseInstrument": "New" |
| line | String | 可能值： <ul><li>新建：指示已由用户帐户添加 SIM 卡。</li><li>现存指示用户已将现有行传输到设备。</li><li>帮指示用户在此步骤结束了购买流程。</li><li>None:指示用户没有达到此步骤。</li></ul> | "line": 新建 " |
| moDirectStatus | 字符串 | 可能值： <ul><li>完成指示用户已成功完成购买。</li><li>ServiceError:指示由于 MO 服务错误, 用户无法完成购买。</li><li>InvalidSIM:指示传递到门户的 ICCID 不正确。</li><li>LogOnFailed: 指示用户无法登录到 MO 门户。</li><li>PurchaseFailed:指示采购因计费错误而失败。</li><li>关于 microsoft.relay指示将无效参数传递到门户。</li>BillingError:指示用户计费出错。</li></ul> | "moDirectStatus": "Complete" |
