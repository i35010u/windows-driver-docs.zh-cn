...
否则为该驱动程序必须取消和完成仅与正在发布文件对象句柄相关联的当前排队的 Irp。 (指向文件对象的指针是否位于**的文件对象**驱动程序的成员[ **IO\_堆栈\_位置**](https://msdn.microsoft.com/library/windows/hardware/ff550659)的 IRP。)取消这些排队 Irp 后，该驱动程序完成清理 IRP 和设置状态\_它 I/O 状态块中的取得成功。

有关处理此请求的详细信息，请参阅[DispatchCleanup 例程](https://msdn.microsoft.com/library/windows/hardware/ff543242)。
