...
下图说明了如何在 I/O 管理器设置[ **IRP\_MJ\_读取**](https://msdn.microsoft.com/library/windows/hardware/ff550794)请求 DMA 传输操作中使用直接 I/O。
...
3.  I/O 管理器或文件系统驱动程序 (FSD) 检查可访问性和调用的用户提供缓冲区[ **MmProbeAndLockPages** ](https://msdn.microsoft.com/library/windows/hardware/ff554664)使用以前创建的 MDL。 **MmProbeAndLockPages**还会填写 MDL 中相应的物理地址范围。
...
5.  如果驱动程序使用基于数据包的系统或总线 master DMA，其[ *AdapterControl* ](https://msdn.microsoft.com/library/windows/hardware/ff540504)例程调用[ **MmGetMdlVirtualAddress** ](https://msdn.microsoft.com/library/windows/hardware/ff554539)与IRP **MdlAddress**用于获取 MDL 的基于页面的条目的基的虚拟地址的指针。

6.  *AdapterControl*例程然后调用[ **MapTransfer** ](https://msdn.microsoft.com/library/windows/hardware/ff554402)使用返回的基址**MmGetMdlVirtualAddress**读取从设备直接插入物理内存的数据。 (有关详细信息，请参阅[适配器对象和 DMA](adapter-objects-and-dma.md)。)
