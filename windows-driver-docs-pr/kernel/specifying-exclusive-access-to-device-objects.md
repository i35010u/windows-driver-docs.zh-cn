...
对于具有这两个 PDO WDM 设备堆栈和 FDO 独占属性仅可通过设置 INF 文件中，通过使用[ **INF AddReg 指令**](https://docs.microsoft.com/windows-hardware/drivers/install/inf-addreg-directive)。 PDO 是在堆栈中的命名的对象，但总线驱动程序 （不是函数驱动程序本身） 创建 PDO，代表功能驱动程序。 指示要独占标志设置为 PDO 的总线驱动程序的唯一方式是由类或设备 INF 文件。 (在调用[ **IoCreateDevice** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdm/nf-wdm-iocreatedevice)例程创建了 FDO; 独占标志设置为 FDO 不起作用。)

可以使用其设备对象不是堆积，如非 WDM 驱动程序和在原始模式下操作的设备驱动程序[ **IoCreateDeviceSecure** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdmsec/nf-wdmsec-wdmlibiocreatedevicesecure)例程，以将其命名的排他属性设置设备对象。
