---
title: 编辑菜单
description: 编辑菜单
ms.assetid: fb3c8b71-93ba-4811-ba86-abc5aef81faa
keywords:
- 编辑菜单 （完整列表）
- 图形界面，编辑菜单
ms.date: 05/23/2017
ms.localizationpriority: medium
ms.openlocfilehash: 6f147d1fe1bd40e2d5ea4617d296bbfb00975e70
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/23/2019
ms.locfileid: "63354911"
---
# <a name="edit-menu"></a>编辑菜单


## <span id="ddk_edit_menu_dbg"></span><span id="DDK_EDIT_MENU_DBG"></span>


此部分将介绍以下命令**编辑**WinDbg 菜单：

[编辑 |剪切](edit---cut.md)

[编辑 |复制](edit---copy.md)

**编辑 |复制已设置格式**

[编辑 |粘贴](edit---paste.md)

[编辑 |选择所有](edit---select-all.md)

[编辑 |向文件写入窗口文本](edit---write-window-text-to-file.md)

**编辑 |将窗口文本复制到剪贴板**

[编辑 |将添加到命令输出](edit---add-to-command-output.md)

[编辑 |清除命令输出](edit---clear-command-output.md)

[编辑 |评估所选内容](edit---evaluate-selection.md)

[编辑 |显示所选的类型](edit---display-selected-type.md)

[编辑 |查找](edit---find.md)

[编辑 |查找下一个](edit---find-next.md)

[编辑 |转到地址](edit---go-to-address.md)

[编辑 |转到行](edit---go-to-line.md)

[编辑 |转到当前指令](edit---go-to-current-instruction.md)

[编辑 |设置当前指令](edit---set-current-instruction.md)

[编辑 |断点](edit---breakpoints.md)

[编辑 |打开/关闭日志文件](edit---open-close-log-file.md)

 

 





