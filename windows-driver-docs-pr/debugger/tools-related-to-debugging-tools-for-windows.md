---
title: 与 Windows 调试工具相关的工具
description: 本部分描述与调试相关但不是包括有关 Windows 调试工具包中的工具。
ms.assetid: A121BF0C-53BF-48E0-9000-3BDA255FD884
ms.date: 06/07/2019
ms.localizationpriority: medium
ms.openlocfilehash: 2084e1a69d5b1d23cb5a1431c310f31712d55b7e
ms.sourcegitcommit: a70dcf63a439d278ae0194733d9fa2adfe496c89
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/07/2019
ms.locfileid: "66813571"
---
# <a name="tools-related-to-debugging-tools-for-windows"></a>与 Windows 调试工具相关的工具

本部分描述与调试相关但不是包括有关 Windows 调试工具包中的工具。

<span id="Application_Verifier"></span><span id="application_verifier"></span><span id="APPLICATION_VERIFIER"></span>[应用程序验证工具](https://docs.microsoft.com/windows-hardware/drivers/devtest/application-verifier)  
监视应用程序操作时在应用程序运行，使用者应用程序到各种压力和测试，并在应用程序执行或设计中生成有关潜在错误的报告。 为 Windows 应用程序验证工具包含在 Windows 软件开发工具包 (SDK)。 您可以找到有关下载和安装 Windows SDK 的 Windows 的信息 [此处](https://go.microsoft.com/fwlink/p?LinkID=271979)。

<span id="Windows_Error_Reporting"></span><span id="windows_error_reporting"></span><span id="WINDOWS_ERROR_REPORTING"></span>[Windows 错误报告](windows-error-reporting.md)  
你可以配置 Windows 错误报告 (WER) 编写用户模式转储文件时在用户模式代码中发生的异常和其他错误。 WER 包含在 Windows Vista 和更高版本的 Windows。

## <a name="span-idrelatedtopicsspanrelated-topics"></a><span id="related_topics"></span>相关主题

[Windows 调试工具中包含的工具](extra-tools.md)

[Windows 调试工具](index.md)
