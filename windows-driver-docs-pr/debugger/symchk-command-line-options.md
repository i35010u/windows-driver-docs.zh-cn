...
## <a name="span-idddk_symchk_command_line_options_dtoolqspanspan-idddk_symchk_command_line_options_dtoolqspanparameters"></a><span id="ddk_symchk_command_line_options_dtoolq"></span><span id="DDK_SYMCHK_COMMAND_LINE_OPTIONS_DTOOLQ"></span>Parameters
...
启用安静模式。 所有输出都将被取消（除非包含 **/ot**选项）。

<span id="_______FileNames______"></span><span id="_______filenames______"></span><span id="_______FILENAMES______"></span>*文件名*   
指定要检查其符号的程序文件。 允许使用绝对路径、相对路径和 UNC 路径。 允许使用星号 **\*** （）通配符。 如果 * 文件名</em>以斜杠结束，则会将其作为目录名称，并且将检查该目录中的所有文件。 如果*文件名*包含空格，则必须用引号将其引起来。
...
指定当前正在执行的程序的名称。 将检查此程序的符号。 *ExeFile*必须包含文件和文件扩展名的名称（通常为 .exe），但不能包含路径信息。 如果有两个具有相同名称的不同可执行文件，则不建议使用此选项。 *ExeFile*可以指定任何程序，包括内核模式驱动程序。 如果*ExeFile*是一个星号（ **\*** ），则 SymChk 将检查所有正在运行的进程（包括驱动程序）的符号。
...
- 如果*ProcessID*为单个星号（ **\*** ），则 SymChk 将检查所有正在运行的进程（包括驱动程序）的符号。
...
指定包含程序文件列表的文本文件。 将检查所有这些程序的符号。 *TextFileList*必须仅指定一个文件（按相对路径、绝对路径或 UNC 路径，但不包含通配符）;如果包含空格，则应该用引号将其引起来。 在此文件中，每行表示一个程序文件（按相对路径、绝对路径或 UNC 路径），并允许使用 **\*** 星号通配符（）。 但是，使用此通配符的任何行都必须使用相对路径。
...
<span id="________s_Opts__SymbolPath"></span><span id="________s_opts__symbolpath"></span><span id="________S_OPTS__SYMBOLPATH"></span> **/s** SymbolPath \[\]  
...
不建议省略 **/s**\[ \] *SymbolPath*参数，但如果省略该参数，则 SymChk 将使用以下默认路径指向公共符号存储区：
...
<td align="left"><p>SymChk 将验证是否从可执行文件中去除了 dbg 信息，并且该信息仅出现在 dbg 文件中，并且可执行文件指向 dbg 文件。 如果程序是在不包含 dbg 符号文件的情况下生成的，则此选项不起作用。 这是默认设置。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>SymChk 不会对 .pdb 文件的内容执行任何检查，只需验证文件是否存在并与二进制文件匹配。 这是默认设置。</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>当 SymChk 检查修补程序 CAB 文件时，它不会在 CAB 内查找符号。 （默认情况下，SymChk 将查找 cab 中的符号以及所提供的符号路径中的符号。）</p></td>
</tr>
<tr class="odd">
<td align="left"><p><strong>/ea</strong><em>文件</em></p></td>
<td align="left"><p>SymChk 不会验证指定文件中列出的程序的符号。 这允许您否决某些程序，否则会进行验证。 <em>文件</em>必须仅指定一个文件（按相对路径、绝对路径或 UNC 路径，但不包含通配符）;如果包含空格，则应该用引号将其引起来。 在<em>文件</em>中，每行表示一个程序文件（按相对路径、绝对路径或 UNC 路径）;不允许使用通配符。 如果此文件中的行包含空格，则应将其括在引号中。 此文件中的分号是注释字符，在分号与行尾之间的所有内容都将被忽略。 如果正在使用符号服务器，则不会将这些程序的符号复制到下游存储区。</p></td>
</tr>
<tr class="even">
