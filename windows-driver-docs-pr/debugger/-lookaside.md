...
如果省略了 *Address* (或 0) 并且未指定 **-all** 选项，则会显示一组众所周知的标准系统检查列表。 列表集并非详尽;也就是说，它不包含所有系统旁的列表。 此外，该集不包含通过调用 [**ExInitializePagedLookasideList**](/windows-hardware/drivers/ddi/wdm/nf-wdm-exinitializepagedlookasidelist) 或 [**ExInitializeNPagedLookasideList**](/windows-hardware/drivers/ddi/wdm/nf-wdm-exinitializenpagedlookasidelist)创建的自定义外观列表。
...
有关 "查找" 列表的信息，请参阅 [使用后备链表列表](../kernel/using-lookaside-lists.md) 和 *Microsoft Windows 内部机制*，并将其标记为 Russinovich 和 David
