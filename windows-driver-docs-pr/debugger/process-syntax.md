...
<tr class="even">
<td align="left"><p><strong>|</strong><em>号</em></p></td>
<td align="left"><p>序数为<em>Number</em>的进程。</p></td>
</tr>
<tr class="odd">
<td align="left"><p><strong>| ~ [</strong><em>PID</em><strong>]</strong></p></td>
<td align="left"><p>进程 ID 为<em>PID</em>的进程。 （括号是必需的，不能在波形符（~）和左括号之间添加空格。</p></td>
</tr>
<tr class="even">
...
当前过程定义了所使用的内存空间和线程集。 调试开始时，当前进程是导致当前异常或调试事件（或调试器附加到的进程）的进程。 该进程将保持当前进程，直到你使用[ **| s （设置当前进程）** ](-s--set-current-process-.md)命令或使用 WinDbg 中的 "[进程和线程" 窗口](processes-and-threads-window.md)指定新的进程。
