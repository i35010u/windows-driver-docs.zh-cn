...
您可以选择并按住 (或右键单击) 书签，以移动到该位置、重命名或删除书签。
...
只需选择 " **视图** " 功能区，然后在上一节中选择 " **强调色** " 选项。 从最近的目标启动未来的会话时，将在目标工作区中保留着色颜色。
...
**文本突出显示** -现在可以在调试器中直接突出显示所选文本的所有实例。 若要使用此功能，只需在命令窗口中选择一些文本，然后在命令功能区中选择 "突出显示"，或按 CTRL + ALT + H。 使用其中一个已突出显示的文本将删除突出显示内容。
...
- 你现在可以通过选择并按住 (或右键) 单击 "命令窗口" 选项卡，然后选择 "关闭自动滚动" 来禁用命令窗口的自动滚动
