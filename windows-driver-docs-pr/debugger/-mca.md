---
title: mca
description: X86 目标计算机 mca 扩展显示计算机检查体系结构 (MCA) 寄存器。 在 Itanium 目标计算机上，mca 扩展显示 MCA 错误记录。
...
## <a name="span-idddkmcadbgspanspan-idddkmcadbgspanparameters"></a><span id="ddk__mca_dbg"></span><span id="DDK__MCA_DBG"></span>参数


<span id="_______Address______"></span><span id="_______address______"></span><span id="_______ADDRESS______"></span> *Address*   
（仅限 Itanium 目标）指定 MCA 错误记录的地址。

<span id="_______Flags______"></span><span id="_______flags______"></span><span id="_______FLAGS______"></span> *标志*   
（仅限 Itanium 目标）指定输出的级别。 *标志*可以是以下位的任意组合。 默认值为 0xff 内，显示日志中的所有部分。
...
此扩展命令只能与基于 x86 或 Itanium 目标计算机。

<a name="remarks"></a>备注
-------

Itanium 目标 **！ mca**显示从系统抽象层 (SAL) 的 MCA 错误记录。 下面是输出的来自此扩展插件示例：
...
X86 目标， **！ mca**显示计算机检查注册 active 处理器支持。 它还会显示 CPU 的基本信息 (与显示的相同[ **！ cpuinfo**](-cpuinfo.md))。 下面是输出的来自此扩展插件示例：
...
请注意，此扩展需要私有 HAL 符号。 不包含这些符号，该扩展将显示消息"HalpFeatureBits 找不到"以及基本的 CPU 信息。 例如：
