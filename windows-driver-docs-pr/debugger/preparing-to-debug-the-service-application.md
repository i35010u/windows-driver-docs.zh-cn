...
*ProgramName*应包含文件扩展名，而不是路径。 例如， *ProgramName*可以是 Myservice 或 Thisservice。
...
-   如果计划从内核模式调试器控制用户模式调试器，请使用-d 选项指定 NTSD。 例如：
    ...
    有关详细信息，请参阅[从内核调试器控制用户模式调试器](controlling-the-user-mode-debugger-from-the-kernel-debugger.md)。
...
**请注意**   如果已启用初始化代码调试（"启用初始化代码调试" 一节中所述的步骤），则应跳过此步骤。 启用初始化代码调试后，调试器将在启动时附加到服务应用程序，这会导致所有崩溃、异常和对**DebugBreak**的调用都路由到调试器，无需额外的准备工作。
...
如果你计划自动启动调试器（当服务启动或遇到异常时），则需要此准备步骤。
...
有时，多个服务会组合到一个服务主机（Svchost）进程中。 如果要调试此类服务，必须首先将其隔离到单独的 Svchost 进程。

可以通过三种方法来隔离服务。 Microsoft 建议将服务移到其自身的组方法，如下所示。 其他方法（更改服务类型和复制 SvcHost 二进制文件）可以临时用于调试，但由于它们改变了服务的运行方式，因此它们不像第一种方法那样可靠。
...
1.  发出以下服务配置工具（Sc.exe）命令，其中*ServiceName*是服务的名称：
    ...
    这会显示服务的当前配置值。 相关值是二进制\_路径\_NAME，它指定用于启动服务控制程序的命令行。 在这种情况下，由于你的服务尚未隔离，因此此命令行将包含目录路径（Svchost.exe）和一些 SvcHost 参数，其中包括-k 开关，后跟组名称。 例如，它可能类似于：
...
2.  找到以下注册表项：
    ...
    使用唯一名称创建一个新的 REG\_多\_SZ 值（例如， **TempGrp**）。
...
6.  使用以下服务配置工具命令来修改在步骤1中找到的路径：
    ...
    在此命令中， *ServiceName*是服务的名称， *RevisedPath*是要为二进制\_路径\_名称提供的新值。 对于*RevisedPath*，使用与步骤1中显示的路径完全相同的路径，包括该行上显示的所有选项，只进行一项更改：将-k 开关后面的参数替换为你在步骤2中创建的新注册表值的名称。 将*RevisedPath*用引号引起来。 需要等号后的空格。
...
1.  Svchost.exe 可执行文件位于 Windows 的 system32 目录中。 复制此文件，将其命名为 svhost2，并将其放入 system32 目录。
...
2.  发出以下服务配置工具（Sc.exe）命令，其中*ServiceName*是服务的名称：
    ...
    此命令显示服务的当前配置值。 相关值是二进制\_路径\_NAME，它指定用于启动服务控制程序的命令行。 在此方案中，由于你的服务尚未隔离，因此此命令行将包含目录路径（Svchost.exe），并且可能会包含一些 SvcHost 参数。 例如，它可能类似于：
...
3.  若要修订此路径，请发出以下命令：
    ...
    在此命令中， *ServiceName*是服务的名称， *RevisedPath*是要为二进制\_路径\_名称提供的新值。 对于*RevisedPath*，使用与步骤2中显示的路径完全相同的路径，包括该行上显示的所有选项，只进行一项更改：将 svchost.exe 替换为 Svchost2。 将*RevisedPath*用引号引起来。 需要等号后的空格。
