---
title: 终止工具
description: Kill 工具 kill 会终止一个或多个进程及其所有线程。 此工具仅适用于在本地计算机上运行的进程。
...
ms.locfileid: "72038086"
---
# <a name="kill-tool"></a>终止工具

Kill 工具 kill 会终止一个或多个进程及其所有线程。 此工具仅适用于在本地计算机上运行的进程。
...
Kill 在[Windows 调试工具](index.md)中包含。
...
### <a name="span-idddk_kill_tool_commands_dtoolsspanspan-idddk_kill_tool_commands_dtoolsspanparameters"></a><span id="ddk_kill_tool_commands_dtools"></span><span id="DDK_KILL_TOOL_COMMANDS_DTOOLS"></span>Parameters

<span id="________f______"></span><span id="________F______"></span> **/f**强制终止进程，而不提示用户进行确认。 需要使用此选项来终止受保护的进程，例如系统服务。

<span id="_______PID______"></span><span id="_______pid______"></span>*PID*指定要终止的任务的进程标识符（PID）。

若要查找任务的 PID，请在 Microsoft Windows XP 和更高版本或 Windows 2000 的[tlist.exe](tlist.md)中使用 TaskList。

<span id="_______Pattern_"></span><span id="_______pattern_"></span><span id="_______PATTERN_"></span><em>模式</em> **\***  
指定任务或窗口的全部或部分名称。 Kill 工具终止进程名称或窗口名称与模式匹配的所有进程。 星号是必需的。

在使用可能会无意中匹配多个进程或窗口名称的模式之前，请使用**tlist.exe** *模式*命令测试模式。 有关详细信息，请参阅[tlist.exe](tlist.md) 。
...
以下命令将终止进程 ID （PID）为2520的进程：

```console
kill 2520
```

以下命令终止名称以 "my @ no__t-0" 开头的进程。 它不提示确认。 即使此进程是系统服务，此命令也会成功：
...
[适用于 Windows 的调试工具中包含的工具](extra-tools.md)
