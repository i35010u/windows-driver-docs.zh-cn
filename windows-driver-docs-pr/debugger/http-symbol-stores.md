...
将 IIS 配置为符号存储相对容易，因为符号文件只作为静态文件提供。 唯一的非默认设置是 MIME 类型的配置，以允许将符号文件作为二进制流下载。 为此，可以使用应用于符号文件夹的虚拟目录的 "\*" 通配符。
...
首先选择将用作符号存储区的目录。 在我们的示例中，我们将此目录 c：\\symstore，并且网络上服务器的名称 \\SymMachineName。
...
可以编辑 web.config 文件，以配置符号的 MIME 类型。 此方法会清除继承的 MIME 类型，并 \* MIME 类型中添加全部捕获通配符。 当在某些 IIS 配置中继承 MIME 类型时，可能需要此方法。
