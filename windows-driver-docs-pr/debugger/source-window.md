...
-   在启动调试器之前，请将 \_NT\_源\_路径[环境变量](environment-variables.md)。
...
-   从 "**文件**" 菜单中选择 "**打开源文件**"，或按 CTRL + O。 你还可以使用工具栏上**的 "打开源文件"** 按钮（![屏幕截图](images/tbopen.png)）。

    **请注意**  在使用菜单或工具栏按钮打开源文件时，该文件的路径会自动追加到源路径。
...
-   **编辑此文件**将在文本编辑器中打开源文件。 编辑器由 WinDiff 编辑器注册表信息或 WINDBG\_调用\_编辑器环境变量的值确定。 例如，在以下情况下，请考虑 WINDBG\_调用\_编辑器的值。
...
-   **选择源语言**显示一系列编程语言。 选择用于生成源文件的编程语言，然后单击 **"确定"** 以为当前源窗口启用基本语法突出显示。 选择 " **&lt;无"&gt;** 可禁用当前源窗口的语法突出显示。
