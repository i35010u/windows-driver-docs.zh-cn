---
title: 激活智能客户端（内核模式）
description: 一旦已激活 KD 连接服务器，可以在另一台计算机上创建智能客户端并开始调试会话。
ms.assetid: bf0f1dd5-e6dc-4168-8476-cf21e77bd335
keywords:
- 激活调试智能客户端 （内核模式） Windows
ms.date: 05/23/2017
topic_type:
- apiref
api_name:
...
一旦已激活 KD 连接服务器，可以在另一台计算机上创建智能客户端并开始调试会话。

有两种方法来启动智能客户端： 通过使用内核协议启动 KD 或 WinDbg **kdsrv**，或使用 WinDbg 图形界面。

需要指定使用 KD 连接服务器的远程传输协议。 此外可以指定 KD 连接服务器和目标计算机之间的实际内核连接的协议，或者可以使用默认值。

正在启动智能客户端的常规语法取决于所使用的协议。 可设置的选项包括：
...
若要使用图形界面来连接到 KD 连接服务器，WinDbg 必须处于休眠模式-它必须或者已启动任何命令行参数，或它必须结束上一个调试会话。 选择**文件 |连接到远程存根 （stub）** 菜单命令。 当**连接到远程的存根 （stub） 服务器**出现对话框，请输入以下字符串之一**连接字符串**文本框中：
...
或者，可以使用**浏览**按钮以找到活动 KD 连接服务器。 请参阅[文件 |连接到远程存根 （stub）](file---connect-to-remote-stub.md)有关详细信息。
...
这是计算机的网络名称或 IP 地址创建 KD 连接服务器。这两个初始反斜杠 (\\ \)是可选的命令行中，但不是允许在 WinDbg 对话框中。
...
如果使用 NPIPE 或 SPIPE 协议，则*PipeName*是创建 KD 连接服务器时提供给管道的名称。
...
此命令成功后，你可以使用激活智能客户端 **-k kdsrv**或通过使用 WinDbg 图形界面。
...
如果使用 TCP 或 SSL 协议，则*套接字*是创建 KD 连接服务器时使用的同一套接字端口号。

<span id="clicon"></span><span id="CLICON"></span>**clicon**  
指定 KD 连接服务器将尝试连接到智能客户端通过反向连接。 客户端必须使用**clicon**当且仅当服务器使用的**clicon**。 在大多数情况下，智能客户端启动 KD 连接服务器之前，使用反向连接时。
...
如果使用 COM 协议，则*BaudRate*应匹配选择创建 KD 连接服务器时的波特率。
...
如果使用 COM 协议，则*COMChannel*应选择创建 KD 连接服务器时的频道号匹配。
...
如果使用 SSL 或 SPIPE 协议，则*协议*应与创建 KD 连接服务器时使用的安全协议。

<span id="________Cert"></span><span id="________cert"></span><span id="________CERT"></span> *Cert*  
如果使用 SSL 或 SPIPE 协议，则应使用完全相同**certuser =**<em>Cert</em>或**machuser =**<em>Cert</em>时的参数时使用创建 KD 连接服务器。
...
如果创建 KD 连接服务器时使用的密码*密码*必须以创建智能客户端提供。 它必须与匹配的原始密码。 密码是区分大小写。 如果提供了错误的密码，则错误消息将指定"错误 0x80004005。"
...
有关这些协议的信息，请参阅[获取设置以便进行调试](getting-set-up-for-debugging.md)。 可以省略任何参数对这些协议-例如，您可以说**trans=@{com:}** -，调试器将默认为 KdSrv 正在其中运行的计算机上的环境变量指定的值。
...
由于 KD 连接服务器仅充当网关的智能客户端的额外*选项*将与所使用内核调试程序开始在计算机上运行 KdSrv 位置相同。 此规则的例外是任何选项，指定的路径或文件名将被计算机上的路径作为智能客户端在何处运行。
