...
为*PoolTag*输入四字符模式，如 Tag1。 它可以包含 **？** （替换为任意单个字符），并 **\*** （替换为多个字符）通配符。 例如，Fat \* 或 Av？4。 池标记始终区分大小写。
...
若要启用对象引用跟踪，则必须在命令中至少包含一个池标记（ **/T** *PoolTag*）或一个映像文件（ **/i** ImageFile）。 有关详细信息，请参阅[示例15：使用对象引用跟踪](example-15--using-object-reference-tracing.md)。
...
检查不同步的访问。 如果此参数检测到使用堆创建的堆 \_NO \_SERIALIZE 标志由不同的线程访问，则会导致中断。
...
GFlags 中的页堆功能仅监视使用标准 Windows 堆管理器函数（**HeapAlloc**、 **GlobalAlloc**、 **LocalAlloc**、 **malloc**、 **new**、 **new \[ \]** 的堆内存分配，或它们对应的释放函数）或使用调用标准堆管理器函数的自定义操作的函数。
