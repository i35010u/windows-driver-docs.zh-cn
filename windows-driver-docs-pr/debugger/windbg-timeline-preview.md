...
![调试器中的时间线](images/windbgx-timeline.png)
...
![调试器中显示异常的时间线](images/windbgx-timeline-exceptions.png)
...
![调试器中显示断点的时间线](images/windbgx-timeline-breakpoints.png)
...
![调试器中显示函数调用的时间线](images/windbgx-timeline-function-calls.png)
...
![调试器中的时间线，显示用于选择活动视图端口的顶部区域](images/windbgx-timeline-manipulation.png)
...
[第1节：生成示例代码](https://docs.microsoft.com/windows-hardware/drivers/debugger/time-travel-debugging-walkthrough#build)

[第2部分：记录 "DisplayGreeting" 示例的跟踪](https://docs.microsoft.com/windows-hardware/drivers/debugger/time-travel-debugging-walkthrough#record)
...
在命令输出中，请注意堆栈（esp）和基指针（ebp）指向两个不同的地址。 这可能表示堆栈损坏，可能是因为函数返回，然后损坏堆栈。 若要对此进行验证，需要在 CPU 状态损坏之前返回到，并查看是否可以确定发生堆栈损坏的时间。
...
如前文所述，基指针（esp）（而不是指向指令）指向我们的消息文本。
...
尽管我们将使用简单的内存访问断点，但可以将断点构建为更复杂的条件语句。 有关详细信息，请参阅[bp、bu、bm.exe （设置断点）](bp--bu--bm--set-breakpoint-.md)。
...
由于 Microsoft 提供的 wscpy_s （）函数不太可能出现类似于下面的代码 bug，我们将在堆栈中进一步进行查找。 此堆栈显示问候语！ main 拨打问候语！GetCppConGreeting. 在我们非常小的代码示例中，现在可以打开代码，这可能会非常容易地发现错误。 但为了阐释可用于更大、更复杂的程序的技术，我们将设置 "添加函数调用时间线"。
...
![调试器中显示内存访问时间线和本地窗口的时间线](images/windbgx-timeline-demo-lab-function-dialog.png)
...
![调试器中显示内存访问时间线和本地窗口的时间线](images/windbgx-timeline-demo-lab-function.png)
