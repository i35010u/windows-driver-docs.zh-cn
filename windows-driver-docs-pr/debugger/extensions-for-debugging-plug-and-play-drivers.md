...
[**!arbiter**](-arbiter.md)  
显示当前的系统资源仲裁器。 仲裁器是代码的一种公开由总线驱动程序将进行仲裁的资源，请求和尝试解决的那些设备连接到该总线上的资源冲突。

[**!cmreslist**](-cmreslist.md)  
显示 CM\_资源\_为指定的设备对象的列表。
...
这将显示与此 CM 资源列表设备使用 I/O 范围 3F8 3FF 和 IRQ 4。

[**!dcs**](-dcs.md)  
此扩展已过时-其功能已经融入到由[ **！ pci**](-pci.md)。 请参阅 ！ pci 100 本部分后面的示例。

[**!devext**](-devext.md)  
显示特定于总线的设备使用不同的设备的扩展信息。
...
[**!drivers**](-drivers.md)  
[ **！ 驱动程序**](-drivers.md)命令不再受支持。 请使用[ **lm t n** ](lm--list-loaded-modules-.md)命令。
...
（x86 目标计算机仅）将一系列值写入到 PCI 配置空间。

[**ib，信息工作者，id**](-ib---id---iw.md)  
从 I/O 端口读取数据。

这三个命令可用于确定是否由正在调试的驱动程序以外的其他设备声明某个特定 I/O 范围。 一个字节值 0xFF 端口上的指示端口未使用。

[**!ioreslist**](-ioreslist.md)  
显示指定的 IO\_资源\_要求\_列表。

[**!irp**](-irp.md)  
显示有关 IRP 的信息。

[**!irpfind**](-irpfind.md)  
显示有关当前分配的目标系统中的所有 Irp 的信息或有关其字段与指定的搜索条件匹配这些 Irp 的信息。

[**!pci**](-pci.md)  
（x86 目标计算机仅）显示 PCI 总线和任何连接的设备的当前的状态。 它还可以显示 PCI 配置空间。
...
下面的示例显示 SCSI 控制器 （总线 1，9，函数 0 设备） 的 PCI 配置空间：
...
[**!pcitree**](-pcitree.md)  
显示有关 PCI 设备对象，包括子 PCI 总线和 CardBus 总线，以及连接到它们的设备的信息。

[**!pnpevent**](-pnpevent.md)  
显示即插即用设备事件队列。

[**!rellist**](-rellist.md)  
显示即插即用关系列表，以及任何相关的 CM\_资源\_列表和 IO\_资源\_列表结构。
