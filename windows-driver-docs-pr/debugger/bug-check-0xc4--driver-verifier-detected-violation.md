...
|0x00020006 |指向描述违反规则条件的字符串的指针。|指向规则状态变量的可选指针)  (s。|保留|驱动程序违反了 DDI 相容性规则 [IrqlExApcLte2](../devtest/wdm-irqlexapclte2.md)。 IrqlExApcLte2 规则指定只有当 IRQL <= APC_LEVEL 时，驱动程序才调用特定例程。|
