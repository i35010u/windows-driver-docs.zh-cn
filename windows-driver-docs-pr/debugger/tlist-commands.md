...
## <a name="span-idddk_tlist_commands_dtoolsspanspan-idddk_tlist_commands_dtoolsspanparameters"></a><span id="ddk_tlist_commands_dtools"></span><span id="DDK_TLIST_COMMANDS_DTOOLS"></span>Parameters
...
*ProcessName*是进程的名称（带有或不带文件扩展名），而不是模式。
...
在其进程的详细显示（**Tlist.exe** *PID*或**Tlist.exe** *模式*）中，tlist.exe 显示以下信息。
...
-   进程中运行的线程的列表。 对于每个线程，Tlist.exe 将显示线程 ID （TID）、线程正在运行的函数、入口点地址、最后报告的错误的地址（如果有）以及线程状态。
