...
对于此练习中，我们将假定生成的应用程序 (MyApp.exe) 和符号文件 (MyApp.pdb) 位于 c:\\MyApp\\x64\\调试。 我们还假设 c： 驱动器中为应用程序源代码\\MyApp\\MyApp。
