...
-   数字值被解释为当前基数 （16、 10 或 8） 中的数字。 若要更改默认基数，请使用[ **n (设置数量 Base)** ](n--set-number-base-.md)命令。 可以通过指定重写默认基数**0x**前缀 （十六进制） **0n**前缀 （十进制） **0t**前缀 （八进制） 或**0y**前缀 （二进制）。
    **请注意**  的默认基数以不同方式行为时使用C++表达式。 有关这些表达式和基数的详细信息，请参阅[评估表达式](evaluating-expressions.md)。
...
**S-v**命令与相同的数据类型的对象的搜索*对象*对象。 可以使用此命令，仅当所需的对象为C++类或具有虚函数表 (Vtable) 相关联的另一个对象。 **S-v**命令搜索*范围*内存区域中针对此类的 Vtable 的地址。 如果多个 Vtable 存在此类中，搜索算法将查找所有这些指针值，分隔适当数目的字节数。 如果找到任何匹配项，调试器将返回的对象和有关此对象-类似的输出的完整信息的基址[ **dt （显示类型）** ](dt--display-type-.md)命令。
