...
本主题列出了具有简单 i/o 插件的设备类型，并指示是否存在测试设备的特定要求。 使用[Windows 硬件实验室工具包（WINDOWS HLK）](https://docs.microsoft.com/windows-hardware/test/hlk/)时需要遵循这些要求。 本主题还介绍了如何对测试失败进行故障排除和会审。
...
### <a name="type-of-io-plug-in-performs-cdrom"></a>I/o 插件的类型执行（CDROM）

- 在 cd-rom 上查找文件，并使用 Win32 [**ReadFile**](https://docs.microsoft.com/windows/desktop/api/fileapi/nf-fileapi-readfile) API 执行读取操作。

### <a name="how-to-triage-test-failures-cdrom"></a>如何对测试失败进行会审（CDROM）
...
#### <a name="type-of-io-plug-in-performs-disk"></a>I/o 插件的类型（磁盘）
...
### <a name="type-of-io-plug-in-performs-display"></a>I/o 插件的类型（显示）
...
### <a name="type-of-io-plug-in-performs-gps"></a>I/o 插件的类型（GPS）

- 对[传感器](#sensors)使用 i/o 插件。

## <a name="lan"></a>LAN

### <a name="requirements-lan"></a>要求（LAN）
...
### <a name="type-of-io-plug-in-performs-lan"></a>I/o 插件的类型执行（LAN）

- Ping IPv6 网络网关地址。

### <a name="how-to-triage-test-failures-lan"></a>如何对测试失败进行会审（LAN）
...
### <a name="requirements-mobile-broadband"></a>要求（移动宽带）

- 没有特殊的测试要求。

### <a name="type-of-io-plug-in-performs-mobile-broadband"></a>I/o 插件的类型执行（移动宽带）
...
### <a name="how-to-triage-test-failures-mobile-broadband"></a>如何对测试失败进行会审（移动宽带）
...
### <a name="requirements-portable-devices"></a>要求（可移植设备）

- 设备具有可在其中创建文件夹和文件的存储组件。

### <a name="type-of-io-plug-in-performs-portable-devices"></a>I/o 插件的类型执行（可移植设备）

- 使用 WPD Api 读取文件并将其写入 WPD 设备上的存储组件。

## <a name="smart-card-readers"></a>智能卡读卡器

### <a name="requirements-smart-card-readers"></a>要求（智能卡读卡器）

- 设备已插入 Athena T0 测试卡。

### <a name="type-of-io-plug-in-performs-smart-card-readers"></a>I/o 插件的类型执行（智能卡读卡器）

- 读取数据并将数据写入读卡器中插入的 Athena T0 卡。

## <a name="sensors"></a>Sensors

### <a name="requirements-sensors"></a>要求（传感器）

- GPS 设备必须在具有适当 GPS 信号的位置进行测试。

## <a name="volume"></a>数据量(Volume)

### <a name="requirements-volume"></a>要求（卷）
...
### <a name="how-to-triage-test-failures-volume"></a>如何对测试失败进行会审（卷）
...
### <a name="requirements-wlan"></a>要求（WLAN）

- 请参阅 HCK 文档中[由设备基础测试记录的 WLAN SimpleIO 插件故障疑难解答](https://go.microsoft.com/fwlink/p/?linkid=309556)。

### <a name="type-of-io-plug-in-performs-wlan"></a>I/o 插件的类型执行（WLAN）

- 请参阅 HCK 文档中[由设备基础测试记录的 WLAN SimpleIO 插件故障疑难解答](https://go.microsoft.com/fwlink/p/?linkid=309556)。

### <a name="how-to-triage-test-failures-wlan"></a>如何对测试失败进行会审（WLAN）

- 请参阅 HCK 文档中[由设备基础测试记录的 WLAN SimpleIO 插件故障疑难解答](https://go.microsoft.com/fwlink/p/?linkid=309556)。

## <a name="usb-controller-and-hub-with-mutt"></a>USB 控制器和集线器与 Mutt

### <a name="requirements-usb"></a>要求（USB）
...
### <a name="how-to-triage-test-failures-usb"></a>如何对测试失败进行会审（USB）
...
在运行以下[设备基础测试](https://docs.microsoft.com/windows-hardware/drivers)之前，必须根据本主题中所述的针对特定设备类型的要求配置测试计算机上的设备。
