...
简单 I/O 插件是实现泛型特定于设备的 I/O 功能扩展到 Windows 驱动程序测试框架 (WDTF)。 如果所测试的设备类型的插件存在[设备基础测试](https://msdn.microsoft.com/windows-drivers/develop/how_to_select_and_configure_the_device_fundamental_tests)使用测试 I/O 的 WDTF 简单 I/O 接口。
...
-   [Webcam](#-webcam)
-   [WLAN](#-wlan)
-   [USB 控制器和使用 Mutt 中心](#-usb-mutt)

具有特定要求的设备基础测试的列表，请参阅[具有特定设备的配置要求的设备基础测试](#-devfund-test-req)
...
-   使用[ **BluetoothFindFirstDevice 函数**](https://msdn.microsoft.com/library/windows/desktop/aa362784)查找 Bluetooth 的设备。
...
-   查找在 CD-ROM 上的文件，并执行读取的操作使用 Win32 [ **ReadFile** ](https://msdn.microsoft.com/library/windows/desktop/aa365467) API。

**如何进行会审测试失败：**

-   在测试计算机上，导航到相关的 CD/DVD 驱动器并确认可以访问的驱动器的内容。
-   要用于执行读取从 CD/DVD 上的文件 CD-Rom 简单 I/O 插件搜索。 请确保 CD/DVD 具有编码在磁盘上的文件。
-   此简单 I/O 即插即用使用 Win32 [ **CreateFile**](https://msdn.microsoft.com/library/windows/desktop/aa363858)， [ **WriteFile**](https://msdn.microsoft.com/library/windows/desktop/aa365747)， [ **ReadFile**](https://msdn.microsoft.com/library/windows/desktop/aa365467)函数。 返回的错误是最有可能从这些 Api 的 Win32 错误代码。
...
-   使用[ **IMbnInterface 接口**](https://msdn.microsoft.com/library/windows/desktop/dd430406) ，并调用 GetHomeProvider， [ **IMbnInterface::GetInterfaceCapability 方法**](https://msdn.microsoft.com/library/windows/desktop/dd323100)，和[**IMbnInterface::GetReadyState 方法**](https://msdn.microsoft.com/library/windows/desktop/dd323102) Api 来执行该设备。
...
-   创建一个名为 WDTF 目录\_卷\_IO，并创建一个名为 SimpleIO.tmp 文件。 通过调用来执行 I/O [ **ReadFile** ](https://msdn.microsoft.com/library/windows/desktop/aa365467)并[ **WriteFile** ](https://msdn.microsoft.com/library/aa365747) Api 对此文件。
...
-   测试没有特殊要求。
    ...
    **请注意**简单 I/O 插件适用于网络摄像头设备上的 Windows Media Player 和相关的技术，例如 Windows 7 N 或 Windows 7 KN 不包含版本不可用的 MFPlat.dll 文件具有依赖关系。 在这些版本的 Windows 上，必须安装媒体功能包。 媒体功能包是可供下载。 有关详细信息，请参阅[KB 文章 968211](https://go.microsoft.com/fwlink/p/?linkid=266437)。
...
运行以下之前[设备基础测试](https://msdn.microsoft.com/windows-drivers/develop/how_to_select_and_configure_the_device_fundamental_tests)，必须根据特定设备类型所述的要求配置在测试计算机上的设备。 请参阅列表[提供简单的 I/O 插件和设备要求。](#-provided-io-plugin-list)
...
[故障排除设备基础测试](https://msdn.microsoft.com/windows-drivers/develop/troubleshooting_the_device_fundamental_tests)  
