...
设备维护接口要求双向请求为字符串形式的 XML 数据。 可以在应用中有意义的地方构造双向请求。 例如，可以将双向请求保存为字符串常量，或根据用户输入动态创建它们。 [打印作业管理和打印机维护](https://go.microsoft.com/fwlink/p/?LinkID=299829)示例在方法中构建默认请求时发生 `OnNavigatedTo` 。 有关双向的详细信息，请参阅 [双向通信](https://go.microsoft.com/fwlink/p/?LinkId=317192)。
...
5. 编辑并保存设备元数据。 若要将设备应用链接到设备，你必须将设备应用关联到设备。 **注意：** 如果尚未创建设备元数据，请参阅 [为 UWP 设备应用创建设备元数据](https://go.microsoft.com/fwlink/p/?LinkId=313644)。
