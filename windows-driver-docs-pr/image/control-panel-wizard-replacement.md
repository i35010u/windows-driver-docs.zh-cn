...
没有任何关系在所有自定义向导和对话框中使用替换[ **IWiaUIExtension::DeviceDialog** ](https://msdn.microsoft.com/library/windows/hardware/ff545069)接口。 但是，请小心。 在控制面板中的图标将始终默认为 Windows Me 和 Windows XP 中的向导。 用户需要右键单击并选择扫描显式以显示默认 WIA\_事件\_扫描\_映像事件处理程序。 在我的电脑，反过来也成立;无论您选择的默认处理程序将是用户双击设备图标上时使用。
