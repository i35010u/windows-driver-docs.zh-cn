...
<td><p>确定指定的属性规范标识符（ID）是否包含在此类值的数组中。 函数可以选择获取在其中找到属性规范 ID 的索引。</p></td>
</tr>
<tr class="odd">
...
<td><p>根据当前的 WIA_IPA_FORMAT 设置（在 Microsoft Windows SDK 文档中定义），计算图像的大小和宽度（以字节为单位），并将新值写入适当的属性。</p></td>
</tr>
<tr class="even">
