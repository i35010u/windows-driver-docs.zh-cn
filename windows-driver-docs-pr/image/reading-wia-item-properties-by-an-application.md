---
title: 读取应用程序的 WIA 项属性
description: 读取应用程序的 WIA 项属性
...
ms.locfileid: "56525165"
---
# <a name="reading-wia-item-properties-by-an-application"></a>读取应用程序的 WIA 项属性





当应用程序发出请求以读取 WIA 项属性时，WIA 服务调用[ **IWiaMiniDrv::drvReadItemProperties** ](https://msdn.microsoft.com/library/windows/hardware/ff545005)方法。
...
2.  通过调用更新当前值[ **wiasWriteMultiple** ](https://msdn.microsoft.com/library/windows/hardware/ff549475)或 **wiasWriteProp * * * Xxx*服务函数，请使用 WIA 属性 id。 这会更新存储在驱动程序项，WIA 属性集和 WIA 服务返回应用程序的新值。
...
**IWiaMiniDrv::drvReadItemProperties**时应用程序尝试读取 WIA 项的属性调用方法。 WIA 服务首先通过调用此方法通知驱动程序。 WIA 驱动程序应验证所读取的属性正确。 这是最好访问需要设备状态的属性的硬件 (如[ **WIA\_DPS\_文档\_处理\_状态**](https://msdn.microsoft.com/library/windows/hardware/ff551386)，或[ **WIA\_DPA\_设备\_时间**](https://msdn.microsoft.com/library/windows/hardware/ff550303)如果设备支持时钟)。

请务必注意 WIA 驱动程序应与硬件通信仅在少数情况下。 与硬件通信过很多此调用中将显示缓慢和较慢的 WIA 驱动程序。

下面的示例演示的实现[ **IWiaMiniDrv::drvReadItemProperties** ](https://msdn.microsoft.com/library/windows/hardware/ff545005)方法：
