...
WIA 平板驱动程序通过调用中 microdriver 的 WIA microdriver 函数来处理来自 WIA 服务请求。 Microdriver 必须实现每个函数。 一个[ **SCANINFO** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wiamicro/ns-wiamicro-_scaninfo)结构传递给 microdriver 来存储和通信扫描参数，例如扫描窗口和解决方法。 WIA 平板驱动程序从 SCANINFO 结构中，读取值，但永远不会将其写入。 它负责 microdriver 设置 SCANINFO 成员。

Microdriver 不应存储一次扫描，任何参数，但应依赖于存储中的值[ **SCANINFO** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wiamicro/ns-wiamicro-_scaninfo)结构。 这一点对于支持多个应用程序访问设备。 如果在同一时间将两个应用程序设置了在同一设备上扫描，则只有一份运行 microdriver。 在此情况下使用任一具体取决于应用程序正在尝试访问设备的两个不同 SCANINFO 结构调用 microdriver。
