...
以下示例实现[ **IWiaMiniDrv::drvDeviceCommand** ](https://msdn.microsoft.com/library/windows/hardware/ff543967)方法演示了如何响应发送给它的 WIA 服务命令并传递到设备命令 WIA 微型驱动程序。 WIA 微型驱动程序将向设备发出命令后，微型驱动程序通知设备项树已更改的应用程序。 在此实现中，该方法确定 WIA 服务已发出"拍照"命令 (WIA\_CMD\_采取\_图片)。 方法调用**TakePicture**方法上根项 （设备项），并通知任何连接的应用程序项树现在包含新的图片。 (这两个 WIA\_CMD\_采取\_图片并**TakePicture** Microsoft Windows SDK 文档中所述。)微型驱动程序将这是通过调用[ **wiasQueueEvent** ](https://msdn.microsoft.com/library/windows/hardware/ff549296)函数。
