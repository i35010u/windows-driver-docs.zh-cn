...
-   框架和驱动程序进行通信使用功能交换过程及其各自的版本。 这些 Ioctl 视为特殊，因为它们通信使用的版本字段及其版本。 因此，关于设备和平台功能检查的实现应显式检查版本返回第一次，并将其存储的使用情况。 版本隶属[ **GNSS\_设备\_功能**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/gnssdriver/ns-gnssdriver-gnss_device_capability)结构进行通信的驱动程序的版本号。 版本隶属[ **GNSS\_平台\_功能**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/gnssdriver/ns-gnssdriver-gnss_platform_capability)结构通信 GNSS 适配器的版本号。
