...
| 对 MultipleAppSessions 的支持 | 建议 |  |  |
|  (IHV 特定) 的 GNSS 协助支持 | 建议 | 必需 |  |
| 通过 Microsoft (使用 Agss_inject IOCTLs) 获取 GNSS 援助支持 | 建议 |  |  |
| 支持完整 GNSS_FixData 结构 | 必需 |  |  |
| 单次拍摄会话 | 必需 |  |  |
| 基于时间的跟踪会话本机支持 | 可选 |  | 如果支持，则必须包含修改会话参数的支持。 |
| 基于距离的跟踪会话本机支持 | 可选 |  | 如果支持，则必须包含修改会话参数的支持。 |
| 上一个已知良好的会话 | 可选 |  |  |
| 地理围栏本机支持 | 可选 | 建议 | 仅要求和支持循环现成 |
| 提供 ChipsetInfo | 必需 |  | 使用 GNSS_ChipsetInfo |
| 报告错误 | 建议 |  | 使用 GNSS_ErrorInfo |
| 通过 NMEA 报告 | 可选 |  |  |
| 运营商波浪或自检)  (的生产测试支持 | 可选 |  |  |
| 与 MBB 的集成的控制平面位置 | 只有移动运营商要求时才是必需的 | 必需 | 通常，移动运营商需要语音支持的设备。 手机几乎始终需要。 |
| SUPL 1。0 | 只有移动运营商要求时才是必需的 |  | 一般替换为 SUPL 2.0。<br><br>包括实现完整的客户端会议移动运营商要求，通过 DDI 进行配置，通过 DDI 将 NI 事件报告给操作系统，并与 MBB 集成。 |
| SUPL 2。x | 只有移动运营商要求时才是必需的 | 必需 | 通常，移动运营商需要语音支持的设备。 手机几乎始终需要。<br><br>包括实现完整的客户端会议移动运营商要求，通过 DDI 进行配置，通过 DDI 将 NI 事件报告给操作系统，并与 MBB 集成。 |
| UPL | 只有移动运营商要求时才是必需的 |  | 如果移动运营商要求，则只需要在中国的 CDMA 设备上提供支持。<br><br>包括实现完整的客户端会议移动运营商要求，通过 DDI 进行配置，通过 DDI 将 NI 事件报告给操作系统，并与 MBB 集成。 |
| GNSS_SetLocationServiceEnabled 驱动程序命令 | 必需 |  |  |
| GNSS_SetLocationNIRequestAllowed 驱动程序命令 | 仅当移动运营商支持和需要 SUPL 时才是必需的 |  | 没有已知的移动运营商需要此操作 |
| GNSS_ForceSatelliteSystem 驱动程序命令 | 建议 |  | 适用于测试目的。 某些移动运营商或 Oem 可能需要此进行测试。 |
| GNSS_ForceOperationMode 驱动程序命令 | 仅当支持 SUPL 时才是必需的 |  | 某些移动运营商可能需要用于测试目的。 |
| GNSS_ResetEngine 驱动程序命令 | 必需 |  | 用于测试目的 |
| GNSS_ClearAgnssData 驱动程序命令 | 必需 |  | 用于测试目的 |
| GNSS_SetNMEALogging 驱动程序命令 | 可选 |  | 仅当移动运营商或 Oem 需要此方法进行测试时 |
| GNSS_SetSuplVersion 驱动程序命令 | 仅当支持 SUPL 时才是必需的 |  | SUPL 必需的 |
| GNSS_SetUplServerAccessInterval 驱动程序命令 | 仅当移动运营商支持和需要 SUPL 时才是必需的 |  | 仅在移动运营商需要时 |
| GNSS_SetNiTimeoutInterval 驱动程序命令 | 仅当移动运营商支持和需要 SUPL 时才是必需的 |  | 仅在移动运营商需要时 |
| GNSS_ResetGeofencesTracking 驱动程序命令 | 仅当支持地理围栏时才是必需的 |  |  |
| GNSS_CustomCommand 驱动程序命令 | 可选 |  |  |
