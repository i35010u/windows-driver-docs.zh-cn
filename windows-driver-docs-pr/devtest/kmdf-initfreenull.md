...
ms.locfileid: "85916990"
---
# <a name="initfreenull-rule-kmdf"></a>InitFreeNull 规则（kmdf）


**InitFreeNull**规则指定 \_ 无法使用指向[WDFDEVICE \_ INIT](https://docs.microsoft.com/windows-hardware/drivers/wdf/wdfdevice_init)结构的**NULL**指针调用作为参数的 DDIs 接收 PWDFDEVICE init。

框架提供的方法初始化[WDFDEVICE \_ INIT](https://docs.microsoft.com/windows-hardware/drivers/wdf/wdfdevice_init)结构。 当驱动程序调用[**WdfDeviceCreate**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdfdevice/nf-wdfdevice-wdfdevicecreate)来创建功能设备对象（FDO）或物理设备对象（PDO）时，如果成功，则**WdfDeviceCreate**方法会将第一个参数设置为**NULL** 。

如果驱动程序在调用设备对象初始化方法或[**WdfDeviceCreate**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdfdevice/nf-wdfdevice-wdfdevicecreate)时遇到错误，则驱动程序必须调用 WdfDeviceInitFree。 成功调用 WdfDeviceInitFree 后，应将指向[WDFDEVICE \_ init](https://docs.microsoft.com/windows-hardware/drivers/wdf/wdfdevice_init)结构的指针设置为**null** （PWDFDEVICE \_ init =**null**）。
...
<tbody>
<tr class="odd">
<td align="left"><p>运行<a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">静态驱动程序验证程序</a>并指定<strong>InitFreeNull</strong>规则。</p>
使用以下步骤来运行代码分析：
<ol>
