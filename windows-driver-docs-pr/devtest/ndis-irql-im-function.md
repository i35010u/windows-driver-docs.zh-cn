...
description: Irql\_IM\_函数规则指定用于中间（IM）驱动程序的 NDIS 函数必须以正确的 IRQL 级别进行调用。
...
ms.locfileid: "72839392"
---
# <a name="irql_im_function-rule-ndis"></a>Irql\_IM\_函数规则（ndis）


Irql\_IM\_函数规则指定用于中间（IM）驱动程序的 NDIS 函数必须以正确的 IRQL 级别进行调用。
