---
title: 其他规则集 (KMDF)
description: 使用这些规则来验证你的驱动程序是否正确遵循了对设备对象、密钥的正确处理的一般要求，以及该驱动程序不会对非 PnP 驱动程序或不是 po 的非 FDO 驱动程序调用 DDIs。wer 策略所有者。
...
ms.locfileid: "72840126"
---
# <a name="miscellaneous-rule-set-kmdf"></a>其他规则集 (KMDF)


使用这些规则来验证你的驱动程序是否正确遵循了对设备对象、密钥的正确处理的一般要求，以及该驱动程序不会对非 PnP 驱动程序或不是 po 的非 FDO 驱动程序调用 DDIs。wer 策略所有者。
