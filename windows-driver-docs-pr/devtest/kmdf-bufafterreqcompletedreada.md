...
description: BufAfterReqCompletedReadA 规则指定，EvtIoRead 回调函数中检索到的 I/O 请求缓冲区后，无法访问 I/O 请求完成。 有 14 DDIs 作为可能的缓冲区的访问方法。
...
ms.locfileid: "56568886"
---
# <a name="bufafterreqcompletedreada-rule-kmdf"></a>BufAfterReqCompletedReadA rule (kmdf)


BufAfterReqCompletedReadA 规则指定，在[ *EvtIoRead* ](https://msdn.microsoft.com/library/windows/hardware/ff541776) I/O 请求完成后，无法访问的回调函数，检索到的 I/O 请求缓冲区。 有 14 DDIs 作为可能的缓冲区的访问方法。
...
<tbody>
<tr class="odd">
<td align="left"><p>运行<a href="https://msdn.microsoft.com/library/windows/hardware/ff552808" data-raw-source="[Static Driver Verifier](https://msdn.microsoft.com/library/windows/hardware/ff552808)">Static Driver Verifier</a>并指定<strong>BufAfterReqCompletedReadA</strong>规则。</p>
使用以下步骤来分析你的代码：
<ol>
