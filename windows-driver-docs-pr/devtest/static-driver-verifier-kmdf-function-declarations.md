...
必须声明回调函数定义之前的驱动程序的回调函数。 下面的示例演示的函数角色类型声明[ *EvtDriverDeviceAdd* ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfdriver/nc-wdfdriver-evt_wdf_driver_device_add)回调函数。 在此示例中，调用该回调函数*EvtDriverDeviceAdd*。
...
有一些函数角色类型可以具有多个与之关联的事件回调函数。 例如，驱动程序可能有多个[ *EvtTimerFunc* ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdftimer/nc-wdftimer-evt_wdf_timer)或[ *EvtDpcFunc* ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfdpc/nc-wdfdpc-evt_wdf_dpc)回调函数。 下表显示 SDV 支持为每个函数角色类型的回调的最大数目。 尽管这不是驱动程序具有多个表中列出的回调函数的最大数目不正确的它使用 SDV 时 does 增加复杂性验证过程。 您可能需要进行的 Sdv map.h 文件以容纳其他回调函数的更改的信息，请参阅[重复函数角色类型的入口点](duplicate-entry-points-for-a-function-role-type.md)。
...
必须声明[ *EvtCleanupCallback* ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfobject/nc-wdfobject-evt_wdf_object_context_cleanup)并[ *EvtDestroyCallback* ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfobject/nc-wdfobject-evt_wdf_object_context_destroy)函数和对象特定函数的角色类型。 SDV 要求这些特定于对象的角色类型以确定驱动程序是否正在正确使用回调函数。 使用下表来确定要使用的函数类型。
