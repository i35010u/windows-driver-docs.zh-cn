...
必须声明回调函数定义之前的驱动程序的回调函数。 下面的示例演示的函数角色类型声明[ *EvtDriverDeviceAdd* ](https://msdn.microsoft.com/library/windows/hardware/ff541693)回调函数。 在此示例中，调用该回调函数*EvtDriverDeviceAdd*。
...
如果回调函数有函数原型声明，必须将函数原型替换函数角色类型声明。 有关函数角色类型声明的详细信息，请参阅[使用函数角色类型声明](using-function-role-type-declarations.md)。

下表显示了回调函数类型和与之关联的事件回调函数。
...
有一些函数角色类型可以具有多个与之关联的事件回调函数。 例如，驱动程序可能有多个[ *EvtTimerFunc* ](https://msdn.microsoft.com/library/windows/hardware/ff541823)或[ *EvtDpcFunc* ](https://msdn.microsoft.com/library/windows/hardware/ff541683)回调函数。 下表显示 SDV 支持为每个函数角色类型的回调的最大数目。 尽管这不是驱动程序具有多个表中列出的回调函数的最大数目不正确的它使用 SDV 时 does 增加复杂性验证过程。 您可能需要进行的 Sdv map.h 文件以容纳其他回调函数的更改的信息，请参阅[重复函数角色类型的入口点](duplicate-entry-points-for-a-function-role-type.md)。
...
### <a name="span-idfunctionroletypesandioqueuesspanspan-idfunctionroletypesandioqueuesspanfunction-role-types-and-io-queues"></a><span id="function_role_types_and_i_o_queues"></span><span id="FUNCTION_ROLE_TYPES_AND_I_O_QUEUES"></span>函数的角色类型和 I/O 队列

使用以下函数角色类型声明时[请求处理程序](https://go.microsoft.com/fwlink/p/?linkid=153345)和依赖于 KMDF 框架将传送到 （适用于连续或平行调度） 的驱动程序的 I/O 请求的回调函数。 请不要将这些函数的角色类型用于手动将从默认队列的请求转发到其他队列 （手动调度） 的函数。 SDV 不支持的内存模型，从而可以从一个队列到另一个跟踪请求。

有关 I/O 队列的详细信息，请参阅[创建的 I/O 队列](https://go.microsoft.com/fwlink/p/?linkid=153346)。
...
<thead>
<tr class="header">
<th align="left">用于 I/O 队列配置为手动调度函数角色类型</th>
</tr>
</thead>
<tbody>
<tr class="odd">
...
必须声明[ *EvtCleanupCallback* ](https://msdn.microsoft.com/library/windows/hardware/ff540840)并[ *EvtDestroyCallback* ](https://msdn.microsoft.com/library/windows/hardware/ff540841)函数和对象特定函数的角色类型。 SDV 要求这些特定于对象的角色类型以确定驱动程序是否正在正确使用回调函数。 使用下表来确定要使用的函数类型。
...
<th align="left">EvtCleanupCallback 函数角色类型</th>
</tr>
</thead>
<tbody>
<tr class="odd">
...
<tr class="header">
<th align="left">对象类型</th>
<th align="left">EvDestroyCallback 函数角色类型</th>
</tr>
</thead>
<tbody>
<tr class="odd">
