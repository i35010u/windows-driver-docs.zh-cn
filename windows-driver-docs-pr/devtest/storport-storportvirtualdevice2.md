...
description: 此规则验证在 HwStorFindAdapter 例程退出后，端口 \_ 配置 \_ 信息（Storport）结构中的 VirtualDevice 字段是否已设置为 TRUE。 此规则仅适用于虚拟 StorPort 微型端口。
...
ms.locfileid: "85917449"
---
# <a name="storportvirtualdevice2-rule-storport"></a>StorPortVirtualDevice2 规则（storport）


此规则验证在[**HwStorFindAdapter**](https://docs.microsoft.com/windows-hardware/drivers/ddi/storport/nc-storport-hw_find_adapter)例程退出后，[**端口 \_ 配置 \_ 信息（Storport）**](https://docs.microsoft.com/previous-versions/windows/hardware/drivers/ff563901(v=vs.85))结构中的**VirtualDevice**字段是否已设置为**TRUE**。 此规则仅适用于虚拟 StorPort 微型端口。
