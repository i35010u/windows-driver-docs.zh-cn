...
ms.openlocfilehash: a900a46022487e2d3c97aadbfcd257173ead2b58
ms.sourcegitcommit: 418e6617e2a695c9cb4b37b5b60e264760858acd
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/07/2020
ms.locfileid: "96804271"
---
...
该命令使用 **-i** 参数来指示 WDK 中 Tracedrv、Tracedrv.sys 的映像文件的完全限定路径。
...
该命令使用 **-o** 参数来指示 Tracefmt 将格式化跟踪消息的输出文件放在 **d： \\ tracedrv \\tracedrv1.txt** 文件中。 此参数还将摘要文件放在与 Tracedrv.txt 文件名称相同的目录中。
...
对于此命令，Tracefmt 在 d： tracedrv 目录中查找并查找 Tracedrv.sys 的 PDB 文件 \\ 。 它从 PDB 文件中提取跟踪消息格式说明，并将其存储在 TMF 文件中，如下面的输出中的粗体语句中所示。 TMF 文件的名称是 Tracedrv 中跟踪提供程序的 [消息 GUID](message-guid.md) 。 Tracefmt 还会创建跟踪消息控件 (TMC) 文件，并将其放在同一目录中。
...
输出中的最后一个语句显示，Tracefmt 已成功格式化跟踪日志中的13个事件并创建 Tracedrv1.txt 和 Tracedrv1.txt sum 文件。
...
此 Tracefmt 运行的主输出 Tracedrv.txt，它是一个文本文件，其中包含 Tracedrv 中跟踪消息的格式化版本。 以下文本显示 Tracedrv.txt 的内容。
