...
指定的名称和扩展名[软件发布者证书 (SPC)](https://docs.microsoft.com/windows-hardware/drivers/install/software-publisher-certificate)包含证书的文件。 该文件可以是.spc 文件或.cer 文件。
...
若要使用[ **SignTool** ](signtool.md)工具来签署驱动程序符合的方式使用 SPC[内核模式代码签署策略](https://docs.microsoft.com/windows-hardware/drivers/install/kernel-mode-code-signing-policy--windows-vista-and-later-)，必须将 SPC 信息添加到对进行签名的驱动程序在本地计算机上个人证书存储。 有关如何将 SPC 信息添加到个人证书存储区的信息，请参阅[软件发布者证书](https://docs.microsoft.com/windows-hardware/drivers/install/software-publisher-certificate)。
