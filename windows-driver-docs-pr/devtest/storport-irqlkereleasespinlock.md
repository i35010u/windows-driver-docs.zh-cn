...
description: 此规则验证 KeReleaseSpinLock 是否在 IRQL 调度\_级别调用。 它还必须将 IRQL 设置为以前的 IRQL 级别。 通常，此调用之前会调用 KeAcquireSpinLock。
...
ms.locfileid: "72839300"
---
# <a name="irqlkereleasespinlock-rule-storport"></a>IrqlKeReleaseSpinLock 规则（storport）


此规则验证**KeReleaseSpinLock**是否在**IRQL = 调度\_级别**调用。 它还必须将 IRQL 设置为以前的 IRQL 级别。 通常，此调用之前会调用**KeAcquireSpinLock**。
