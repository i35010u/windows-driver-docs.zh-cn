...
| 24 | 标记 | 8 | 该主机在 "产品/服务" 组件中插入唯一标记。 此令牌必须由提供响应中的组件返回。 <<br><br>如果需要组件区分主机的不同主机/类型，则此方法很有用。<br><br>要使用的确切值是特定于实现的。 例如，一个值可用于某个驱动程序，另一个用于应用程序。 这允许当前设备固件考虑 CFU 命令的可能多个发件人。 一个可能的实现可能是接受第一个 CFU 命令，并拒绝具有不同标记的所有其他命令，直到第一个 CFU 事务完成。 |
