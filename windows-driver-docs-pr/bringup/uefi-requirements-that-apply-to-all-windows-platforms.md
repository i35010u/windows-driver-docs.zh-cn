...
<td><p>此 GUID 必须指向平台的 ACPI 根系统说明指针（RSDP）。</p></td>
</tr>
<tr class="even">
...
<li><p>如果无法确定显示功能，则必须在已知的模式下初始化已连接的显示，该模式与尽可能多的监视器（通常为640x480 或1024x768，以60Hz 为限）兼容。</p></li>
</ul></td>
</tr>
<tr class="odd">
<td>在启动时输入</td>
...
<tr class="even">
<td>本地存储启动</td>
<td><p>Windows 需要块 i/o 协议和设备路径协议对包含 EFI 系统分区和 Windows OS 分区的存储解决方案的支持。 若要从需要磨损或其他 flash 管理的闪存存储启动，必须在固件中实现此操作（而不是在 UEFI 应用程序中）。</p></td>
</tr>
</tbody>
</table>
...
<td>UEFI 安全启动</td>
<td><ul>
<li><p>要求3：必填。 在 UEFI v1.0 第27部分中定义的安全引导必须附带启用，并使用启动计算机安全预配所需的签名数据库（EFI_IMAGE_SECURITY_DATABASE）。 签名数据库的初始内容取决于所需的第三方 UEFI 驱动程序、恢复需求以及计算机上安装的 OS 启动加载程序，但应包括 Microsoft 提供的 EFI_CERT_X509 签名。 不应存在其他签名。</p></li>
<li><p>要求4：必填。 需要提供 UEFI "禁止" 的签名数据库（EFI_IMAGE_SECURITY_DATABASE1）。</p></li>
...
<li><p>要求17：必填。 支持 EFI_IMAGE_EXECUTION_INFO_TABLE （即在启动时启动或未启动的映像的创建和存储信息）。</p></li>
<li><p>需求18：必填。 支持 EFI_IMAGE_SECURITY_DATABASE 的 GetVariable （）（授权和禁止的签名数据库）。</p></li>
<li><p>要求19：必填。 使用 Microsoft KEK 进行身份验证时，支持 EFI_IMAGE_SECURITY_DATABASE 的 SetVariable （）（授权和禁止的签名数据库）。</p></li>
<li><p>要求20：必填。 EFI_HASH_SERVICE_BINDING_PROTOCOL：服务支持： CreateChild （）、DestroyChild （）。</p></li>
...
<li><p>授权的签名数据库（请参阅 UEFI 规范的第27.8 节，请参阅第节）列表中的内容摘要必须在作为 EV_EFI_VARIABLE_CONFIG 事件的度量启动中扩展。 "扩展" 操作应为 "TPM PCR [03]"。</p></li>
...
<p>此外，必须通过对 ExitBootServices （）的调用屏蔽中断，直到操作系统解除中的关联设备驱动程序，然后在设备上重新启用中断。 如果在启动服务期间启用中断，则假定固件将对其进行管理。</p></td>
</tr>
<tr class="odd">
<td>调试</td>
<td>Windows 支持通过 USB 3 主机（XHCI）、USB 2 主机（EHCI）1、IEEE 1394、串行和 USB 函数接口（以及 PCI 以太网适配器）进行调试。 在 OS 切换之前，固件中必须至少有一个处于 "已启动"、"时钟" 和 "已被" 初始化。 如果提供了任何选项，则它必须具有公开的端口用于调试，并且必须通过专用（非共享）外围总线进行连接。</td>
</tr>
<tr class="even">
<td>其他平台设置要求</td>
