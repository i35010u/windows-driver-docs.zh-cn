...
description: OID_WDI_SET_SAE_AUTH_PARAMS 由 WDI 发送以响应来自驱动程序的 NDIS_STATUS_WDI_INDICATION_SAE_AUTH_PARAMS_NEEDED 指示。 它包含为 Equals （SAE） Commit 或 Confirm 请求同时发送身份验证所需的参数，或指示无法使用 BSSID 执行 SAE 的错误消息。
...
ms.locfileid: "85968506"
---
# <a name="oid_wdi_set_sae_auth_params"></a>OID_WDI_SET_SAE_AUTH_PARAMS

**OID_WDI_SET_SAE_AUTH_PARAMS**由 WDI 发送以响应来自驱动程序的[NDIS_STATUS_WDI_INDICATION_SAE_AUTH_PARAMS_NEEDED](ndis-status-wdi-indication-sae-auth-params-needed.md)指示。 它包含为 Equals （SAE） Commit 或 Confirm 请求同时发送身份验证所需的参数，或指示无法使用 BSSID 执行 SAE 的错误消息。 
