...
-   如果提供的*SaveDataSize*不够大，无法容纳所需的保存数据，则扩展会将方法结构的*BytesNeeded*字段设置为 NDIS\_SIZEOF\_NDIS\_交换机\_NIC\_将\_状态\_1 加上保存保存数据所需的缓冲区大小，并通过 NDIS\_状态\_缓冲区来完成 OID。\_\_\_ 将重新颁发具有所需大小的 OID。
