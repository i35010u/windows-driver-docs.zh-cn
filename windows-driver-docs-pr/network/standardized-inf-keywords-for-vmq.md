...
ms.locfileid: "72841840"
---
# <a name="standardized-inf-keywords-for-vmq"></a>VMQ 的标准化 INF 关键字


定义了以下标准化 INF 关键字，以启用或禁用对网络适配器的虚拟机队列（VMQ）功能的支持。

<a href="" id="-vmq"></a> **\*VMQ**  
一个值，该值描述设备是否已启用或禁用 VMQ 功能。

<a href="" id="-vmqlookaheadsplit"></a> **\*VMQLookaheadSplit**  
一个值，该值描述设备是否已启用或禁用将接收缓冲区拆分为预测先行缓冲区和后期预测缓冲区的能力。 微型端口驱动程序通过 NDIS\_接收\_筛选器来报告此功能\_ [**ndis\_接收\_筛选器的 SupportedQueueProperties 成员中\_SPLIT\_支持的标志\_功能**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_receive_filter_capabilities)结构。 有关此功能的详细信息，请参阅[接收缓冲区中的共享内存](shared-memory-in-receive-buffers.md)。
...
一个值，该值描述设备是否已启用或禁用通过使用媒体访问控制（MAC）标头中的 VLAN 标识符来筛选网络数据包。 微型端口驱动程序通过 NDIS\_接收\_FILTER\_MAC\_标头来报告此功能 MAC 标头\_VLAN\_接收的**SupportedMacHeaderFields**成员中支持的标志[ **\_筛选\_功能**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_receive_filter_capabilities)结构。

<a href="" id="-rssorvmqpreference"></a> **\*RssOrVmqPreference**  
一个值，该值定义是否应启用 VMQ 功能而不是接收方缩放（RSS）功能。
...
必须在 INF 文件中指定的关键字的名称。 此名称还会显示在网络适配器的**NDI**\\**params**密钥下的注册表中。
...
由于网络适配器未被禁用，然后在未绑定到 TCP/IP 堆栈并绑定到 Hyper-v 驱动程序堆栈（或相反）时重新启用，因此此类网络适配器无法自动在 VMQ 和 RSS 之间切换。

当 NDIS 调用[*MiniportInitializeEx*](https://docs.microsoft.com/windows-hardware/drivers/ddi/ndis/nc-ndis-miniport_initialize)函数时，微型端口驱动程序会在向 NDIS 报告当前启用的 VMQ 或 RSS 功能之前执行以下步骤：
...
<thead>
<tr class="header">
<th align="left"><em>RssOrVmqPreference</th></em>VMQ 
<th align="left"></th>
...
<td align="left"><p>0到（MAXIMUM_PROC_PER_GROUP-1）、</p></td>
</tr>
<tr class="even">
<td align="left"><p></p>
...
支持 VMQ 的微型端口驱动程序不得读取**HKEY\_本地\_计算机**\\**SYSTEM**\\**CurrentControlSet**\\**services** **\\** \\**参数**注册表项。
...
<td align="left"><p>在所有10千兆位/秒（Gbps）网络适配器上启用或禁用 VMQ。</p></td>
<td align="left"><p>0 = 系统默认值（禁用 Windows Server 2008 R2）。</p>
<p>1 = 已启用。</p>
<p>2 = 显式禁用。</p></td>
</tr>
<tr class="even">
