...
| [**WDI\_MAC\_ADDRESS**](https://msdn.microsoft.com/library/windows/hardware/dn926071) | 指定的网络适配器必须使用它生成 ARP 响应数据包的源硬件地址 (SHA) 字段的 MAC 地址。 但是，它应为 MAC 标头中的源地址使用的网络适配器的当前 MAC 地址。                                                                                                          |
