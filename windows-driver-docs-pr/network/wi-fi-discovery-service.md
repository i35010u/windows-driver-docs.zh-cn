...
下面的示例演示了一个典型的提交请求。 `X-FinalBatchRequest`如果标头和数值为 "1", 则表示此批处理中只有一个作用点, 这是最终提交请求。 因此, 这是唯一的提交请求。 如果批处理包含多个热点, 则应为所有提交请求 (最后一个) 删除此行。
...
WifiProviderExe 是 Microsoft 提供的一个命令行工具, 它采用热点数据文件作为输入, 将其转换为所需的格式, 并将其上传到指定的发现服务服务器。

若要运行 WiFiProvider, 请使用以下语法:
...
| CertFile | 可选。 一个指针, 指向包含用于身份验证的证书的实际 pfx 文件。 使用此身份验证方法时, 必须指定证书密码参数 (**CertPassword**)。 |
| CertPassword | 可选。 在**CertFile**中指定的证书的密码。 |
| CertSubject | 可选。 证书的使用者名称。 它位于当前用户的 "我的证书存储" 中。 使用此身份验证机制时, **CertFile**和**CertPassword**不是必需的。 但是, 必须为证书创建私钥, 并在访问控制列表中, 向将使用该证书的帐户授予对密钥的访问权限。 |
...
| 转换器 | 数据格式 | 描述 |
| --- | --- | --- |
| WifiService. ProviderSdk. JsonHotspotDataTransformer | JSON | 数据必须符合[Wi-fi 热点数据提交格式](wi-fi-hotspot-data-submission-format.md)中指定的 JSON 格式。 |
| WifiService. ProviderSdk. JsonHotspotDataTransformer | 简单 Excel | 您必须提供一个映射文件。 |
...
下面的示例显示了映射文件的一部分。 每个**MappingRule**元素将 Excel 列 (**PartnerColumnName**和**PartnerColumnNumber**) 与所需的 JSON 元素 (**MicrosoftColumnName**) 相关联。 位于结束规则标记 (`</Rules>`) 后面的 ContainsHeaderRow 元素指示该文件包含一个标题行, 该标题行应在读取数据时跳过。
