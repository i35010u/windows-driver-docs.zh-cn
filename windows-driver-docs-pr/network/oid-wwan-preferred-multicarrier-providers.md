...
ms.date: 08/08/2017
keywords: -从 Windows Vista 开始 OID_WWAN_PREFERRED_MULTICARRIER_PROVIDERS 网络驱动程序
...
支持*set*和*query*请求。 微型端口驱动程序必须异步处理*集*和*查询*请求，最初返回 NDIS\_状态\_指示\_需要原始请求，稍后发送[**ndis\_状态\_WWAN\_首选\_多\_提供程序**](https://docs.microsoft.com/windows-hardware/drivers/network/ndis-status-wwan-preferred-multicarrier-providers)状态通知，其中包含[**NDIS\_WWAN\_首选\_多\_提供程序**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ndiswwan/ns-ndiswwan-_ndis_wwan_preferred_multicarrier_providers)结构。

在\_WWAN @no__t 响应 OID 时，微型端口驱动程序应将**PreferredListHeader**成员设置为**WwanStructProvider2** ，并将**PreferredListHeader**成员设置为列表中的提供程序数。_4_ 首选\_提供程序*查询*请求。 *查询*中返回的多运营商提供程序必须能够在首选多运营商列表返回到服务时设置为主提供商。

当响应\_WWAN\_首选\_时，微型端口驱动程序应将**PreferredListHeader**成员设置为0，并将**PreferredListHeader**成员**设置为 0**提供程序*设置*请求。

出现错误时，微型端口应将 NDIS\_WWAN\_首选\_\_多的**uStatus**成员设置为失败状态，将**PreferredListHeader**设置为**elementcount 多于，然后PreferredLIstHeader**到**WwanStructProvider2**。
