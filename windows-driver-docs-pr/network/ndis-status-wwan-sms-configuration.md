...
description: 微型端口驱动程序使用 NDIS_STATUS_WWAN_SMS_CONFIGURATION 通知来通知 MB 服务有关上一个 OID_WWAN_SMS_CONFIGURATION \ 160; 个查询或设置请求的完成，或在 SMS 中发生更改时的事件通知configuration. 小型端口驱动程序还可以通过此通知发送未经请求的事件。此通知使用 NDIS_WWAN_SMS_CONFIGURATION 结构。
...
小型端口驱动程序使用 NDIS\_状态\_WWAN\_SMS\_配置通知来通知 MB 服务有关上一个 OID 的完成[\_wwan\_SMS\_配置](oid-wwan-sms-configuration.md) 查询或设置请求，或在 SMS 配置发生更改的情况下的事件通知。
