...
**InformationBuffer**的成员[ **NDIS\_OID\_请求**](https://msdn.microsoft.com/library/windows/hardware/ff566710)结构是可用于启用组合的标志的位掩码唤醒事件：
...
协议驱动程序使用的网络适配器中的唤醒功能[ **NDIS\_绑定\_参数**](https://msdn.microsoft.com/library/windows/hardware/ff564832)来启用关联的网络适配器的唤醒功能。 协议驱动程序还可以查询此 OID 确定哪些唤醒功能已启用的网络适配器。
...
在其中收到此 OID 请求时的上边缘的中间驱动程序必须始终将对基础微型端口驱动程序的请求传播通过调用[ **NdisOidRequest** ](https://msdn.microsoft.com/library/windows/hardware/ff563710)或[ **NdisCoOidRequest** ](https://msdn.microsoft.com/library/windows/hardware/ff561711)函数。
