...
## <a name="esim-architecture"></a>eSIM 体系结构

Windows 操作系统的本地 Profile Assitant (LPA) 组件是 LPA 服务，而低级别 UICC 访问通过 WWAN 服务公开。 LPA 服务处理配置文件发现、下载配置文件和配置文件管理。

![eSIM 体系结构](images/esim_lpa_block.png "eSIM 块示意图")

## <a name="mb-interface-update-for-esim-operations"></a>ESIM 操作的 MB 接口更新

对于 eSIM 功能，调制解调器需要支持以下 [MB 低级别 UICC 访问 cid](mb-low-level-uicc-access.md) 。
...
## <a name="esim-service-initialization"></a>eSIM 服务初始化

![eSIM 服务初始化](images/esim_lpa_init.png "eSIM 服务初始化流程图")

## <a name="esim-profile-download-and-install"></a>eSIM 配置文件下载和安装

![eSIM 配置文件下载和安装](images/esim_lpa_download_install.png "eSIM 下载和安装流关系图")

## <a name="esim-profile-operations"></a>eSIM 配置文件操作
...
下面是 "启用配置文件" 操作的示例流。 其他配置文件操作遵循类似的流，只不过 MBIM_CID_MS_UICC_APDU 将包含相应操作的 Es10c 命令。

![eSIM 启用配置文件](images/esim_lpa_enable.png "eSIM 启用配置文件流关系图")

### <a name="card-refresh"></a>卡刷新
ESIM 配置文件操作预期会按照 " [MB ESIM MBIM" 就绪状态指南](mb-esim-mbim-ready-state-guidance.md)，执行卡刷新，然后再执行状态更改。

## <a name="hardware-lab-kit-hlk-tests"></a>硬件实验室工具包 (HLK) 测试

可以使用 HLK 中的以下测试来验证 eSIM 功能：
...
可以通过 netsh 运行 **TestLowLevelUiccAccess** HLK testlist。 有关使用 netsh 工具的详细信息，请参阅 [**mbn**](/windows-server/networking/technologies/netsh/netsh-mbn) and [**netsh-mbn**](mb-netsh-mbn-test.md)。

```
netsh mbn test feature=esim testpath="C:\data\test\bin" taefpath="C:\data\test\bin" param="AccessString=internet"
```

显示 HLK 测试结果的文件应已在运行 "netsh mbn test" 命令的目录中生成： `TestLowLevelUiccAccess.htm` 。

## <a name="manual-tests"></a>手动测试

### <a name="test-esim-profile-management"></a>测试 eSIM 配置文件管理
1. 所需资源：下载配置文件的 eSIM 和激活代码或 QR 码。 配置文件需要要下载的确认代码。
1. 打开设置->网络 & Internet-> 手机网络。
1. 单击 " ***管理 eSIM 配置文件***"。
1. 单击 " ***添加新的配置文件***"。
1. 选择 **"使用我的移动运营商输入激活代码"**，并单击 "_下一步_"。
1. 扫描 QR 代码或键入激活代码。
1. 等到确认页面出现。 填写确认代码，并单击 "  ***下一步***"。
1. 关闭对话框，然后启用配置文件。
1. 更改配置文件的名称。 应为该配置文件显示新名称。
1. 删除配置文件。
1.  (重复步骤 4-7) 再次安装该配置文件，但这一次带有错误的确认代码。 例如，在扫描 QR 码时，代码哈希显示一秒，然后再与服务器取得联系。 非常快速地单击它内部的任意位置，然后按 "后退" 键删除单个字符，然后将其与服务器进行通信。
1. 你应会看到一条错误消息，并最终在确认页上进行陆地。 在此时键入正确的确认代码，并单击 " ***下一步***"。
1. 关闭对话框并删除配置文件。

### <a name="test-autoconnect-after-reboot"></a>重新启动后测试自动连接
1. 请确保已拔下以太网，并关闭 Wi-Fi。 如果设备中的物理 eSIM 卡上有一个已知的良好 eSIM 配置文件，请浏览到 "设置-> 网络" "& Internet-> > >  验证浏览 internet 是否正常工作。
1. 重新启动计算机，登录，并浏览到 "设置"-> 网络 & Internet-> 手机-> 管理 eSIM 配置文件-> eSIM 配置文件。 配置文件应显示为 "活动"，托盘应显示 "移动电话" 连接栏，并且浏览 internet 应正常工作。
1. 返回 "管理 eSIM 配置文件"，选择配置文件，然后单击 " ***停止使用***"。 配置文件应断开数据的连接。


## <a name="mb-esim-troubleshooting-guide"></a>MB eSIM 故障排除指南

按照此指南调试 eSIM 问题。

### <a name="profile-download-and-install-failures"></a>配置文件下载和安装失败

1. 使用[收集日志](mb-collecting-logs.md)中的指令收集和解码日志
2. 打开在[TextAnalysisTool](mb-analyzing-logs.md)中生成的 .txt 文件。
3. 加载 [ESIM 下载和安装筛选器](mb-esim-download-install-tat.md)
...
### <a name="profile-operation-failures"></a>配置文件操作失败

1. 使用[收集日志](mb-collecting-logs.md)中的指令收集和解码日志
2. 打开在[TextAnalysisTool](mb-analyzing-logs.md)中生成的 .txt 文件。
3. 加载 [ESIM 配置文件操作筛选器](mb-esim-profile-operations-tat.md)

下面是 "启用配置文件" 操作的示例成功日志：
