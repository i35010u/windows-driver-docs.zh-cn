...
ms.locfileid: "56563243"
---
# <a name="overview-of-network-direct-kernel-provider-interface-ndkpi"></a>网络直接内核提供程序接口 (NDKPI) 概述


网络直接内核提供程序接口 (NDKPI) 是允许 Ihv 提供网络适配器 （也称为 RNIC） 中的内核模式远程直接内存访问 (RDMA) 支持的 NDIS 的扩展。 若要显示适配器的 RDMA 功能，IHV 必须实现 NDKPI 接口中定义[NDKPI 引用](https://msdn.microsoft.com/library/windows/hardware/jj206456)。
...
在处理来自 NDK 使用者的请求时，该提供程序可以调用使用者的 NDK 回调函数。 这些文档位于[NDKPI 使用者的回调函数](https://msdn.microsoft.com/library/windows/hardware/jj879316)。

NDK 提供程序必须实现中所述的 NDKPI 接口中的所有元素[NDKPI 引用](https://msdn.microsoft.com/library/windows/hardware/jj206456)，除[NDKPI 使用者的回调函数](https://msdn.microsoft.com/library/windows/hardware/jj879316)。
...
NDK 使用者调用提供程序的*NdkOpenAdapter* ([*打开\_NDK\_适配器\_处理程序*](https://msdn.microsoft.com/library/windows/hardware/hh440105)) 到回调函数创建一个适配器的对象和*NdkCloseAdapter* ([*NDK\_FN\_关闭\_对象*](https://msdn.microsoft.com/library/windows/hardware/hh439863)) 以将其关闭。 一旦该提供程序创建的适配器对象，使用者调用其他提供程序回调函数来创建其他 NDK 对象。

使用者实现 NDK [NDKPI 使用者的回调函数](https://msdn.microsoft.com/library/windows/hardware/jj879316)，由 NDK 提供程序调用。
