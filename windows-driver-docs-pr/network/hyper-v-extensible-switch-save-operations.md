...
-   如果扩展具有要保存的运行时数据，则它会初始化[**NDIS\_交换机\_NIC\_保存\_状态**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_switch_nic_save_state)结构，并设置各个成员（例如**ExtensionId**成员），以标识自身及其要保存的数据。 该扩展还会将数据保存在**NDIS\_交换机\_NIC\_SAVE\_状态**结构，从结构的开头开始 SaveDataOffset 字节，然后 NDIS 完成 OID 方法请求，\_成功状态。\_

-   如果[**ndis\_交换机\_NIC\_SAVE\_状态**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_switch_nic_save_state)结构不提供足够的缓冲区大小，在 NDIS\_对象\_标头**大小**成员中枚举以保存运行时状态，扩展会将方法结构的*BytesNeeded*字段设置为 ndis\_SIZEOF\_\_\_\_\_\_\_，并通过 NDIS 完成 OID\_状态\_缓冲器\_太\_SHORT。 将重新颁发具有所需大小的 OID。
...
在保存运行时数据的操作期间，可扩展交换机的协议边缘[\_交换机\_nic](https://docs.microsoft.com/windows-hardware/drivers/network/oid-switch-nic-save)上发出 oid 请求，\_SAVE 和 OID\_交换机\_nic\_已连接的 hyper-v 子分区的网络接口的 "保存\_"。 如果已停止或实时迁移多个 Hyper-v 子分区，则协议边缘\_NIC\_交换机 NIC 上发出不同的 OID 集\_SAVE 和 OID\_交换机\_NIC\_保存每个网络接口连接\_完成的请求。
