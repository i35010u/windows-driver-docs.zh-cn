...
下面的代码示例演示 WSK 应用程序如何使用[ **\_WSK\_事件\_回调**](https://docs.microsoft.com/windows-hardware/drivers/network/so-wsk-event-callback)套接字选项在面向连接的套接字上禁用[*WskReceiveEvent*](https://docs.microsoft.com/windows-hardware/drivers/ddi/wsk/nc-wsk-pfn_wsk_receive_event)事件回调函数。
...
对于侦听套接字，仅当 WSK 应用程序在套接字上启用条件接受模式时，才会启用[*WskInspectEvent*](https://docs.microsoft.com/windows-hardware/drivers/ddi/wsk/nc-wsk-pfn_wsk_inspect_event)和[*WskAbortEvent*](https://docs.microsoft.com/windows-hardware/drivers/ddi/wsk/nc-wsk-pfn_wsk_abort_event)事件回调功能。 WSK 应用程序通过在将套接字绑定到本地传输地址之前设置套接字[ **\_条件\_accept**](https://docs.microsoft.com/windows-hardware/drivers/network/so-conditional-accept) socket 选项，在侦听套接字上启用条件接受模式。 有关如何设置套接字选项的详细信息，请参阅[在套接字上执行控制操作](performing-control-operations-on-a-socket.md)。
...
如果 WSK 应用程序使用 WSK\_设置\_静态\_事件\_回调客户端控制操作自动启用特定的事件回调函数，则必须在创建任何套接字之前执行此操作。
