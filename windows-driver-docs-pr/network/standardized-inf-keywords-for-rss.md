...
与列表中的每个选项关联的枚举整数值。 此值存储在 NDI\\参数\\ *SubkeyName*\\*值*中。
...
PCI 扩展卡的驱动程序不应在其 INF 中静态指定 NUMA 节点 ID，因为最近的节点取决于插入卡的 PCI 插槽。  仅当网络适配器集成到系统中时，才指定 **\*NumaNodeId** ，将提前知道 NUMA 节点，并且在运行时无法通过查询 ACPI 确定节点。

**请注意**  如果此关键字存在并且其值小于计算机上的 NUMA 节点数，ndis 将在[**ndis\_RSS\_处理器\_信息**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_rss_processor_info)结构中的**PreferredNumaNode**成员中使用此值。

 

**注意**  在 Windows 8 中，如果 NIC RSS 配置文件设置为**NUMAScaling**（2）或**NUMAScalingStatic**（3），则将忽略 **\*NumaNodeId**值。
...
RSS 接口的最大处理器数目。
如果指定 **\*RssMaxProcNumber** ，则还应指定 **\*RssMaxProcGroup** 。
...
**\*RssBaseProcGroup**与 **\*RssBaseProcNumber**一起构成一个 PROCESSOR_NUMBER 结构，该结构标识可用于 RSS 的最小处理器数。
**\*RssMaxProcGroup**与 **\*RssMaxProcNumber**一起构成一个 PROCESSOR_NUMBER 结构，该结构标识可与 RSS 一起使用的最大处理器数目。

例如，假设 **\*RssBaseProcGroup**设置为1， **\*RssBaseProcNumber**设置为16， **\*RssMaxProcGroup**设置为3， **\*RssMaxProcNumber**设置为8。
使用 <group>：<processor> 表示法，基本处理器为1:16，最大处理器为3:8。
...
NDIS 6.30 添加了对 **\*RssMaxProcNumber**的支持，并 **\*NumRSSQueues**关键字。
...
<a href="" id="type"></a>类别  
可以编辑的值的类型。 该值可以是数字（Int），也可以是可编辑的文本（"编辑"）。

<a href="" id="default-value"></a>默认值  
整数或文本的默认值。 定义&gt; &lt;IHV 表示该值与特定的独立硬件供应商（IHV）要求相关联。

<a href="" id="min"></a>Min  
允许用于整数的最小值。 定义&gt; &lt;IHV 指示最小值与特定的 IHV 要求关联。

<a href="" id="max"></a>数量  
整数允许的最大值。 定义&gt; &lt;IHV 指示最小值与特定的 IHV 要求关联。
...
**请注意**  虽然 **\*RssBaseProcGroup**的有效范围为0到最大\_组-1，在 Windows 7 中，它必须为零。 否则，TCP/IP 协议不会将任何处理器用于 RSS。

 

**请注意**   **\*NumaNodeId** （65535）的默认值表示网络适配器对于 NUMA 节点是不可知的，NDIS 不应尝试使用任何节点。
如果不存在 **\*NumaNodeId**关键字，NDIS 会根据 ACPI 中的提示自动选择最近的节点。
