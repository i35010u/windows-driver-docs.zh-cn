...
NDIS 可支持同一数据结构的多个版本。 使用 Windows Server 2008 中的更新结构、带有 Service Pack 1 （SP1）的 Windows Vista 或这两个操作系统的 NDIS 6.1 驱动程序必须在驱动程序初始化 NDIS 6.1 数据结构的情况下，在结构的**标头**成员（如果有）中报告正确的**修订版**成员和**Size**成员[ **\_\_** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_object_header)值（如果有）。
