...
description: 以下各节介绍了实现接收段合并（RSC）功能的微型端口驱动程序时要考虑的问题。
...
NDIS、过量驱动程序和用户模式应用程序使用[oid\_TCP\_RSC\_STATISTICS](https://docs.microsoft.com/windows-hardware/drivers/network/oid-tcp-rsc-statistics) OID 来获取微型端口适配器的 RSC 统计信息。 支持 RSC 的微型端口驱动程序必须支持此 OID。
...
WFP 标注驱动程序可以支持对大于链接 MTU 的支持接收数据包的处理。 （有关数据包大小限制的详细信息，请参阅[跟踪和指示合并段](https://docs.microsoft.com/windows-hardware/drivers/network/indicating-coalesced-segments)。）此类 WFP 标注驱动程序应执行以下操作：

-   在注册期间选择加入，以处理大数据包。

-   设置[**FWPS\_CALLOUT2**](https://docs.microsoft.com/windows-hardware/drivers/ddi/fwpsk/ns-fwpsk-fwps_callout2_)结构的参考页中指定的标注驱动程序标志。
