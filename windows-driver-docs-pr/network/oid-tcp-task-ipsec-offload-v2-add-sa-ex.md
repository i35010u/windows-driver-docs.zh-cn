...
作为集，TCP/IP 传输使用 OID\_TCP\_任务\_IPSEC\_卸载\_V2\_添加\_SA\_EX OID 来请求微型端口驱动程序添加指定的安全关联（SAs）到 NIC。
...
微型端口驱动程序将 NIC 配置为针对 SAs 的 IPsecOV2 处理。 成功设置到 OID 后\_TCP\_任务\_IPSEC\_卸载\_V2\_添加\_SA\_例如，微型端口驱动程序提供了用于标识**OffloadHandle**成员的[**IPSEC\_卸载\_V2\_添加\_SA\_EX**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ndis/ns-ndis-_ipsec_offload_v2_add_sa_ex)结构。 （例如，传输使用发送路径中的句柄来指示要使用的卸载 SA）。 如果卸载了 SA，则该设置请求成功。
