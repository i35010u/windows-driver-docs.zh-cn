...
-   如果 WSK 应用程序使用由较高级别驱动程序或 i/o 管理器向下传递给它的 IRP，则在调用 WSK 函数之前应为 IRP 设置**IoCompletion**例程，前提是该函数必须在由 WSK functi 执行的操作时收到通知完成。 如果 WSK 应用程序未为 IRP 设置**IoCompletion**例程，则完成 irp 后，irp 将按正常的 irp 完成处理方式传递回更高级别的驱动程序或 i/o 管理器。 如果 WSK 应用程序为 IRP 设置了**IoCompletion**例程，则**IoCompletion**例程可以返回状态\_成功或状态\_需要更多\_处理\_。 如果**IoCompletion**例程返回状态\_成功，则 IRP 完成处理将继续正常进行。 如果**IoCompletion**例程返回状态\_\_需要更多的\_处理，则 WSK 应用程序必须在处理完该操作的结果后调用[**IoCompleteRequest**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocompleterequest) ，才能完成 IRP由 WSK 函数执行。 WSK 应用程序绝不应释放由较高级别的驱动程序或 i/o 管理器向下传递给它的 IRP。

**请注意**  如果 WSK 应用程序为 IRP 设置了一个**IoCompletion**例程，该例程由较高级别的驱动程序或 i/o 管理器向下传递给它，则**IoCompletion**例程必须检查如果**PendingReturned**成员为**TRUE**，则 IRP 并调用[**也**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iomarkirppending)函数。 有关详细信息，请参阅[实现 IoCompletion 例程](https://docs.microsoft.com/windows-hardware/drivers/kernel/implementing-an-iocompletion-routine)。
