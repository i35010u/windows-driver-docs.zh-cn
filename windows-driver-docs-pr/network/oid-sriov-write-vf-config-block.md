...
description: 过量驱动程序发出 OID_SRIOV_WRITE_VF_CONFIG_BLOCK 的对象标识符（OID）设置请求，以将数据写入 PCI Express （PCIe）虚函数（VF）配置块。
...
-   NDIS\_SRIOV\_将包含偏移量（以字节为单位）的[ **\_VF\_配置\_块写入**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_sriov_write_vf_config_block_parameters)包含数据的缓冲区中的位置。写入 VF 配置块。
...
在\_SRIOV 的 OID 集请求\_写入\_VF\_CONFIG\_块之前，过量驱动程序必须将[**NDIS\_SRIOV 的成员设置\_\_\_t_11_** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_sriov_write_vf_config_block_parameters)按以下方式\_参数结构：
...
-   PF 微型端口驱动程序必须验证由 NDIS\_SRIOV 的**VFId**成员指定的 vf 是否[ **\_写入\_vf\_配置\_块\_参数**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_sriov_write_vf_config_block_parameters)结构，是否包含之前已有的资源分配. 在 oid 的 OID 方法请求中，PF 微型端口驱动程序为一个 VF 分配资源[\_NIC\_交换机\_分配\_VF](oid-nic-switch-allocate-vf.md)。 如果没有为指定的 VF 分配资源，则驱动程序必须使 OID 请求失败。
