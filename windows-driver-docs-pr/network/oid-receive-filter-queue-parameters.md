...
ms.date: 08/08/2017
keywords: -从 Windows Vista 开始 OID_RECEIVE_FILTER_QUEUE_PARAMETERS 网络驱动程序
...
过量驱动程序发出 OID\_接收\_FILTER\_QUEUE\_参数的对象标识符（OID）方法请求，以获取接收队列的当前配置参数。 [**Ndis\_OID\_请求**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ndis/ns-ndis-_ndis_oid_request)结构的**InformationBuffer**成员包含指向[**ndis\_接收\_队列**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_receive_queue_parameters)的指针，\_参数结构的队列标识符类型为**NDIS\_接收\_队列\_ID**。 成功从 OID 方法请求返回后， **ndis\_OID\_请求**结构的**InformationBuffer**成员包含指向**ndis\_接收\_队列的指针\_参数**构造.
...
在过量驱动程序分配队列后，它可以在**Flags**成员中更改具有相应更改标志的配置参数（NDIS\_接收\_队列\_参数\_*Xxx*\_更改）在[**NDIS\_接收\_队列\_参数**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddndis/ns-ntddndis-_ndis_receive_queue_parameters)结构。 但是，在分配队列后，过量驱动程序无法更改没有相应更改标志的配置参数。
...
<td><p>信息缓冲区太短。 NDIS 设置<strong>数据</strong>。<strong>METHOD_INFORMATION</strong>。NDIS_OID_REQUEST 结构中的<strong>BytesNeeded</strong>成员到所需的最小缓冲区大小。</p></td>
</tr>
<tr class="even">
