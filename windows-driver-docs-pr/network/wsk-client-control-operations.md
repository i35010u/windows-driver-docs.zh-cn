---
title: WSK 客户端控制操作
description: WSK 客户端控制操作
ms.assetid: aa1159f5-defd-440e-9628-906a246c11df
ms.date: 07/18/2017
keywords:
- WSK WDK 网络、 控制操作
- 网络、 Winsock 内核 WDK 控制操作
- WSK WDK 网络、 操作
- 网络、 Winsock 内核 WDK 操作
ms.localizationpriority: medium
ms.openlocfilehash: 932d834d2400c4d7983be34f8e78ba398c95e02e
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/31/2019
ms.locfileid: "56541724"
---
# <a name="wsk-client-control-operations"></a>WSK 客户端控制操作


WSK 子系统支持以下 WSK 客户端控制操作：

[**WSK\_CACHE\_SD**](wsk-cache-sd.md)

[**WSK\_RELEASE\_SD**](wsk-release-sd.md)

[**WSK\_设置\_静态\_事件\_回调**](wsk-set-static-event-callbacks.md)

[**WSK\_TDI\_BEHAVIOR**](wsk-tdi-behavior.md)

[**WSK\_TDI\_DEVICENAME\_MAPPING**](wsk-tdi-devicename-mapping.md)

[**WSK\_传输\_列表\_更改**](wsk-transport-list-change.md)

[**WSK\_传输\_列表\_查询**](wsk-transport-list-query.md)

 

 




