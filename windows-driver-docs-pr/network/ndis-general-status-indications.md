---
title: NDIS 常规状态指示
description: NDIS 常规状态指示
ms.assetid: 0ce19114-9861-4904-8222-1c229aa0008d
ms.date: 07/18/2017
ms.localizationpriority: medium
ms.openlocfilehash: c6492e11c2ff39ddff77994a9321ae879c81c1d3
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/31/2019
ms.locfileid: "56567644"
---
# <a name="ndis-general-status-indications"></a>NDIS 常规状态指示





本部分包括：

[**NDIS\_状态\_媒体\_连接**](ndis-status-media-connect.md)

[**NDIS\_状态\_媒体\_断开连接**](ndis-status-media-disconnect.md)

[**NDIS\_状态\_重置\_开始**](ndis-status-reset-start.md)

[**NDIS\_状态\_重置\_结束**](ndis-status-reset-end.md)

[**NDIS\_状态\_媒体\_忙**](ndis-status-media-busy.md)

[**NDIS\_状态\_媒体\_特定\_指示**](ndis-status-media-specific-indication.md)

[**NDIS\_状态\_链接\_速度\_更改**](ndis-status-link-speed-change.md)

[**NDIS\_STATUS\_LINK\_STATE**](ndis-status-link-state.md)

[**NDIS\_状态\_端口\_状态**](ndis-status-port-state.md)

[**NDIS\_状态\_工序\_状态**](ndis-status-oper-status.md)

[**NDIS\_状态\_网络\_更改**](ndis-status-network-change.md)

[**NDIS\_状态\_数据包\_筛选器**](ndis-status-packet-filter.md)

 

 




