...
1.  如果尚未完成此操作，请按照[预配计算机以便进行驱动程序部署和测试 (WDK 8.1)](https://docs.microsoft.com/windows-hardware/drivers/gettingstarted/provision-a-target-computer-wdk-8-1) 中的说明进行操作。
2.  将你想要测试的设备连接到测试计算机。
...
你还可以启用并设置[驱动程序验证程序](https://docs.microsoft.com/windows-hardware/drivers/devtest/driver-verifier)（驱动程序的运行时验证工具）的选项。 当你在测试计算机上运行测试时，驱动程序验证程序会监视你的驱动程序。 有关针对部署设置驱动程序验证程序选项的信息，请参阅[驱动程序项目的驱动程序验证程序属性](driver-verifier-properties-for--driver-projects.md)。
...
从 WDK 8.1 开始，你可以选择要在测试计算机上运行的 HCK 测试套件。 HCK 测试套件包括[设备基础功能测试](https://docs.microsoft.com/windows-hardware/drivers/devtest/device-fundamentals-tests)，以及图形、映像、无线 LAN、移动宽带（CDMA 和 GSM）和 WiFi Direct 设备的 Windows 硬件认证工具包 (HCK) 基本测试。
...
在为测试组选择测试后，你可以配置传递到驱动程序测试的任何一个运行时参数。 例如，许多设备基础功能测试具有参数 *DQ*，它代表“设备查询”。 这是[简单数据评估语言](https://docs.microsoft.com/windows-hardware/drivers/wdtf/simple-data-evaluation-language-overview) (SDEL) 查询。 [Windows 驱动程序测试框架](https://docs.microsoft.com/windows-hardware/drivers/wdtf/index)提供了 SDEL 作为查询语言，以简化根据属性或关系收集目标的任务。
...
有关驱动程序测试和测试类别的信息，请参阅[如何选择和配置设备基础功能测试](how-to-select-and-configure-the-device-fundamental-tests.md)。 有关测试框架的信息，请参阅[测试授权和执行框架](https://docs.microsoft.com/windows-hardware/drivers/taef/index) (TAEF) 和 [Windows 驱动程序测试框架](https://docs.microsoft.com/windows-hardware/drivers/wdtf/index) (WDTF)。
