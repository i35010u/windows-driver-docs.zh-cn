...
每个平台定义的传感器属性**PROPERTYKEY**都基于一个名为 "传感器\_属性"\_常见\_guid 的通用**GUID** ：
...
所有传感器都必须支持以下 Windows 便携设备（WPD）属性。
