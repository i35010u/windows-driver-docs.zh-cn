...
**HKEY\_本地\_计算机\\系统\\CurrentControlSet\\控件\\USBFN**
...
<tbody>
<tr class="odd">
<td><strong>供</strong></td>
<td>此子项包含其他子项，用于描述具有一个或多个备用设置的接口。</td>
</tr>
<tr class="even">
<td><strong>相关性</strong></td>
<td>此子项定义接口关联描述符（IADs）。 每个 IAD 允许将多个接口分组为一个函数。 每个子项表示不同的 IAD，Oem 可以修改这些子项的值。</td>
</tr>
<tr class="odd">
...
<td>这是一个 Microsoft 拥有的子项。 它包含默认配置的值。 如果在 <strong>HKEY_LOCAL_MACHINE \System\CurrentControlSet\Control\USBFN 键下将<strong>IncludeDefaultCfg</strong>值设置为1，则在当前配置出现之前添加默认配置中的接口。</td>
</tr>
<tr class="even">
...
下表描述了 Oem 可以在**HKEY\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN**项中定义的值。 未在此项中定义的值假设 Microsoft 在 HKEY 下定义的默认值 **\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN\\默认**值。

所有 Oem 必须设置**idVendor**、 **idProduct**、 **ManufacturerString**和**ProductString**值。 创建和添加自己的接口的 Oem 还必须将**CurrentConfiguration**设置为**HKEY\_本地\_计算机\\系统\\CurrentControlSet\\控件\\USBFN\\配置**，其中包括其在**InterfaceList**中的接口。

| 值 | 类型 | 所有者 | 说明|
|--|--|--|--|
| **IncludeDefaultCfg** | REG\_DWORD | OEM | 如果 Oem 要包括默认配置的接口（如 IpOverUsb 或 MTP），则设置为1。 |
| **idVendor** | REG\_DWORD | OEM | 枚举过程中发送到主机的设备描述符的供应商标识符。 |
| **idProduct** | REG\_DWORD | OEM | 枚举过程中发送到主机的设备描述符的产品标识符。 |
...
| **iSerialNumber**| REG\_DWORD | OEM | 如果此值设置为0，则设备不具有序列号。 如果此值不是零或不存在，则每个设备的序列号都是唯一的。 |
...
下表描述了 Oem 可为 HKEY 下的子项定义的值 **\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN\\配置**。 每个子项表示不同的 USB 配置。 如果 OEM 要创建自己的接口，则 OEM 必须定义一个新配置，其中包含要使用的接口。 为此，请在 "HKEY" 下创建一个子项 **\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN\\** 使用配置名称的配置，并使用此表中的值填充该子项。 此外，若要使 USB 驱动程序使用新配置，则必须将**CurrentConfiguration**值（前面的表中所述）设置为配置子项的名称。
...
下表描述了 Oem 可以为 HKEY 下的子项修改的值 **\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN\\接口**。

每个子项表示不同的 USB 接口。 若要定义接口，请在 "HKEY" 下创建一个子注册表 **\_本地\_计算机\\System\\CurrentControlSet\\** 使用该接口的名称\\"USBFN"，并使用下表中的值填充该接口。\\ 此外，如果接口是**CurrentConfiguration**的**InterfaceList**的一部分，则仅包含接口。

| 值                              | 类型        | 所有者            | 说明                                                                                                                                                                                                                                                                                                  |
|------------------------------------|-------------|------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **InterfaceDescriptor**            | REG\_二进制 | OEM 或 Microsoft | 要在 USB 枚举过程中发送到主机的接口描述符的二进制表示形式。 **BInterfaceNumber**和**iInterface**值将在编译完整配置描述符后由 USB 函数堆栈自动填充，以避免与其他接口描述符冲突。 |
| **InterfaceGUID**                  | REG\_SZ     | OEM 或 Microsoft | 一个 GUID，用于唯一标识总线上的接口。                                                                                                                                                                                                                                                     |
| **InterfaceNumber**                | REG\_DWORD  | OEM 或 Microsoft | 可选。 此值用于将固定的接口号分配给函数。 接口号 0-1F 是为传统函数保留的，20-3F 是为 Microsoft 保留的，40-5F 保留供 Oem 使用。                                                                                           |
| **MSOSExtendedPropertyDescriptor** | REG\_二进制 | OEM 或 Microsoft | 可选。 定义接口的扩展属性 OS 功能描述符。                                                                                                                                                                                                                              |
...
替代子项用于定义具有一个或多个备用接口的单个接口。 下表描述了 Oem 可为 HKEY 下的子项修改的值 **\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN\\备用**项。

每个子项表示不同的接口。 若要使用备用设置来定义接口，请在 "HKEY" 下创建一个子注册表 **\_本地\_计算机\\System\\CurrentControlSet\\** 使用该接口的名称将其替换为备用，并使用下表中的值对其进行填充。\\\\

| 值                              | 类型           | 所有者            | 说明                                                                                                                                                                                                                                                                                                                                                        |
|------------------------------------|----------------|------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **InterfaceList**                  | REG\_多\_SZ | OEM 或 Microsoft | 与在 HKEY 中定义的接口对应的两个接口名称的列表， **\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN\\接口**。 该密钥共同定义具有替代设置的接口。 第一个接口对应于备用设置0，第二个接口对应于替代设置1，依此类推。 |
| **InterfaceNumber**                | REG\_DWORD     | OEM 或 Microsoft | 可选。 此值用于将固定的接口号分配给函数。 接口号 0-1F 是为传统函数保留的，20-3F 是为 Microsoft 保留的，40-5F 保留供 Oem 使用。                                                                                                                                                 |
| **MSOSExtendedPropertyDescriptor** | REG\_二进制    | OEM 或 Microsoft | 可选。 定义接口的扩展属性 OS 功能描述符。                                                                                                                                                                                                                                                                                    |

 

## <a name="usbfnassociations-registry-key"></a>USBFN\\Association 注册表项


Oem 可以通过定义接口关联描述符（IADs）来指定关联。 每个 IAD 允许将多个接口分组为一个函数。 下表描述了 Oem 可以为 HKEY 下的子项修改的值 **\_本地\_计算机\\System\\CurrentControlSet\\控件\\USBFN\\关联**。

每个子项表示不同的 IAD。 若要定义关联，请在 "HKEY" 下创建一个子注册表 **\_本地\_计算机\\System\\CurrentControlSet\\** 使用 IAD 的名称\\"USBFN\\关联"，并使用下表中的值填充该关联。

| 值                              | 类型           | 所有者            | 说明                                                                                                                                                                                                                 |
|------------------------------------|----------------|------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **InterfaceList**                  | REG\_多\_SZ | OEM 或 Microsoft | 与 USB 函数关联的接口或备用接口的列表。 如果列表的大小小于2，则函数驱动程序堆栈无法加载。 其他函数或接口将继续加载。 |
| **bFunctionClass**                 | REG\_DWORD     | OEM 或 Microsoft | 函数的类代码，设置为02。                                                                                                                                                                                  |
| **bFunctionSubClass**              | REG\_DWORD     | OEM 或 Microsoft | 函数的子类代码，设置为0d。                                                                                                                                                                               |
| **bFunctionProtocol**              | REG\_DWORD     |                  | 函数的协议代码，设置为01。                                                                                                                                                                               |
| **MSOSExtendedPropertyDescriptor** | REG\_二进制    | OEM 或 Microsoft | 可选。 定义接口的扩展属性 OS 功能描述符。                                                                                                                                             |
...
-   通过设置上表中显示的注册表值，通过使用接口关联描述符（IAD）创建通信和数据接口的关联。
-   除了注册表设置，将此注册表值设置为非零值。

    | 值          | 类型       | 所有者            | 说明                                                                                                                     |
    |----------------|------------|------------------|---------------------------------------------------------------------------------------------------------------------------------|
    | **MirrorLink** | REG\_DWORD | OEM 或 Microsoft | 非零值指示接口支持 MirrorLink。 USB 函数堆栈不会延迟 MirrorLink USB 命令。 |
