...
是的，如果将手机连接到支持 USB 布告栏的设备，根据 [布告栏设备规范的 Usb 设备类定义](https://go.microsoft.com/fwlink/p/?linkid=620207)，会通知用户。 USB 连接器管理器 (UCM) 客户端驱动程序无需处理通知。 如果系统无法识别备用模式，请不要进入该模式。
...
USB 类型-C 连接器系统软件接口 (UCSI) 规范介绍了 USB C # 连接器系统软件接口 (UCSI) 的功能，并说明了硬件组件设计器、系统构建者和设备驱动程序开发人员的寄存器和数据结构。 从此 [站点](https://go.microsoft.com/fwlink/p/?LinkId=703713)获取规范。
