...
在前面的跟踪中，主机通过发送 PID 设置为 OUT （输出令牌）的令牌数据包，启动向高速大容量终结点的批量传出传输。 数据包包含设备和目标终结点的地址。 在输出数据包之后，主机将发送包含大容量有效负载的数据包。 如果终结点接受传入的数据，则会发送 ACK 数据包。 在此示例中，我们可以看到主机向设备地址发送了31个字节： 1;终结点地址：2。
...
在前面的跟踪中，主机将通过将 PID 设置为（在令牌中）的令牌数据包来启动该事务。 然后，设备将使用大容量有效负载发送数据包。 如果该终结点没有要发送的数据或尚未准备好发送数据，则该设备可以发送 NAK 握手数据包。 主机会重试传入传输，直至接收到设备收到的 ACK 数据包为止。 该 ACK 数据包表示设备已接受数据。
...
-   客户端驱动程序必须已创建框架 USB 目标设备对象，并通过调用[**WdfUsbTargetDeviceCreateWithParameters**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdfusb/nf-wdfusb-wdfusbtargetdevicecreatewithparameters)方法获取了 WDFUSBDEVICE 句柄。
    ...
    如果使用的是随 Microsoft Visual Studio Professional 2012 一起提供的 USB 模板，则模板代码将执行这些任务。 模板代码获取目标设备对象的句柄，并将其存储在设备上下文中。 有关详细信息，请参阅[了解 USB 客户端驱动程序代码结构（KMDF）中的](understanding-the-kmdf-template-code-for-usb.md)"设备源代码"。
...
### <a href="" id="step-1--get-the-transfer-buffer--"></a>步骤1：获取传输缓冲区。

传输缓冲区或传输缓冲区 MDL 包含要发送或接收的数据。 本主题假设你要在传输缓冲区中发送或接收数据。 在 WDF 内存对象中描述了传输缓冲区（请参阅[Wdf 内存对象引用](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdfmemory/)）。 若要获取与传输缓冲区关联的内存对象，请调用以下方法之一：
...
### <a href="" id="step-2--format-and-send-a-framework-request-object-to-the-usb-driver-stack-"></a>步骤2：设置框架请求对象的格式并将其发送到 USB 驱动程序堆栈。
...
此列表中的方法格式请求。 如果以异步方式发送请求，请通过调用[**WdfRequestSetCompletionRoutine**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdfrequest/nf-wdfrequest-wdfrequestsetcompletionroutine)方法（在下一步中介绍），设置指向驱动程序实现的完成例程的指针。 若要发送请求，请调用[**WdfRequestSend**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdfrequest/nf-wdfrequest-wdfrequestsend)方法。
...
### <a href="" id="step-3--implement-a-completion-routine-for-the-request-"></a>步骤3：为请求实现完成例程。
...
-   指向[**WDF\_的指针请求\_完成\_** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdfrequest/ns-wdfrequest-_wdf_request_completion_params)包含完成信息的 PARAMS 结构。 USB 特定的信息包含在 **&gt;CompletionParams 参数. usb**成员中。
...
-   通过获取**CompletionParams-&gt;IoStatus**值来检查请求的状态。
-   检查 USB 驱动程序堆栈设置的 USBD 状态。
-   如果出现管道错误，请执行错误恢复操作。 有关详细信息，请参阅[如何从 USB 管道中恢复错误](how-to-recover-from-usb-pipe-errors.md)。
-   检查传输的字节数。

    当请求的字节数已传入或传出设备时，将完成批量传输。 如果通过调用 KMDF 方法发送请求缓冲区，请检查在 CompletionParams 中收到的值 **-&gt;参数。 PipeWrite-&gt;** 或**CompletionParams-&gt;PipeRead-&gt;的参数. 长度**成员。
