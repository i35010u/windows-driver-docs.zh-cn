...
<p>本部分介绍 Windows 操作系统中用于开发模拟的通用串行总线（USB）主机控制器驱动程序和连接的虚拟 USB 设备的 USB 模拟设备（UDE）支持。 这两个组件组合成单个 KMDF 驱动程序，该驱动程序可以与 Microsoft 提供的 USB 设备模拟类扩展 (UdeCx) 通信。</p>
...
<ul>
<li>UdeCx：（UdeCx）函数驱动程序使用的 WDF 扩展。 此扩展包含在 Windows 中。</li>
<li>指向存根库（Udecxstub）的链接。 该存根库在 WDK 中。</li>
