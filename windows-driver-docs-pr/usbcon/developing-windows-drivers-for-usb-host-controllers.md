...
<p>UCX 是可扩展的，旨在支持各种主机控制器驱动程序。 Windows 提供了以 USB xHCI 主机控制器为目标的 xHCI 驱动程序（Usbxhci .sys）。</p>
<p>主机控制器驱动程序是 UCX 的客户端，以<a href="https://docs.microsoft.com/windows-hardware/drivers/debugger/kernel-mode-driver-framework-debugging" data-raw-source="[Kernel-Mode Driver Framework](https://docs.microsoft.com/windows-hardware/drivers/debugger/kernel-mode-driver-framework-debugging)">内核模式驱动程序框架</a>（KMDF）驱动程序形式写入。</p>
<p><strong>Microsoft 提供的二进制文件</strong></p>
<p>若要编写主机控制器驱动程序，需要 UCX （Ucx01000）和存根库（Ucx01000）。 存根库位于 Windows 驱动程序工具包（WDK）中。 库执行两个主要功能。</p>
<ul>
...
<p>阅读描述体系结构的不同组件（设备、主机控制器和集线器）的预期行为的官方规范。</p>
...
<p>UCX 会审传入的 USB 请求块（URBs），然后将其转发到正确的终结点队列。</p>
...
<p>提供客户端驱动程序使用的 i/o 请求、支持例程、结构和接口的规范。 这些例程和相关的数据结构是在 WDK 标头中定义的。</p>
