...
-   **系统**：台式机、便携式计算机、平板电脑、服务器或带有公开的类型 C USB 端口的电话。 系统必须运行 Windows 10 版本，如适用于桌面版的 Windows 10 （家庭版、专业版、企业版和教育版）、Windows 10 移动版或其他版本。
-   **Dock**：任何 USB 类型为 C 的设备，该设备公开多个端口。
-   **设备**：任何 USB 设备，都可以连接到系统或插接的类型为 C 的端口。 此类别包括传统 USB 设备以及支持 USB 类型 C 规范中定义的附件和备用模式的设备。

USB 类型 C 互操作性测试过程分为两部分：功能测试（FT）和压力测试（ST）。 每个测试部分介绍了测试用例并标识了应用于测试的类别。 必须针对整个适用的类别对产品进行测试。 某些测试用例包含指向相关提示的链接和有关其他信息的提示。 本文档重点介绍 USB 类型 C 功能和体验。 USB 类型 C 解决方案可能包含其他 USB 组件，如 USB 集线器或 USB 控制器。 USB [xHCI 互操作性测试过程](https://go.microsoft.com/fwlink/p/?LinkId=623257)和 Windows 硬件认证工具包中介绍了 usb 集线器和控制器的详细测试。
...
<a href="" id="charging-and-power-delivery--pd-"></a>[充电和电源交付（PD）](#ft7)  
确认用 USB 类型 C 进行收费。

<a href="" id="role-swap"></a>[角色交换](#ft8)  
确认角色交换。

压力测试部分介绍了用于在一段时间内测试设备稳定性的压力和边缘案例方案的过程。 压力测试需要自定义设备，即[MICROSOFT USB 测试工具（MUTT）设备](microsoft-usb-test-tool--mutt--devices.md)，用于旧式 USB 验证（非 USB 类型 C）。 可以通过即将出现的 USB 类型 C 测试设备实现其他测试和自动化。
...
5.  确认插入的设备不出现错误。
6.  断开设备连接（如果适用，则停靠）并观察**设备管理器**中的更改。 停靠和设备不应出现在**设备管理器**中。 有关详细信息，请参阅[如何确认设备删除](#remove)。
7.  [翻转或反转](#flip)USB 类型 C 电缆的方向，并重复步骤3到步骤6。

## <a href="" id="ft2"></a>FT 案例2：系统启动


适用于：系统、中心、设备

**确认所测试的产品不会阻止正常的系统启动过程**
...
## <a href="" id="ft3"></a>FT 案例3：系统电源转换


适用于： System、dock、device

**确认系统的电源转换与较低电源状态的唤醒功能是否不受产品影响**
...
7.  从低功耗状态唤醒系统。 如果设备支持远程唤醒，请使用设备唤醒系统。 有关详细信息，请参阅[系统唤醒故障排除](#wake)。 否则，请使用电源按钮或键盘正常唤醒系统。
8.  确认设备仍正常工作。 有关详细信息，请参阅[如何确认设备功能](#function)。

对其他可用系统电源状态重复此测试：睡眠（S3）、休眠（S4）和混合睡眠。

**请注意**  使用 Windows 驱动程序工具包（WDK）中包含的 pwrtest，以简化到电源状态的过渡。 有关详细信息，请参阅[PwrTest](https://docs.microsoft.com/windows-hardware/drivers/devtest/pwrtest)。

 

## <a href="" id="ft4"></a>FT 案例4：选择性挂起
...
-   [通过 USB 设备选择性挂起 HID](https://docs.microsoft.com/windows-hardware/drivers/hid/selective-suspend-for-hid-over-usb-devices)
-   [揭密选择性挂起]( https://go.microsoft.com/fwlink/p/?LinkId=623308)

## <a href="" id="ft5"></a>FT 事例5：停靠标识
...
**请注意**  在[2015 WinHEC 幻灯片演示文稿](https://go.microsoft.com/fwlink/p/?LinkId=623309)中可以找到有关停靠标识的其他信息（约为 "将设备标识为插接" 一节中的幻灯片26）。

 

## <a href="" id="ft6"></a>FT 情况6：备用模式协商
...
3.  将启用备用模式的 USB 类型-C 设备连接到系统上备用模式启用的 USB 类型 C 端口;如有必要，请确保设备和系统共享至少一种备用模式，并且设备已通电或连接到外部电源。
    **请注意**，对于类型-c 转换器/适配器  ，请确保已打开适当的外围设备并连接到转换器/适配器的非类型 C 端。
...
## <a href="" id="ft7"></a>FT 案例7：充电和电源交付（PD）
...
1.  按照 USB.org 的定义执行[USB 电源提供测试]( https://go.microsoft.com/fwlink/p/?LinkId=623310)。
2.  重新启动测试系统并登录到 Windows。
3.  对于系统，请执行以下步骤：
    1.  将两个系统与 USB 类型 C 电缆连接。 确认每个系统仅接收最新的。
    2.  如果系统包含多个 USB 类型 C 端口，请使用 USB 类型 C 电缆连接同一系统上的两个 USB 类型 C 端口。 确认系统不会充电（本身）。
    3.  将捆绑的 USB 类型 C 充电器（如果已捆绑）连接到系统的 USB 类型 C 端口。 确认系统正在充电。
...
5.  对于设备，请执行以下步骤：
    ...
    2.  可有可无将设备连接到支持 USB 类型 C 的系统。 确认设备将对系统计费。

## <a href="" id="ft8"></a>FT 事例8：角色交换
...
3.  [在（认证）测试过程中，通过 IO 运行 DF-睡眠]( https://go.microsoft.com/fwlink/p/?LinkId=623311)：
4.  使用 USB 类型 C 测试设备重复步骤3。

## <a href="" id="st2"></a>ST 案例2：传输事件
...
## <a href="" id="st3"></a>ST Case 3：即插即用
...
## <a href="" id="st4"></a>ST Case 4：设备拓扑
...
**请注意**  在测试过程中，验证是否没有通过 USB 类型-C A/V 转换器（如视频失真或音频下拉）连接的设备的 glitching。

 

## <a href="" id="ft-plan"></a>功能系统互操作性测试计划
...
-   系统： Windows 10Windows 10 系统（电脑、平板电脑或手机），具有公开的 USB 类型 C 端口。
-   外设
...
-   使用 USB 类型 C 充电器执行[FT 案例7：充电和电源交付（PD）](#ft7)的精简版。 跳过需要两台计算机的部分，仅验证系统是否能够使用第三方电源适配器对其进行计费（接受电源）。
...
-   外设
    ...
    -   USB 类型-C 存储驱动器
    -   USB 类型 C 视频（可以接受转换器）
    -   USB 类型-C A/V 转换器（包括视频、USB，甚至可能是单个适配器的音频）
...
-   Systems
    -   Windows 10Windows 10 系统（电脑、平板电脑或手机），具有公开的 USB 类型 C 端口。
    -   附加的 Windows 10Windows 10 系统（电脑、平板电脑或手机），具有公开的 USB 类型 C 端口。 带有公开 USB 类型 C 端口的系统（PC、平板电脑或手机）。 我们建议从另一个产品线或 OEM 进行系统。
-   外设
    ...
    -   USB 类型-C 存储驱动器
    -   USB 类型 C 视频（可以接受转换器）
    -   USB 类型-C A/V 转换器（包括视频、音频和 USB，作为单个单元）
-   电源：来自不同供应商的两个 USB 类型 C 充电器。
...
## <a href="" id="remove"></a>如何确认删除设备


-   在**设备管理器**中标识设备。
-   执行测试步骤，从系统中删除设备。
-   确认设备不再存在于**设备管理器**中。
-   对于 USB 3.0 集线器，请检查是否已删除这两个设备（SuperSpeed 和配套集线器）。 在这种情况下，删除设备失败可能是设备故障，并应通过与会审适当根本原因所涉及的所有组件进行调查。
...
如果测试系统无法进入睡眠或休眠状态，请确保系统中的所有设备都安装了最新的设备驱动程序。 最常见的原因之一是系统中不支持的视频卡。 有关解决系统电源状态问题的详细信息，请参阅[为什么无法使用睡眠？]( https://go.microsoft.com/fwlink/p/?LinkId=623315)。

## <a name="using-etw-to-log-issues"></a>使用 ETW 记录问题


若要为 USB 2.0 端口启用 ETW，请参阅[Windows 7 USB core 堆栈中的 etw](https://go.microsoft.com/fwlink/p/?LinkId=623316)。
...
## <a href="" id="analyzer"></a>使用分析器确认选择性挂起
...
当某个测试要求设备进入 "挂起" 状态时，你应该能够将上面的 &lt;挂起&gt; 事件关联到你预期设备进入挂起状态的时间。

## <a name="using-an-analyzer-to-confirm-lpm-u1-and-u2-transitions"></a>使用分析器确认 LPM U1 和 U2 转换


分析器跟踪应该显式显示每个链接状态转换：语句在事件中显示为 "Rx U0-&gt; U2"。 例如，通过使用 LeCroy software，在 "**报表**" 选项卡中选择 " **USB3" 链接状态计时视图**。 此选项显示时间轴上的链接状态。 请注意，在这种情况下，分析器可能不会正确显示 U1 到 U2 的转换。 你可能会看到链接状态进入 U1，但从 U2 恢复。
...
## <a href="" id="flip"></a>翻转或反转 USB 类型-C 线
