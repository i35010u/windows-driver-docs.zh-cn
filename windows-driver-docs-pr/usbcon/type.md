...
**注意**   有关停靠标识的其他信息，请参阅[2015 WinHEC 幻灯片演示文稿](https://go.microsoft.com/fwlink/p/?LinkId=623309)中的第26节 (大约第26节，主题：将设备标识为插接) 。
...
3.  将启用备用模式的 USB 类型-C 设备连接到系统上备用模式启用的 USB 类型 C 端口;如有必要，请确保设备和系统共享至少一种备用模式，并且设备已通电或连接到外部电源。
    **注意**   对于类型-C 连接器/适配器，请确保已打开适当的外围设备并连接到转换器/适配器的非类型 C 端。
...
**注意**   在测试过程中，验证是否没有通过 USB 类型-C A/V 转换器（如视频失真或音频下拉）连接的设备的 glitching。
