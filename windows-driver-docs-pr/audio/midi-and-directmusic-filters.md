...
-   它通过调用实例化 MIDI 端口驱动程序对象[ **PcNewPort** ](https://msdn.microsoft.com/library/windows/hardware/ff537715)使用的 GUID 值**CLSID\_PortMidi**。

-   它将调用端口驱动程序[ **IPort::Init** ](https://msdn.microsoft.com/library/windows/hardware/ff536943)方法绑定到端口驱动程序的微型端口驱动程序。

中的代码示例[子创建](subdevice-creation.md)说明了此过程。 通过与其他通信的端口和微型端口驱动程序及其[IPortMidi](https://msdn.microsoft.com/library/windows/hardware/ff536891)并[IMiniportMidi](https://msdn.microsoft.com/library/windows/hardware/ff536703)接口。
...
-   DLS （可下载声音） 包含描述 MIDI instruments 波形和清晰表述数据的资源。 一个[ **KSPROPERTY\_合成\_DLS\_下载**](https://msdn.microsoft.com/library/windows/hardware/ff537396)集属性请求将 DLS 资源下载到筛选器。

-   为扩展可选择 instruments 数信道组。 [ **DMU\_内核\_事件**](https://msdn.microsoft.com/library/windows/hardware/ff536340)结构，它用于打包 MIDI 流中的每个时间戳 MIDI 消息，指定要使用该消息的通道组。
...
-   它通过调用实例化 Dmu 端口驱动程序对象[ **PcNewPort** ](https://msdn.microsoft.com/library/windows/hardware/ff537715)使用的 GUID 值**CLSID\_PortDMus**。

-   它将调用端口驱动程序[ **IPort::Init** ](https://msdn.microsoft.com/library/windows/hardware/ff536943)方法绑定到端口驱动程序的微型端口驱动程序。

中的代码示例[子创建](subdevice-creation.md)说明了此过程。 通过与其他通信的端口和微型端口驱动程序及其[IPortDMus](https://msdn.microsoft.com/library/windows/hardware/ff536879)并[IMiniportDMus](https://msdn.microsoft.com/library/windows/hardware/ff536699)接口。
...
MIDI 筛选器上的 MIDI pin 实例公开[IMiniportMidiStream](https://msdn.microsoft.com/library/windows/hardware/ff536704)接口。 DirectMusic 筛选器的 MIDI 或 DirectMusic pin 实例公开[IMXF](https://msdn.microsoft.com/library/windows/hardware/ff536782)接口。
...
适配器驱动程序可以访问的系统提供的微型端口驱动程序通过调用[ **PcNewMiniport** ](https://msdn.microsoft.com/library/windows/hardware/ff537714)函数。 FMSynth 和 DMusUART 微型端口驱动程序，还提供作为示例音频驱动程序 Windows Driver Kit (WDK) 中。 通过修改这些示例中的源代码，硬件供应商可以扩展的驱动程序来管理其设备的专有功能。

DMusUART 是 Dmu 微型端口驱动程序的公开 MIDI 和 DirectMusic 插针，但不支持 DLS 下载或硬件序列化的示例。 微型端口驱动程序的 DirectMusic 呈现 pin 都有一个合成器节点 ([**KSNODETYPE\_合成器**](https://msdn.microsoft.com/library/windows/hardware/ff537203))，支持多个[KSPROPSETID\_合成器](https://msdn.microsoft.com/library/windows/hardware/ff537486)属性。 微型端口驱动程序包括本身类别 KSCATEGORY\_呈现器和 KSCATEGORY\_捕获，但不是在 KSCATEGORY\_合成器 （因为它不包含内部合成器）。 有关详细信息，请参阅 DMusUART 示例音频驱动程序 WDK 中。
