...
`KSPROPSETID_RTAudio`属性集指定 WaveRT 音频设备的属性。 在 Windows Vista 和更高版本的 Windows 操作系统中支持这些属性。

在以下属性定义，该属性是只读属性，且目标为 pin:

-   此属性中的所有属性都设置的支持**获取**属性从客户端，但不是请求**都设置**属性请求。

-   在此集中的所有属性，向其客户端发送的属性请求的目标是一个 pin 实例。 （其他 KS 属性设置，但是，筛选器实例上的支持属性。）
...
[**KSPROPERTY\_RTAUDIO\_查询\_通知\_支持**](ksproperty-rtaudio-query-notification-support.md)
