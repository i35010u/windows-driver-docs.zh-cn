---
title: KSNODETYPE\_音
description: KSNODETYPE\_音
ms.assetid: d9d55db0-8305-403f-8d0c-56c2cd736912
keywords:
- KSNODETYPE_TONE 音频设备
topic_type:
- apiref
api_name:
- KSNODETYPE_TONE
api_type:
- NA
ms.date: 11/28/2017
ms.localizationpriority: medium
ms.openlocfilehash: 934686bc19e3ca42bfca8a045c1415619972c7d6
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/31/2019
ms.locfileid: "56546432"
---
# <a name="ksnodetypetone"></a>KSNODETYPE\_音


## <span id="ddk_ksnodetype_tone_ks"></span><span id="DDK_KSNODETYPE_TONE_KS"></span>


KSNODETYPE\_音节点表示音控件。 音控件具有一个输入的流和一个输出流;两个流具有相同的数据格式。 它可以衰减的低音、 高音或输出流的频率中旬。 此外，它可以根据需要支持低音增强或提升。

KSNODETYPE\_音节点应支持至少一个以下属性：

[**KSPROPERTY\_AUDIO\_BASS**](ksproperty-audio-bass.md)

[**KSPROPERTY\_音频\_MID**](ksproperty-audio-mid.md)

[**KSPROPERTY\_AUDIO\_TREBLE**](ksproperty-audio-treble.md)

[**KSPROPERTY\_AUDIO\_BASS\_BOOST**](ksproperty-audio-bass-boost.md)

 

 





