...
ms.locfileid: "72833640"
---
# <a name="channel-mask"></a>声道掩码


在 Windows 中， [**WAVEFORMATEXTENSIBLE**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ksmedia/ns-ksmedia-waveformatextensible)结构定义多通道 PCM 音频流的数据格式。 此结构指定参数，例如每个 PCM 样本的位数、流中的通道数和通道掩码。 通道掩码指定到扬声器的通道映射。 下图显示通道掩码中的单个位。
...
上图的通道掩码中的发言人位置编码类似于用于[**KSPROPERTY\_音频\_通道\_CONFIG**](https://docs.microsoft.com/windows-hardware/drivers/audio/ksproperty-audio-channel-config)属性请求的属性值的编码。 有关详细信息，请参阅[**KSAUDIO\_信道\_CONFIG**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ksmedia/ns-ksmedia-ksaudio_channel_config)。
...
例如， **7.1 家庭影院扬声器**的配置通过 "0x63F" 的通道掩码值进行描述，这表示将流中的8个通道分配给以下扬声器位置（按以下顺序）： FL、FR、FC、LFE、BL、BR、SL 和 SR-IOV。 另一个示例是， **7.1 范围的配置扬声器**配置由通道掩码值0xff 描述，这表示将流中的八个通道分配给以下发言人位置： FL、FR、FC、LFE、BL、BR、FLC 和 FRC。
...
上图的右侧显示了使用8个扬声器环绕配置播放的同一7.1 通道流。 在这种情况下，每个小型黑色矩形表示一个扬声器。 7个扬声器映射到侦听器周围网格上的位置。 映射不会将网格位置分配给 LFE 扬声器（低音炮）;这种说法的假设是，这些扬声器通常仅生成低频率声音，这是 nondirectional 的。
