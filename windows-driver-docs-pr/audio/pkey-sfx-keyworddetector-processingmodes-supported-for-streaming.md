...
description: 在 Windows 10 及更高版本，主键\_SFX\_KeywordDetector\_ProcessingModes\_支持\_为\_流式处理属性键标识关键字检测器的流式处理驱动程序支持的处理模式。
...
ms.locfileid: "56547940"
---
# <a name="pkeysfxkeyworddetectorprocessingmodessupportedforstreaming"></a>PKEY\_SFX\_KeywordDetector\_ProcessingModes\_Supported\_For\_Streaming


在 Windows 10 及更高版本，**主键\_SFX\_KeywordDetector\_ProcessingModes\_支持\_有关\_流式处理**属性键标识流处理模式驱动程序支持关键字检测程序。 驱动程序开发人员应列出支持的流式处理其驱动程序支持的关键字检测器 pin 处理模式。
...
INF 文件指定音频处理过程中为该设备的添加注册表部分模式效果设置。 下面的示例 INF 显示字符串和添加注册表加载到注册表处理关键字检测程序 pin 模式下的模式影响的部分。
