...
description: 自定义加密服务提供程序 (Csp) 的第三方验证码签名已从 Windows Vista 开始提供, 并已通过此下载返回到2013到的 Windows XP SP3 和 Windows Server 2003 SP2。
...
ms.locfileid: "70025345"
---
# <a name="authenticode-signing-of-third-party-csps"></a>对第三方 CSP 进行验证码签名


自定义加密服务提供程序 (Csp) 的第三方验证码签名已从 Windows Vista 开始提供, 并已通过[此下载](https://support.microsoft.com/help/2836198)返回到2013到的 WINDOWS XP SP3 和 windows SERVER 2003 SP2。

因此, Microsoft 将不再对 Csp 进行签名, 并且手动 CSP 签名服务已停用。 Microsoft 将不再处理发送cspsign@microsoft.com到cecspsig@microsoft.com或要签名的电子邮件和 csp。

相反, 所有第三方 Csp 现在都可以通过执行以下步骤自签名:

1.  从证书颁发机构 (CA) 购买一个代码签名证书, Microsoft 还会颁发一个交叉证书。 "[内核模式代码签名的交叉证书](cross-certificates-for-kernel-mode-code-signing.md)" 主题提供了一个 ca 列表, 其中 Microsoft 还提供了交叉证书和相应的交叉证书。 请注意, 这些是与 Microsoft 颁发的 "Microsoft 代码验证根" 链接的唯一交叉证书, 这会使 Windows 运行第三方 Csp。
...
**请注意**  , 从 Windows 8 开始, 不再需要 csp 进行签名。
...
-   &lt;sha1_hash&gt;是与代码签名证书相对应的 SHA1 指纹
...
**请注意**  , 不需要在 csp DLL \#中包含资源 ID 666, 或在注册表中包含签名, 这是较旧 CSP 签名所需的。
