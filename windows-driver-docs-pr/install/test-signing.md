...
从 Windows Vista 开始，Windows 基于 x64 的版本要求在内核模式（包括驱动程序）下运行的所有软件都要进行数字签名。 最初，您可以使用 F8 开关（在每次启动时，在 Windows 加载之前）暂时禁用需要在您的驱动程序中使用有效签名的加载时间强制。 但这在第一次使用后将变得枯燥乏味。 您可以将内核调试器附加到您的测试计算机上，这将在您使用正确的 BCDEdit 命令后禁用相同的加载时间强制检查。 但是，最终需要在其开发期间对驱动程序进行测试签名，并最终在将驱动程序发布给用户之前对其进行签名。
...
命令提示符的五个快捷方式中的任何一个都将具有 makecert、inf2cat、signtool、certmgr.msc 等命令。
...
4.  *摘录自*[创建测试证书](creating-test-certificates.md)：
    ...
    -   **-N CN =** option 指定证书的名称（"Contoso .com"）。 此名称与[**SignTool**](https://docs.microsoft.com/windows-hardware/drivers/devtest/signtool)工具一起用于标识证书。
    ...
    *摘录自*[查看测试证书](viewing-test-certificates.md)：
    ...
    若要查看有关 Contoso （Test）证书的详细信息，请在右窗格中双击该证书。 以下屏幕截图显示了有关证书的详细信息。
...
7.  *修改后的摘录*[通过嵌入的签名对驱动程序进行测试签名](test-signing-a-driver-through-an-embedded-signature.md)：
    ...
    如果单击 "查看签名"，将看到下面的屏幕截图，其中提供了 "查看证书" 的下一个查看选项，然后从对话框本身提供 "安装证书" 选项。 尽管我们使用 certmgr.msc 工具来提供安装证书的首选命令行选项。
...
3.  验证 cat 文件和 sys 文件的签名。 打开提升的命令窗口，并假设计算机上有 signtool，请访问 cat、inf 和 sys 文件所在的驱动程序包目录。 在相应的目录中执行以下命令
    ...
    [验证版本签名驱动程序文件的签名](verifying-the-signature-of-a-release-signed-driver-file.md)：
    ...
    现在，驱动程序已准备好在测试计算机中进行安装和测试。 始终建议将以下注册表项设置为正确，以便在安装过程中收集 setupapi.log 文件（适用于 Windows Vista 和更高版本的操作系统）中的详细日志。
    ...
    成功安装驱动程序后，可以在开发计算机或测试计算机中对驱动程序进行测试。
...
1.  通过使用 Dpinst （Dpinst）工具，该工具是一个用于安装驱动程序的 WDK 命令行工具，并且是可再发行的。
2.  使用 Devcon （devcon）工具，它是用于安装驱动程序的 WDK 命令行工具，但不可再发行。 WDK 中提供了 Devcon 工具的示例代码。 若要重新分发，你可以从示例代码实现你自己的 Devcon 工具，并可以重新发布你的工具版本。
3.  通过使用 OS 提供的 Pnputil （Pnputil）工具。
4.  使用 "Windows 添加硬件向导"。

Dpinst 和 Pnputil 预安装驱动程序包，而借助 Devcon 和 Windows 添加硬件向导，还可以安装驱动程序和设备。 安装驱动程序后，操作系统会在设备连接到计算机时帮助操作系统查找驱动程序。

**使用 DPInst 安装（和卸载）驱动程序包**

1.  打开提升的命令窗口，并将默认目录设置为 c\\： toaster。
2.  在 WDK Dpinst 目录中提供了 x86 版本、amd64 版本和 ia64 版本。 将相关版本复制到 c：\\toaster 目录，并运行以下命令。
...
1.  打开提升的命令窗口，并将默认目录设置为 c\\： toaster。
2.  在 WDK 工具目录中提供了一种 x86 版本、amd64 版本和 ia64 版本。 将相关版本复制到 c：\\toaster 目录，并运行以下命令。 此命令将安装驱动程序和设备。
    ...
    删除具有指定硬件或实例 ID 的设备。 仅在本地计算机上有效。 （若要在必要时重启，请包含-r。）
...
1.  打开提升的命令窗口
2.  运行 hdwwiz 以启动添加硬件向导，并单击 "下一步" 以前往第二页
3.  选择 "高级" 选项并单击 "下一步"
4.  从列表框中选择 "显示所有设备"，然后单击 "下一步"
5.  选择 "具有磁盘" 选项
6.  输入包含 C：\\toaster 驱动程序包的文件夹的路径
7.  选择 inf 文件并单击 "打开"
8.  单击“确定”
...
3.  若要打开驱动程序的 "属性" 对话框，请双击 "Toaster Package Sample Toaster"
4.  若要确认 Toaster 是否正常工作，请在 "常规" 选项卡上选中 "状态" 框

可以使用设备管理器从 "属性" 对话框中卸载设备和驱动程序。
