---
title: GUID_CLASS_INPUT
description: GUID_CLASS_INPUT
ms.assetid: 71619883-6ef8-411b-862d-a7cbf12e22bd
keywords:
- GUID_CLASS_INPUT 设备和驱动程序安装
topic_type:
- apiref
api_name:
- GUID_CLASS_INPUT
api_location:
- Hidclass.h
api_type:
- HeaderDef
ms.localizationpriority: medium
ms.date: 10/17/2018
ms.openlocfilehash: faa3437f37eac87a062153f21e7ad87d9f474316
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/23/2019
ms.locfileid: "63392472"
---
# <a name="guidclassinput"></a>GUID_CLASS_INPUT


GUID_CLASS_INPUT 是已过时标识符[设备接口类](https://msdn.microsoft.com/library/windows/hardware/ff541339)有关[HID 集合](https://msdn.microsoft.com/library/windows/hardware/ff539861)。 从 Microsoft Windows 2000 开始，使用[ **GUID_DEVINTERFACE_HID** ](guid-devinterface-hid.md)此类的新实例的类标识符。

<a name="requirements"></a>要求
------------

<table>
<colgroup>
<col width="50%" />
<col width="50%" />
</colgroup>
<tbody>
<tr class="odd">
<td align="left"><p>Version</p></td>
<td align="left"><p>已过时。 从 Windows 2000 开始，请改用 GUID_DEVINTERFACE_HID。</p></td>
</tr>
<tr class="even">
<td align="left"><p>Header</p></td>
<td align="left">Hidclass.h （包括 Hidclass.h）</td>
</tr>
</tbody>
</table>

## <a name="see-also"></a>请参阅


[**GUID_DEVINTERFACE_HID**](guid-devinterface-hid.md)

 

 






