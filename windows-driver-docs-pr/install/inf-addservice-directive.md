...
指定在*setupapi.log*中定义的以下系统定义的标志的一个或多个（运算），表示为十六进制值：

<a href="" id="0x00000001--spsvcinst-tagtofront-"></a>**0x00000001** （SPSVCINST_TAGTOFRONT）  
将命名服务的标记移到其组顺序列表的前面，以确保在该组中首先加载该服务的标记（除非随后使用此 INF 规范进行了安装的设备视角）。 通过 WDM 驱动程序安装专用设备和设备的 INF 文件不应设置此标志。

<a href="" id="0x00000002--spsvcinst-assocservice-"></a>**0x00000002** （SPSVCINST_ASSOCSERVICE）  
为由此 INF 文件安装的设备分配指定的服务作为 PnP 函数驱动程序（或旧驱动程序）。
...
如果系统中已存在此服务，请不要覆盖给定的服务的（可选）描述。

<a href="" id="0x00000400--spsvcinst-clobber-security---windows-xp-and-later-versions-of-windows--"></a>**0x00000400** （SPSVCINST_CLOBBER_SECURITY）（windows XP 及更高版本的 windows）   
...
引用由 INF 写入方定义的部分，其中包含用于为此设备（或设备）安装命名服务的信息。 有关详细信息，请参阅下面的 "**备注**" 部分。

<a href="" id="event-log-install-section"></a>*事件日志-安装-部分*  
（可选）引用由 INF 写入方定义的部分，其中设置了此设备（或多个设备）的事件日志记录服务。
...
例如，可以检测到旧设备的 PnP 驱动程序应在其 INF 中指定此值，以便调用其[*DriverEntry*](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nc-wdm-driver_initialize)例程查找旧设备，即使该设备无法由 PnP 管理器枚举。
...
根据启动方案，可以使用**BootFlags**注册表值来控制操作系统应将驱动程序的**StartType**值升级为0x0 （SERVICE_BOOT_START）。 可以指定以下数值的一个或多个（运算），以十六进制值表示：
...
-   名为**EventMessageFile**的值项的类型为[REG_EXPAND_SZ](https://docs.microsoft.com/windows/desktop/SysInfo/registry-value-types)，由 FLG_ADDREG_TYPE_EXPAND_SZ 值**0x00020000**指定。 其值括在双引号（"）中，它将系统提供的*IoLogMsg* （但它可以将其他日志记录 dll 关联）与驱动程序二进制文件相关联。 通常，按如下所示指定每个文件的路径：
