...
**注意**：为了使软件枚举的组件设备正常运行，必须启动其父项。 如果父设备没有可用的驱动程序，则驱动程序开发人员可以创建自己的驱动程序，还可以选择利用直通驱动程序 "umpass"。 此驱动程序包含在 Windows 中，并且实际上不会启动设备。 为了使用 umpass，开发人员应将每个可能的 [DDInstall] 部分的[DDInstall 部分](inf-ddinstall-section.md)中的 Include/需求 INF 指令用于 \* 相应的 [umpass \* ] 部分，而不管 INF 是否为该部分指定了任何指令：
...
1. 使用软件组件的设备实例 ID 调用[**CM_Locate_DevNode**](https://docs.microsoft.com/windows/desktop/api/cfgmgr32/nf-cfgmgr32-cm_locate_devnodea)以检索设备句柄。
2. 调用[**CM_Get_Parent**](https://docs.microsoft.com/windows/desktop/api/cfgmgr32/nf-cfgmgr32-cm_get_parent)检索该设备父对象的句柄。  此父级是使用[INF AddComponent 指令](inf-addcomponent-directive.md)添加软件组件的设备。
3. 然后，若要检索父级的设备实例 ID，请从[**CM_Get_Parent**](https://docs.microsoft.com/windows/desktop/api/cfgmgr32/nf-cfgmgr32-cm_get_parent)中的句柄调用[**CM_Get_Device_ID**](https://docs.microsoft.com/windows/desktop/api/cfgmgr32/nf-cfgmgr32-cm_get_device_idw) 。

如果软件组件仅以桌面[目标平台](../develop/target-platforms.md)为目标，请使用以下过程：

1. 调用[**SetupDiCreateDeviceInfoList**](https://docs.microsoft.com/windows/desktop/api/setupapi/nf-setupapi-setupdicreatedeviceinfolist)创建一个空的设备信息集。
2. 调用具有软件组件设备的设备实例 ID 的[**SetupDiOpenDeviceInfo**](https://docs.microsoft.com/windows/desktop/api/setupapi/nf-setupapi-setupdiopendeviceinfoa) 。
3. 调用[**SetupDiGetDeviceProperty**](https://docs.microsoft.com/windows/desktop/api/setupapi/nf-setupapi-setupdigetdevicepropertyw) ， `DEVPKEY_Device_Parent` 以检索父的设备实例 ID。
...
对于常规 Inf，与组件 Inf 相同的驱动程序验证和提交过程是相同的。 有关详细信息，请参阅[WINDOWS HLK 入门](https://docs.microsoft.com/windows-hardware/test/hlk/getstarted/windows-hlk-getting-started)。

有关安装程序类的详细信息，请参阅[供应商可用的系统定义的设备安装程序类](https://docs.microsoft.com/windows-hardware/drivers/install/system-defined-device-setup-classes-available-to-vendors)。
