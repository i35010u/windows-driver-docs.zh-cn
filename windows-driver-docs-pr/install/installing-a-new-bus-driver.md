...
1.  使用唯一的 GUID 来标识总线类型。 总线驱动程序报告子设备的总线类型 (表示为物理设备对象 (*PDO*) 以响应[ **IRP_MN_QUERY_BUS_INFORMATION** ](https://msdn.microsoft.com/library/windows/hardware/ff551654)为请求子设备。 此类请求的响应，总线驱动程序返回一个指向[ **PNP_BUS_INFORMATION** ](https://msdn.microsoft.com/library/windows/hardware/ff559608)结构，它返回中的 GUID **PNP_BUS_INFORMATION。BusTypeGuid**成员。 此外，总线驱动程序应设置**PNP_BUS_INFORMATION。LegacyBusType**到**PNPBus**，和**PNP_BUS_INFORMATION。BusNumber**为适当的自定义值。
