...
为集成的设备电子学 (IDE) 设备标识符类似于 SCSI 标识符。 设备 ID 格式如下所示：
...
- *v(40)* 是一个字符串，包含供应商名称、 下划线、 供应商的产品名称和足够下划线来使总 40 个字符。
...
如下所示的 SCSI 情况下，不存在只有一个兼容 ID，类似于标准 SCSI 类型名称由 SCSI 端口驱动程序提供，但有十只有一个条目，而不是十八的泛型类型名称。 IDE 设备的通用设备类型名称如下所示：
...
对于 IDE 换带机设备，泛型类型名称是**GenChanger**而不是**ScsiChanger**通信设备对于泛型类型名称是**GenNet**而不是**ScsiNet**。 SCSI 端口驱动程序返回的顺序访问和"处理器"的设备，根本没有通用名称，而 IDE 总线驱动程序将返回**GenSequential**并**GenProcessor**。 此外，IDE 总线驱动程序返回十个泛型类型，而 SCSI 端口驱动程序当前将返回十八。 在其他方面，返回的 IDE 总线驱动程序的一般名称是由 SCSI 端口驱动程序返回的相同。

IDE 磁带驱动器的兼容 ID 如下所示：

GenSequential

LS-120 设备的特殊情况下，在 IDE 总线驱动程序返回以下兼容 ID:

GenSFloppy

下面显示了可为 IDE 硬盘驱动器生成的标识符的类型：
