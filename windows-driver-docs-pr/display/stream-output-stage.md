...
通过流输出写入的输出量是动态的因为新的绘图，类型[ **DrawAuto**](https://msdn.microsoft.com/library/windows/hardware/ff556123)，需要允许流输出缓冲区，而不重用与输入装配器若要确定实际写入的数据量的 CPU 参与。 此外，查询所需缓解流输出溢出，以及检索多少数据写入到流输出缓冲区 (D3D10DDI\_查询\_STREAMOVERFLOWPREDICATE 和 D3D10DDI\_查询\_STREAMOUTPUTSTATS [ **D3D10DDI\_查询**](https://msdn.microsoft.com/library/windows/hardware/ff541850)枚举)。
