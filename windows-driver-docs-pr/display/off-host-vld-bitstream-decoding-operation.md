...
发送反量化矩阵缓冲区以初始化位流解码的反量化矩阵。 反量化矩阵缓冲区提供有关如何对位流中的所有当前和后续视频进行解码的信息，直到提供新的反量化矩阵缓冲区。 （因此，反量化矩阵是永久性的。）一次只能将一个反量化矩阵缓冲区从主机发送到加速器。 [**DXVA\_QmatrixData**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dxva/ns-dxva-_dxva_qmatrixdata)结构加载量化视频照片解码的量化矩阵数据。
...
切片控制缓冲区指导 VLD 位流处理的操作。 主机软件解码器确定位流中切片级别重新同步点的位置。 切片定义为 multimacroblock 层，其中包含位流数据中的重新同步点。 在261轨道中，将块（GOB）的261组视为切片。 在轨道中，一个或多个 GOBs 从 GOB 开始代码开始，不包含其他 GOB 开始代码的序列被视为切片。 切片控制缓冲区包含[**DXVA\_SliceInfo**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dxva/ns-dxva-_dxva_sliceinfo)切片控制结构的数组，该结构应用于相应位流数据缓冲区的内容。
...
1.  除了 MPEG-2 和 MPEG-2 以外，每张图片的第一个位流缓冲区必须以所有数据（如果有）开头，后跟在位流中当前图片的第一个切片之前的所有数据（例如，序列标头或图片标题）。

2.  对于 MPEG-2 和 MPEG-2，每个图片的第一个位流缓冲区必须从图片的第一个切片的切片起始代码开始（例如，没有序列标头或图片标题），因为在其他参数中提供了所有相关数据。
