---
title: 报告裁切矩形支持
description: 报告裁切矩形支持
ms.assetid: fd44bdf4-b7ca-43da-92ea-7ca18185b5d4
keywords:
- 剪刀矩形 WDK DirectX 9.0 中，报告支持
- 矩形的剪辑区域 WDK DirectX 9.0 中，报告支持
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 69ea83b352eedca2c4f1bab3f02e6c415b2824b4
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/23/2019
ms.locfileid: "63383241"
---
# <a name="reporting-support-for-scissor-rectangle"></a>报告裁切矩形支持


## <span id="ddk_reporting_support_for_scissor_rectangle_gg"></span><span id="DDK_REPORTING_SUPPORT_FOR_SCISSOR_RECTANGLE_GG"></span>


DirectX 9.0 版本驱动程序指示其设备支持通过剪式测试通过设置 D3DPRASTERCAPS\_SCISSORTEST 功能位**RasterCaps** D3DCAPS9 结构中的成员。

 

 





