...
该驱动程序指定 D3DFORMAT\_OP\_DMAP 标志**dwOperations**的成员[ **DDPIXELFORMAT** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ksmedia/ns-ksmedia-_ddpixelformat)为特定的结构要将标记为位移映射采样格式的图面上格式。 Direct3D 运行时创建纹理图面时，设置 DDSCAPS3\_DMAP 位**dwCaps3** DDSCAPSEX 的成员 ([**DDSCAPS2**](https://docs.microsoft.com/previous-versions/windows/hardware/drivers/ff550292(v=vs.85))) 结构若要指示可以进行纹理采样的分割单元中。
