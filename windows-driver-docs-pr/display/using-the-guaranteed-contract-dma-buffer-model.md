...
在确保协定模式下，用户模式显示驱动程序可以识别 DMA 缓冲区和修补程序位置列表的准确大小，当用户模式显示驱动程序填充命令缓冲区并调用[**pfnRenderCb**](https://docs.microsoft.com/windows-hardware/drivers/ddi/d3dumddi/nc-d3dumddi-pfnd3dddi_rendercb)将其提交到显示微型端口驱动程序。 对**pfnRenderCb**的每次调用后，用户模式显示驱动程序将接收可用于以下转换的 DMA 缓冲区和修补位置列表的大小（即，对**pfnRenderCb**的以下调用）。
