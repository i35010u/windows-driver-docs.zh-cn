...
子设备（包括视频捕获设备）的每个驱动程序都必须返回适配器 GUID，指示与设备关联的硬件。 适配器 GUID 提供给[**DXGK\_启动\_INFO**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/ns-dispmprt-_dxgk_start_info)结构的**AdapterGuid**成员中的显示微型端口驱动程序，该驱动程序由在初始化适配器时发送的[**DxgkDdiStartDevice**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_start_device)函数的*DxgkStartInfo*参数指向。 用户模式捕获组件随后可以将此适配器 GUID 映射到显示适配器。
