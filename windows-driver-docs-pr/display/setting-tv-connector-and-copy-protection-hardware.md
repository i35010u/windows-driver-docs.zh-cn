...
为任何通过中的微型端口驱动程序设置的位**dwFlags**的成员[ **VIDEOPARAMETERS** ](https://docs.microsoft.com/windows/desktop/api/tvout/ns-tvout-_videoparameters)上副总裁\_命令\_GET、 微型端口驱动程序可以执行一组副总裁\_命令\_设置。 调用要设置仅微型端口驱动程序为其指示副总裁上的支持该功能的微型端口驱动程序的调用方负责\_命令\_获取。 微型端口驱动程序应响应副总裁\_命令\_设置设置为其每个 VIDEOPARAMETERS 字段中设置了相应的位的值与硬件**dwFlags**。 例如：
