---
title: 显示与 VPE 交错的视频
description: 显示与 VPE 交错的视频
ms.assetid: f78ccefb-77e3-43a4-88ad-d98c69259da9
keywords:
- DirectX VPE 支持 WDK DirectDraw，显示交错的视频
- 绘制 VPEs WDK DirectDraw，显示交错视频
- DirectDraw VPEs WDK Windows 2000 显示，显示交错的视频
- 视频端口扩展 WDK DirectDraw，显示交错的视频
- VPEs WDK DirectDraw，显示交错的视频
- 显示交错的视频
- 交错的视频显示 WDK 的视频端口扩展
- 交错视频 WDK 的视频端口扩展
- 取消隔行扫描 WDK 的视频端口扩展
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 9c9700a4898d1a602da3db71d0ca6b678b1959cc
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/31/2019
ms.locfileid: "56521994"
---
# <a name="displaying-interleaved-video-with-vpe"></a>显示与 VPE 交错的视频


## <span id="ddk_displaying_interleaved_video_with_vpe_gg"></span><span id="DDK_DISPLAYING_INTERLEAVED_VIDEO_WITH_VPE_GG"></span>


去隔行存在很多方法。 专业电视生成者时去隔行为大尺寸 rear 投影显示和效果系统与动态自适应的筛选器创建缩放和慢动作序列时使用行 doublers 等设备。

两个简单的方法了可用于渐进式计算机监视器上显示隔行扫描： [bob](bob-method.md)并[将](weave-method.md)。 这些条款是为简单起见，此处使用，因为计算机和电视行业术语不同。

 

 





