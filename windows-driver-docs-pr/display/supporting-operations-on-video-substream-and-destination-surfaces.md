...
除了视频子流上的操作显示您的驱动程序[ **DeinterlaceBltEx** ](https://msdn.microsoft.com/library/windows/hardware/ff563927)并[ **DeinterlaceBlt** ](https://msdn.microsoft.com/library/windows/hardware/ff563924)函数执行，您的驱动程序必须支持以下操作：
...
VMR 和其他 Microsoft DirectShow 组件必须能够以填充为已知的初始颜色值，如透明的黑色的视频子流图面。 因此，您的驱动程序应支持对调用其[ *DdBlt* ](https://msdn.microsoft.com/library/windows/hardware/ff549205)回调函数使用 DDBLT\_COLORFILL 标志视频子流图面所在的目标位块传输 （blt)。
...
Line21 关闭标题解码器和 teletext 解码器创建包含一系列缓存字符标志符号的源视频的子流图面。 通过将适当字符标志符号缓存中复制到视频子流图面，您的驱动程序应生成输出的每个帧。 VMR 然后向您的驱动程序发送视频子流图面[ **DeinterlaceBltEx** ](https://msdn.microsoft.com/library/windows/hardware/ff563927)函数。

因此，您的驱动程序的[ *DdBlt* ](https://msdn.microsoft.com/library/windows/hardware/ff549205)函数应支持将任何 FOURCC 面复制到相同的 FOURCC 格式的视频子流图面。

您的驱动程序应指示它通过设置 DDCAPS2 支持复制 FOURCC 格式\_COPYFOURCC 标志**dwCaps2**的成员[ **DDCORECAPS** ](https://msdn.microsoft.com/library/windows/hardware/ff549248)结构。 该驱动程序指定 DDCORECAPS 结构中的**ddCaps**的成员[ **DD\_HALINFO** ](https://msdn.microsoft.com/library/windows/hardware/ff551627)结构。 DD\_驱动程序的返回 HALINFO [ **DrvGetDirectDrawInfo** ](https://msdn.microsoft.com/library/windows/hardware/ff556229)函数。
...
您的驱动程序必须在您的驱动程序支持以下操作上使用的目标面[ **DeinterlaceBltEx** ](https://msdn.microsoft.com/library/windows/hardware/ff563927)函数：
...
因为 VMR 必须初始化到 YUV 不透明的黑色，您的驱动程序必须在目标面还支持调用其[ *DdBlt* ](https://msdn.microsoft.com/library/windows/hardware/ff549205)回调函数使用 DDBLT\_COLORFILL 标记的位置位块传输的目标是目标图面。 VMR 指定的不透明黑色墨水颜色**dwFillColor** DDBLTFX 结构中的成员。 该驱动程序接收中的 DDBLTFX 结构**bltFX** DD 成员\_BLTDATA 结构时其*DdBlt*调用。
