...
所有目标为客户端 Sku 的 Windows 8 系统都必须支持统一可扩展固件接口（UEFI）图形输出协议（GOP）。 在启动阶段，GOP 在系统的集成显示面板上设置本机计时和本机分辨率。 当操作系统准备接管显示的所有权时，GOP 会直接退出帧缓冲区，该缓冲区可用于向外浏览。 此时，操作系统不会尝试重置显示时间或解决方法，而只是使用提供的帧缓冲区，从而消除了一个屏幕闪烁。

**硬件认证要求**

有关硬件设备实现此功能时必须满足的要求的信息，请参阅 WHCK**上的**相关[文档](https://docs.microsoft.com/windows-hardware/test/hlk/windows-hardware-lab-kit)。
...
启动后，当操作系统将显示的所有权移交给 WDDM 驱动程序时，它会通过调用[*DxgkDdiStartDevice*](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_start_device)函数启动设备即插即用（PnP）启动。 或者，在从休眠状态恢复后，操作系统将调用[*DxgkDdiSetPowerState*](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_set_power_state)函数来启动设备，并将*DeviceUid*参数设置为**显示\_适配器\_HW\_ID** （在中定义）视频点播）。 此时，在 WDDM 图形驱动程序控制时，屏幕通常会遮蔽（呈现为黑色）。

驱动程序可以调用[**DxgkCbAcquirePostDisplayOwnership**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkcb_acquire_post_display_ownership)函数（在 Windows 8 中开始提供），在操作系统中查询当前帧缓冲区的确切状态以及固件和启动加载程序设置的显示模式。 使用 DXGK 中的信息[ **\_显示**](https://docs.microsoft.com/windows-hardware/drivers/ddi/d3dkmdt/ns-d3dkmdt-_dxgk_display_information)此函数检索到\_信息结构，驱动程序可以使显示控制器保持活动状态，而不会导致监视器重新同步。 由于驱动程序还包含有关帧缓冲区的详细信息，因此可以执行更平稳的转换。

在[WDDM 1.2 和更高版本的即插即用（PnP）](plug-and-play--pnp--start-and-stop-cases.md)中提供了有关 PnP 启动的更多详细信息。
...
操作系统可以通过调用[*DxgkDdiStopDevice*](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_stop_device)函数请求显示设备的 PnP 停止。 此时，在操作系统接管显示控制时，屏幕通常会遮蔽（呈现为黑色）。 操作系统可以调用[*DxgkDdiStopDeviceAndReleasePostDisplayOwnership*](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_stop_device_and_release_post_display_ownership)函数（在 Windows 8 中开始提供），该函数要求 WDDM 驱动程序设置配置用于扫描的帧缓冲区。操作系统可以在此帧缓冲区内呈现，同时控制其显示，使其可以执行平滑转换。

在[WDDM 1.2 和更高版本的即插即用（PnP）](plug-and-play--pnp--start-and-stop-cases.md)中提供了有关 PnP 停止的更多详细信息，包括其他方案。

**硬件认证要求**

有关此移交的详细信息，请参阅相关[WHCK 文档](https://docs.microsoft.com/windows-hardware/test/hlk/windows-hardware-lab-kit) **PnpStopStartSupport**。
...
有时，操作系统会遇到不可恢复的错误，并且必须发出系统 bug 检查。 发生这种情况时，某些情况下操作系统必须控制显示，但不能停止 WDDM 驱动程序。 需要使用 WDDM 1.2 和更高版本的驱动程序来实现[*DxgkDdiSystemDisplayEnable*](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_system_display_enable)和[*DxgkDdiSystemDisplayWrite*](https://docs.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_system_display_write)函数，使操作系统无缝地转换为可在其中显示错误屏幕的状态以高分辨率和颜色深度保持图形界面。 此过渡消除了 jarring 的用户体验。

**硬件认证要求**

有关硬件设备实现此功能时必须满足的要求的信息，请参阅相关[WHCK 文档](https://docs.microsoft.com/windows-hardware/test/hlk/windows-hardware-lab-kit) **DisplayOutputControl**。
...
<span id="_1.2_and_later_drivers__dxgkddi_interface_version____dxgkddi_interface_version_win8_"></span><span id="_1.2_AND_LATER_DRIVERS__DXGKDDI_INTERFACE_VERSION____DXGKDDI_INTERFACE_VERSION_WIN8_"></span>WDDM 1.2 和更高版本的驱动程序（**DXGKDDI\_接口\_版本**&gt;= **DXGKDDI\_接口\_版本\_WIN8**）  
为了进一步消除显示闪烁，从 Windows 8 开始，不会在 WDDM 1.2 和更高版本驱动程序的固件上调用 Int10 模式更改请求。

此外，如果在关闭监视器后发生模式更改，操作系统只调用[*DxgkDdiCommitVidPn*](https://docs.microsoft.com/windows-hardware/drivers/ddi/d3dkmddi/nc-d3dkmddi-dxgkddi_commitvidpn)函数一次，并将*pCommitVidPnArg*参数设置为它在监视器已打开时的值，并且*PCommitVidPnArg*-的**PathPoweredOff**成员设置为**TRUE**&gt;**标志**。

<span id="_1.0_and_1.1_drivers__DXGKDDI_INTERFACE_VERSION___DXGKDDI_INTERFACE_VERSION_WIN8_"></span><span id="_1.0_and_1.1_drivers__dxgkddi_interface_version___dxgkddi_interface_version_win8_"></span><span id="_1.0_AND_1.1_DRIVERS__DXGKDDI_INTERFACE_VERSION___DXGKDDI_INTERFACE_VERSION_WIN8_"></span>WDDM 1.0 和1.1 驱动程序（**DXGKDDI\_接口\_版本**&lt; **DXGKDDI\_接口\_版本\_WIN8**）  
...
此外，如果在关闭监视器后发生模式更改，操作系统会按上述针对 WDDM 1.2 驱动程序调用[*DxgkDdiCommitVidPn*](https://docs.microsoft.com/windows-hardware/drivers/ddi/d3dkmddi/nc-d3dkmddi-dxgkddi_commitvidpn)函数，并在*第二*次调用*DxgkDdiCommitVidPn* *pCommitVidPnArg*中的视频呈现网络（VidPN）-&gt;**HFunctionalVidPn** ，且*pCommitVidPnArg*中未设置的任何标志值-&gt;**标志**。

如果在休眠后恢复系统，并且监视同步生成仍处于启用状态，则也会发生这两个部分的调用序列。 在这种情况下，当驱动程序收到第二次调用[*DxgkDdiCommitVidPn*](https://docs.microsoft.com/windows-hardware/drivers/ddi/d3dkmddi/nc-d3dkmddi-dxgkddi_commitvidpn)时，该驱动程序应不执行任何操作。
