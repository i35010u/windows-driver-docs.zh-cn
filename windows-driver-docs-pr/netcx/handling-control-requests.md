...
在 NetAdapterCx 模型中，客户端可以使用两种类型的队列来处理控制请求（Oid）：
* 普通请求（Oid）的顺序队列。 NetAdapterCx 每次向客户端传递一次请求。
* 直接请求（Oid）的并行队列。 NetAdapterCx 以并行方式传递请求。
...
对于三个主要请求类型（查询数据、设置数据和方法）中的每一个，客户端驱动程序都可以提供单个默认处理程序或一个或多个特定于 OID 的处理程序。
...
例如，如果协议驱动程序发出 `NDIS_REQUEST_TYPE = NdisRequestGeneric1`的 OID 请求，则 NetAdapterCx 将调用[*EVT_NET_REQUEST_DEFAULT*](https://docs.microsoft.com/windows-hardware/drivers/ddi/netrequestqueue/nc-netrequestqueue-evt_net_request_default)。 如果客户端驱动程序未提供此类处理程序，NetAdapterCx 将会失败。
