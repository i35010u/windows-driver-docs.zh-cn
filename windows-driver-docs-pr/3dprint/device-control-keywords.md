...
ms.openlocfilehash: 615860b320b5af13db906c5bc4bea87764471fb6
ms.sourcegitcommit: 418e6617e2a695c9cb4b37b5b60e264760858acd
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/07/2020
ms.locfileid: "96783627"
---
...
<tr class="even">
<td>目录</td>
<td><p>正好包含1个 &lt; 值 &gt; 子元素，该元素必须包含 Job3DOutputAreaWidth、Job3DOutputAreaDepth 和 Job3DOutputAreaHeight 属性，并可能包含 Job3DOutputAreaMesh。</p>
...
描述输出卷的形状（如果不是矩形 prism）。 字符串值是一个 XML blob，后面是 &lt; &gt; 包含顶点和三角形的 3MF (规范，并且符合3MF 网格) 的 manifoldness 标准。 此多面体必须完全包含在以前的 OutputArea 属性所描述的边界框中。
...
该设备可以标识默认工作流应用程序，而不是默认 (示例包含 "打印" 对话框在选择此打印机时将调用的默认工作流) 。 此工作流应用允许可能需要的任何自定义 UI，或者最好为此设备设置3D 打印作业。
...
设备必须确定要从 Windows 打印系统接收的3MF 文件的版本。 版本由 URI 命名空间从适当版本的核心规范指定。 为了向后兼容，如果省略此关键字，则假定它采用默认值 " <https://schemas.microsoft.com/3dmanufacturing/2013/01”> ，指示3MF 的旧0.93 版本，这是不推荐的。
...
该设备可能会指定 (按命名空间的扩展，形成可理解的以空格分隔的列表) ，例如允许打印系统发送切片数据（如果有）。
