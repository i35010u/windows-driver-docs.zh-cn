...
ms.locfileid: "75652797"
---
# <a name="enable-a-custom-usb-interface-for-a-3d-printer"></a>为3D 打印机启用自定义 USB 接口

本主题中所述的体系结构支持在 v3 和 v4 print 生态系统中支持自定义 USB 接口3D 打印机。 标准端口监视器**3dmon**将3d 打印作业命令转发到使用本地服务凭据运行的 Windows **3DPrintService** 。 服务将加载并与伙伴 DLL 通信，以执行三维打印作业所需的自定义命令。 合作伙伴 DLL 以及**3dmon**和**3dprintservice**可再发行组件由设备的 USB 驱动程序包安装。 伙伴 DLL 必须实现和导出一组函数以便与**3DPrintService**进行通信。 与打印后台处理程序服务交互所需的其他功能在**3dmon**中实现。

> [!NOTE]
> 此体系结构要求伙伴 DLL 为多实例线程安全。

## <a name="architecture-decisions"></a>架构决策
...
用于在枚举\\3DPrint 下创建3D 打印机开发节点的内核模式设备驱动程序。 PnP 子系统通过基于 Winusb 创建的设备节点直接匹配 VID & PID 来调用它。 驱动程序 .inf 文件设置用于设置**3DPrintService**的自定义 DLL （如果系统上尚未安装）。

##### <a name="3dmondll"></a>3dmon

3DMon 是由 Microsoft 发布的端口监视器可再发行的二进制文件，由后台处理程序调用以与3D 打印机通信。

##### <a name="3dprintserviceexe"></a>3dprintservice

3DPrintService 是在驱动程序安装过程中作为 Windows 服务安装的 Microsoft 发布的二进制文件。 3DMon 与此服务通信，以执行打印、双向等操作和3D 打印机。

##### <a name="partnerimpldll"></a>Partnerimpl

Partnerimp 是合作伙伴实现的已发布 Microsoft 界面。 DLL 使用其协议与伙伴的设备通信。 3DPrintService 在运行时加载此 DLL 以驱动3D 打印机设备的操作。

![3dprintservice](images/3dprintservice.png)

### <a name="printer-usage-sequence"></a>打印机使用顺序

- 后台处理程序与3dmon 进行通信，后者将命令发送到 3DPrintService windows 服务
- 3DPrintService 运行时的帐户凭据为 NetworkService
- 当使用三维打印机时，通过3dmon 的后台处理程序将命令发送到3DPrintService
...
### <a name="hresult-installin-lpcwstr-args"></a>HRESULT 安装（在\] LPCWSTR 参数中\[）
...
### <a name="hresult-printfilein-dword-jobid-in-lpwstr-portname-in-lpwstr-printername-in-lpwstr-pathtorenderedfileinlpvoid-pppartnerdata"></a>HRESULT PrintFile （\] DWORD jobId 中的\[，\[\] LPWSTR Portvalue，\[LPWSTR printerName，\] \[LPWSTR pathToRenderedFile，\] LPVOID\[ppPartnerData）
...
- 可使用端口名称从注册表中获取**printerName** 。 第三方制造商 maynot 可以使用端口名称与其设备通信。 打印机名称在 Windows 计算机上是唯一的，其软件将能够确定要打印作业的打印机。 在计算机上处于活动状态的所有打印机都可以在以下注册表项中找到：
    ...
    **HKEY\_本地\_计算机\\系统\\CurrentControlSet\\控件\\打印\\打印机**

![3d 打印机注册表](images/3d-printer-registry.png)

### <a name="hresult-query_in_-lpcwstr-command-_in_-lpcwstr-commanddata-_out_-lpwstr-resultbuffer-_out_-resultbuffersize--_in_-lpvoid-pppartnerdata"></a>HRESULT 查询（\_\_ LPCWSTR 命令，\_\_ LPCWSTR commandData，\_\_ LPWSTR resultBuffer，\_RESULTBUFFERSIZE\_ LPVOID \_ppPartnerData）
...
在每次调用**Query （）** 函数时，DLL 都可以使用以前的**IntializePrint （）** 调用中的实例数据与设备通信，而无需打开新的通信通道。

此 API 用于与打印机进行通信，以获取有关设备配置和打印进度的信息，或者通知伙伴 DLL 设备的拔出事件。

此制造商必须支持以下命令：

| 命令 | CommandData | 输出 | 说明 |
|---------|-------------|--------|----------|
...
| \\\\打印机：数据 | | 符合 PrintDeviceCapabilites （PDC）架构的 XML 字符串。 | PDC 查询由想要获取有关打印机的详细信息的应用程序调用。 数据用于描述设备的功能，如果驱动程序依赖于 Microsoft 切片器，则可以包括切片器设置。 请参阅下面的示例 PDC。 |
...
### <a name="hresult-uninstallinlpcwstr-args"></a>HRESULT UnInstall （\[在\]LPCWSTR 参数中）
