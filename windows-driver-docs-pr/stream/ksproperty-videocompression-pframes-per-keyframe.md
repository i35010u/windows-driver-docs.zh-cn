...
description: KSPROPERTY\_VIDEOCOMPRESSION\_PFRAMES\_按\_关键帧属性控制预测帧（P 帧）间隔。 必须实现此属性。
...
ms.locfileid: "72837882"
---
# <a name="ksproperty_videocompression_pframes_per_keyframe"></a>KSPROPERTY\_VIDEOCOMPRESSION\_PFRAMES\_每\_关键帧


KSPROPERTY\_VIDEOCOMPRESSION\_PFRAMES\_按\_关键帧属性控制预测帧（P 帧）间隔。 必须实现此属性。
...
属性值（操作数据）是指定每个关键帧的预测帧数的 LONG。

<a name="remarks"></a>备注
-------

KSPROPERTY\_VIDEOCOMPRESSION\_S 结构的**值**成员指定每个关键帧 P 帧的数目。 如果 set 请求提供**负值，则**微型驱动程序应将 "P 帧速率" 设置为默认值。

支持此属性的微型驱动程序应将 KS\_VideoCompressionCaps\_CanBFrame 标志设置为[**KSPROPERTY\_VIDEOCOMPRESSION\_GETINFO\_S**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ksmedia/ns-ksmedia-ksproperty_videocompression_getinfo_s)结构的**功能**成员，检索微型驱动程序的视频压缩功能。
