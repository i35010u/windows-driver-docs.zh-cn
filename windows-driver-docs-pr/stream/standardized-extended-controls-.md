...
[ **KSPROPERTY\_CAMERACONTROL\_扩展\_元数据**](https://msdn.microsoft.com/library/windows/hardware/dn917952)中定义的属性 ID [ **KSPROPERTY\_CAMERACONTROL\_扩展\_属性**](https://msdn.microsoft.com/library/windows/hardware/dn917962)枚举由元数据缓冲区要求，例如所需的元数据大小，内存对齐要求，客户端查询和所需的针对元数据缓冲区分配内存分配类型。
...
[ **KSSTREAM\_元数据\_信息**](https://msdn.microsoft.com/library/windows/hardware/dn936959)结构，以及以下标志，用于将元数据缓冲区发送到该驱动程序。

```cpp
#define KSSTREAM_HEADER_OPTIONSF_METADATA           0x00001000
```

一旦缓冲区 （元数据 + 帧） 排队发送至该驱动程序，DevProxy 会发送标准[ **KSSTREAM\_标头**](https://msdn.microsoft.com/library/windows/hardware/ff567138)结构后, 跟[ **KS\_帧\_信息**](https://msdn.microsoft.com/library/windows/hardware/ff567645)结构，并跟**KSSTREAM\_元数据\_信息**结构。 将进一步屏蔽 DevProxy **KSSTREAM\_标头。OptionFlags**与**KSSTREAM\_标头\_OPTIONSF\_元数据**将传递到驱动程序缓冲区之前。
...
-   [**KSCAMERA\_元数据\_ITEMHEADER**](https://msdn.microsoft.com/library/windows/hardware/dn925184)

-   [**KSCAMERA\_元数据\_PHOTOCONFIRMATION**](https://msdn.microsoft.com/library/windows/hardware/dn925187)
...
[ **KSPROPERTY\_CAMERACONTROL\_扩展\_FOCUSPRIORITY** ](https://msdn.microsoft.com/library/windows/hardware/dn917942)属性 ID 是唯一的焦点优先级 DDI 与相关联的控件。

## <a name="focus-state"></a>焦点状态


[ **KSPROPERTY\_CAMERACONTROL\_扩展\_FOCUSSTATE** ](https://msdn.microsoft.com/library/windows/hardware/dn917944)属性 ID 是唯一使用 DDI 的焦点状态相关联的控件。
...
-   [**KSPROPERTY\_CAMERACONTROL\_扩展\_投资回报率\_ISPCONTROL**](https://msdn.microsoft.com/library/windows/hardware/dn917966)

## <a name="photo-confirmation"></a>照片确认


[ **KSPROPERTY\_CAMERACONTROL\_扩展\_PHOTOCONFIRMATION** ](https://msdn.microsoft.com/library/windows/hardware/dn917957)属性 ID 是唯一使用 DDI 的照片确认相关联的控件。

## <a name="photo-sequence-submode"></a>照片序列子模式


[ **KSPROPERTY\_CAMERACONTROL\_扩展\_PHOTOMODE** ](https://msdn.microsoft.com/library/windows/hardware/dn567582)属性 ID 是唯一的控件与照片序列 DDI 相关联。
...
<td>此属性包含 EV 补偿步骤标志以及时捕获照片应用到该驱动程序的步骤的单位中的 EV 补偿值。 <a href="https://msdn.microsoft.com/library/windows/hardware/dn897242" data-raw-source="[&lt;strong&gt;CapturedMetadataExposureCompensation&lt;/strong&gt;](https://msdn.microsoft.com/library/windows/hardware/dn897242)"> <strong>CapturedMetadataExposureCompensation</strong> </a>数据结构描述仅限此属性的 blob 格式。 EV 补偿的元数据项目结构格式 (<strong>KSCAMERA_METADATA_ITEMHEADER</strong> + EV 补偿元数据有效负载) 驱动程序提供，并且必须是对齐的 8 字节。</td>
</tr>
<tr class="even">
...
<td><p><strong>UINT32</strong></p>
<p>(Q16)</p></td>
<td>此属性包含应用的缩放值，这是相同的值可以从查询<a href="https://msdn.microsoft.com/library/windows/hardware/dn936756" data-raw-source="[&lt;strong&gt;KSPROPERTY_CAMERACONTROL_EXTENDED_ZOOM&lt;/strong&gt;](https://msdn.microsoft.com/library/windows/hardware/dn936756)"> <strong>KSPROPERTY_CAMERACONTROL_EXTENDED_ZOOM</strong> </a> GET 调用中。 值必须为 Q16。</td>
</tr>
<tr class="odd">
...
[ **KSPROPERTY\_CAMERACONTROL\_扩展\_ISO\_高级**](https://msdn.microsoft.com/library/windows/hardware/dn917947)属性 ID 是唯一的整数 ISO DDI 与关联的控件。

## <a name="advanced-focus"></a>高级的焦点


[ **KSPROPERTY\_CAMERACONTROL\_扩展\_FOCUSMODE** ](https://msdn.microsoft.com/library/windows/hardware/dn567576)属性 ID 是唯一的整数 ISO DDI 与关联的控件。

## <a name="flash"></a>Flash


[ **KSPROPERTY\_CAMERACONTROL\_扩展\_FLASHMODE** ](https://msdn.microsoft.com/library/windows/hardware/dn567575)属性 ID 是唯一与闪存 DDI 相关联的控件。

## <a name="zoom"></a>Zoom


[ **KSPROPERTY\_CAMERACONTROL\_扩展\_缩放**](https://msdn.microsoft.com/library/windows/hardware/dn936756)属性 ID 是唯一使用 DDI 的缩放相关联的控件。

## <a name="scene-mode"></a>场景模式


[ **KSPROPERTY\_CAMERACONTROL\_扩展\_SCENEMODE** ](https://msdn.microsoft.com/library/windows/hardware/dn567585)属性 ID 是唯一使用 DDI 的场景模式相关联的控件。
