...
具体而言, 照相机驱动程序必须在与捕获设备接口相关联的 ACPI \_PLD 结构中设置新引入的字段旋转:
...
*ACPI \_PLD Panel 字段的定义 (Rev 5.0 a)*
...
如果传感器无法以0度的偏移量装入, 则*建议*的方案是让 AV 流微型端口驱动程序 (或用户模式插入的 DMFT 或 MFT0 形式的插件) 更正生成的捕获帧, 使其在0度内公开给管道ree 偏移量。
