...
ms.locfileid: "72834478"
---
# <a name="encoder-implementation-and-support"></a>编码器实现和支持

在 Windows XP Service Pack 1 中，Microsoft 在*ksmedia*中定义了三个内核流式处理属性集和一个枚举，以支持仅视频编码器设备。 每个属性集都包含一个属性。 换言之，每个属性都接收其自己的属性集。 如果你的驱动程序进行了*get*属性或*设置*属性调用，则在设置了时，请在[**KSPROPERTY**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ks/ns-ks-ksidentifier)结构的**SET**成员中指定属性集的 GUID （如*ksmedia*中所定义） ，并在设置调用：
...
客户端通过从**IEncoderAPI** com 接口（如 Windows 软件开发工具包（SDK）文档中所述）派生**IVideoEncoder** com 接口，来访问这些属性。

微型驱动程序必须为每个 ENCAPIPARAM\_*Xxx*属性指定默认值。 主题[编码器代码示例](encoder-code-examples.md)演示如何指定默认属性值。 在开发和调试编码器筛选器的过程中，可以从支持 ENCAPIPARAM\_比特率属性集的微型驱动程序触发当前属性页。
...
<td><p>如果设备支持编码视频流（包括电视音频等辅助音频），则对此属性集实现支持。</p></td>
</tr>
<tr class="even">
...
<td><p>如果设备是仅音频编码器，则实现对此属性集（而不是 CODECAPI_VIDEO_ENCODER）的支持。</p></td>
</tr>
<tr class="odd">
...
<td><p>如果设备通过 CODECAPI_SUPPORTSEVENTS 属性集支持响应用户模式事件，则实现此事件集以返回编码器设置列表，这些设置已更改为客户端的以前<em>设置</em>属性调用的结果CODECAPI_SETALLDEFAULTS 或 CODECAPI_ALLSETTINGS。</p></td>
</tr>
</tbody>
</table>

客户端通过**ICodecAPI** COM 接口（如 Windows SDK 文档中所述）访问这些属性。 有关 COM 接口的详细信息，请参阅[编码器安装和注册](encoder-installation-and-registration.md)，其中包括如何指定 KsProxy 应公开的接口。
...
有关使用媒体和筛选器的多个实例（在单独的筛选器图形中）的详细信息，请参阅[媒体和类别](mediums-and-categories.md)。
