...
标准功能是大多数打印机通常提供的[打印机功能](printer-features.md)。 它们由 GPD 语言识别的预定义名称标识。 （表示这些名称的字符串的资源标识符包含在 stdnames gpd 中，后者随 Microsoft Windows 驱动程序工具包 \[WDK\]提供。）某些标准功能是必需的，并且必须为每台打印机指定。 其他选项是可选的。
