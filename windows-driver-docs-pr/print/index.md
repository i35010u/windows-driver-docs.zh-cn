---
title: 打印机驱动程序设计指南
description: 本部分提供了有关设计、测试和调试打印机驱动程序的信息。
ms.assetid: ff6b9c2e-920a-4cfc-8872-38976ed872f9
keywords:
- 打印设备 WDK
- 打印 WDK，请参阅打印机驱动程序
- 打印 WDK，请参阅打印
ms.date: 01/24/2018
ms.topic: article
ms.prod: windows-hardware
ms.technology: windows-devices
ms.openlocfilehash: 3f4280e8755357f0cc9adf76961078462bc1050c
ms.sourcegitcommit: f500ea2fbfd3e849eb82ee67d011443bff3e2b4c
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/31/2020
ms.locfileid: "89205885"
---
# <a name="printer-driver-design-guide"></a>打印机驱动程序设计指南


本部分提供了有关设计、测试和调试打印机驱动程序的信息。

## <a name="in-this-section"></a>本节内容


-   [V4 打印机驱动程序](v4-printer-driver.md)
-   [Microsoft 打印机驱动程序](microsoft-printer-drivers.md)
-   [打印机驱动程序概述](printer-driver-overview.md)
-   [Microsoft 通用打印机驱动程序](microsoft-universal-printer-driver.md)
-   [Microsoft PostScript 打印机驱动程序](microsoft-postscript-printer-driver.md)
-   [Microsoft 绘图仪驱动程序](microsoft-plotter-driver.md)
-   [GDI 打印机驱动程序](gdi-printer-drivers.md)
-   [打印票证和打印功能技术](print-ticket-and-print-capabilities-technologies.md)
-   [编写 64 位打印机驱动程序](writing-64-bit-printer-drivers.md)
-   [安装和配置打印机驱动程序](installing-and-configuring-printer-drivers.md)
-   [打印机配置](printer-configuration.md)
-   [通用描述语言 (GDL)](generic-description-language.md)
-   [GDL 参考](gdl-reference.md)
-   [GDL 示例](gdl-examples.md)
-   [打印机自动配置](printer-autoconfiguration.md)
-   [自动配置详细信息](autoconfiguration-details.md)
-   [自动配置实现选项](autoconfiguration-implementation-options.md)
-   [双向通信](bidirectional-communication.md)
-   [双向通信架构参考](bidi-communications-schema-reference.md)
-   [双向接口和枚举](bidi-interfaces-and-structures.md)
-   [异步用户通知架构](asynchronous-user-notification-schema.md)
-   [设备打印服务架构的 Web 服务](web-services-for-devices-print-service-schema.md)
-   [打印机驱动程序隔离](printer-driver-isolation.md)
-   [自定义 Microsoft 打印机驱动程序](customizing-microsoft-s-printer-drivers.md)
-   [打印机网页的 COM 接口](com-interfaces-for-printer-web-pages.md)
-   [打印管道属性包](print-pipeline-property-bag.md)
-   [其他打印机主题](additional-printer-topics.md)
-   [打印后台处理程序组件](print-spooler-components.md)
-   [打印机驱动程序测试和调试](printer-driver-testing-and-debugging.md)

 
## <a name="related-sections"></a>相关章节


-   [打印 DDI 参考](/windows-hardware/drivers/ddi/_print)
