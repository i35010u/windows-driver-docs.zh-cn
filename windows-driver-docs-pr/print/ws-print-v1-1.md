...
## <a name="ws-print-v11"></a>WS-Print 1.1 版

对于 Windows 8， (WSD) 设备上的 web 服务的打印架构已更新到1.1 版。 此版本的架构 (称为 WS-Print v1.1) 已更新，以支持增强的驱动程序配置、更好的墨迹/墨粉颜色表示形式和设备型号 Id。

## <a name="ws-print-v12"></a>WS-Print 1.2 版

对于 Windows 8.1，WS-Print 包括 WS-Print 1.1 版中使用的所有操作和架构元素，但已更新设备上 web 服务的打印服务定义。 用于打印的设备上生成的新 Web 服务 WS-Print 为版本1.2。

在 WS-Print 中，为新架构元素提供支持，并已添加新操作。 新架构元素 "SupportsWSPrintV12" 用于标识对 WS-Print v1.0 的支持。 新操作 "SetPrinterElements" 使客户端能够在打印机上设置架构元素的值。 例如，客户端可以设置一个名为 "InkHeadAlignmentValue" 的自定义元素，打印机将使用该元素重新调整喷墨打印头。
...
下面的序列图演示了客户端和打印机之间的交互，以确定支持的 WS-Print 命名空间的版本，然后检索扩展的架构元素。

### <a name="ws-print-v11-sequence-diagram"></a>WS-Print 1.1 序列图

下面是支持 WS-Print 1.1 版的打印机的交互序列图：

![显示有关 ws 打印 v1.1 支持的客户端打印机交互，以及用于打印机说明和配置的后续查询的序列图。](images/wsprint-diagv11.png)

如果打印机支持 WS-Print 1.1 版，则在响应来自客户端的 GetPrinterElements (wprt： PrinterDescription) 查询时，打印机将回发信息以指示它的功能。

在打印机确认它支持 WS-Print 1.1 版后，客户端将发送一个 GetPrinterElements (wprt11： DriverConfiguration) 查询，并使用请求的驱动程序配置信息进行响应。

### <a name="ws-print-v12-sequence-diagram"></a>WS-Print 1.2 序列图

下面是支持 WS-Print 1.2 版的打印机的交互序列图：

![显示有关 ws 打印 v1.0 支持的客户端打印机交互，以及用于打印机说明和配置的后续查询的序列图。](images/wsprint-diagv12.png)

如果打印机支持 WS-Print 1.2 版，则在响应来自客户端的 GetPrinterElements (wprt： PrinterDescription) 查询时，打印机将发送回信息以指示它的功能。
...
在打印机确认它支持 WS-Print 1.2 版后，客户端将发送 GetPrinterElements (wprt12： DriverConfiguration) 查询，打印机将使用请求的驱动程序配置信息进行响应。

## <a name="namespaces"></a>命名空间

### <a name="ws-print-v11-namespace"></a>WS-Print v1.1 命名空间
...
### <a name="ws-print-v12-namespace"></a>WS-Print 1.2 命名空间
...
## <a name="specifying-ws-print-11-support"></a>指定 WS-Print 1.1 支持

支持 WS-Print 1.1 元素的打印机必须将其 PrinterDescription 更新为包含 wprt11： SupportsWSPrintv11。 如果未指定 wprt11： SupportsWSPrintv11 并将其设置为 true，则 WSDMon 将不会从打印机请求任何 WS-Print 1.1 元素。

支持 WS-Print 1.1 版的打印设备必须在其 PrinterDescription 中包含以下内容，Windows 才能查询该命名空间中的任何其他元素。
...
以下 XML 代码片段显示了支持 WS-Print 1.1 版的打印设备的架构。
...
## <a name="specifying-ws-print-12-support"></a>指定 WS-Print 1.2 支持

支持 WS-Print 1.2 元素的打印机必须将其 PrinterDescription 更新为包含 wprtV12： SupportsWSPrintV12。 如果未指定 wprtV12： SupportsWSPrintV12 并将其设置为 true，则 WSDMon 将不会从打印机请求任何 WS-Print 1.2 元素。

支持 WS-Print 1.2 版的打印设备必须在其 PrinterDescription 中包含以下内容，Windows 才能查询该命名空间中的任何其他元素。
...
以下三个部分中的架构示例说明了如何使用 WS-Print v1.1 引入的一些新元素。 有关 WS-Print v1.1 命名空间中引入的所有元素的详细信息，请参阅下面 " **下载** " 部分中列出的 WS-Print 1.0 版–1.2 版的支持文件。
...
如本主题前面所述，在 WS-Print 1.2 版 "部分中，WS-Print 1.2 命名空间引入了以下新操作。

## <a name="setprinterelements"></a>SetPrinterElements

**SetPrinterElements** 操作是新操作，可让客户端在打印机上设置架构元素的值。 SetPrinterElements 操作包含八个请求元素和四个响应元素。 Request 和 response 元素使客户端能够在与 WS-Print 设备架构的连接中对数据插入和检索进行精细控制。

例如，"ElementPath" 数据元素 ("SetPrinterElements") 操作的一部分，它是一个 XPath 字符串，表示要设置的数据元素的打印机架构内的位置。

有关 SetPrinterElements 操作的更多详细信息，请参阅下面 " **下载** " 部分中列出的 WS-Print v1.0 – v2.0 1.2 的支持文件。

## <a name="downloads"></a>下载

### <a name="specification-and-supporting-files-for-ws-print-v10--v12"></a>WS-Print v1.0 –1.2 版的规范和支持文件
