...
大部分 Srb 更高级别的存储类驱动程序已加载后，发送到[ **HwScsiStartIo** ](https://docs.microsoft.com/previous-versions/windows/hardware/drivers/ff557323(v=vs.85))例程具有**函数**成员设置为 SRB\_函数\_EXECUTE\_SCSI。
...
如果微型端口驱动程序使用系统 DMA，它必须调用[ **ScsiPortIoMapTransfer**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/srb/nf-srb-scsiportiomaptransfer)*之前*编程 HBA 将数据传输。 **ScsiPortIoMapTransfer**将设置系统 DMA 控制器，调用微型端口驱动程序*HwScsiDmaStarted*日常的更高版本中所述[SCSI 微型端口驱动程序 HwScsiDmaStarted 例程](scsi-miniport-driver-s-hwscsidmastarted-routine.md).
