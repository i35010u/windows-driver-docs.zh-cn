...
ms.date: 10/17/2018
ms.openlocfilehash: 61004e81d1332caa93267c5248b2662a8d57dfdd
ms.sourcegitcommit: fb7d95c7a5d47860918cd3602efdd33b69dcf2da
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/25/2019
ms.locfileid: "67368296"
---
...
中指定的管理操作[**设备\_管理\_数据\_设置\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)的系统缓冲区中包含的结构[ **IOCTL\_存储\_管理\_数据\_设置\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ni-ntddstor-ioctl_storage_manage_data_set_attributes)请求。
...
对于此操作， **ParameterBlockOffset**并**ParameterBlockLength**的成员[**设备\_管理\_数据\_设置\_特性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)结构指定通知操作的参数。 这些参数的格式为[**设备\_DSM\_通知\_参数**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_notification_parameters)结构。
...
对于此操作， **ParameterBlockOffset**并**ParameterBlockLength**的成员[**设备\_管理\_数据\_设置\_特性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)结构指定的卸载参数读取操作。 这些参数的格式为[**设备\_DSM\_卸载\_读取\_参数**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_offload_read_parameters)结构。
...
对于此操作， **ParameterBlockOffset**并**ParameterBlockLength**的成员[**设备\_管理\_数据\_设置\_特性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)结构指定的卸载参数写入操作。 这些参数的格式为[**设备\_DSM\_卸载\_编写\_参数**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_offload_write_parameters)结构。
...
逻辑块范围指定在单个[**设备\_数据\_设置\_范围**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)结构。
...
逻辑块范围指定在单个[**设备\_数据\_设置\_范围**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)结构。 中指定的修复副本数**设备\_数据\_设置\_修复\_参数**结构位于指定的输入缓冲区中**ParameterBlockOffset**的成员[**设备\_管理\_数据\_设置\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)结构。
...
<span id="DeviceDsmAction_DrtQuery"></span><span id="devicedsmaction_drtquery"></span><span id="DEVICEDSMACTION_DRTQUERY"></span>**DeviceDsmAction\_DrtQuery** (8 |DeviceDsmActionFlag\_非破坏性) 驱动程序将执行查询指定的逻辑块范围和检查如果包含在存储空间脏区域跟踪。 中返回该范围的同步状态**OperationStatus**的成员[**设备\_管理\_数据\_设置\_属性\_输出**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes_output)结构。

指定逻辑块范围，则在输入时，在单个[**设备\_数据\_设置\_范围**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)结构。
...
在中设置新的目标优先级**设备\_DSM\_NVCACHE\_更改\_优先级\_参数**结构位于**ParameterBlockOffset**中[**设备\_管理\_数据\_设置\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)。 要更改的优先级的逻辑块范围给出了一个或多个[**设备\_数据\_设置\_范围**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)结构。
...
如果按位**或者**的**设备\_数据\_管理\_设置\_操作**常量值和**DeviceDsmActionFlag\_非破坏性**执行标志，指定的操作会造成任何破坏。 如果设置此标志，该驱动程序可以安全地将转发[ **IOCTL\_存储\_管理\_数据\_设置\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ni-ntddstor-ioctl_storage_manage_data_set_attributes)请求即使堆栈中的下一个较低驱动程序到驱动程序不处理指定的操作。

&gt; \[!请注意\]&gt;转发之前[ **IOCTL\_存储\_管理\_数据\_设置\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ni-ntddstor-ioctl_storage_manage_data_set_attributes)请求，该驱动程序仍执行的数据集范围中指定的块的正常处理[**设备\_管理\_数据\_设置\_特性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)结构。 此块位于**DataSetRangesOffset**和包含格式化为一个或多个连续项[**设备\_数据\_设置\_范围**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)结构。 在中设置数据集范围的长度，以字节为单位， **DataSetRangesLength**的成员**设备\_管理\_数据\_设置\_特性**。
...
[**设备\_DSM\_通知\_参数**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_notification_parameters)
