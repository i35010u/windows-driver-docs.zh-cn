...
本主题介绍 Windows 驱动程序工具包 (WDK) 和企业版 WDK (EWDK) 的早期版本以及用于提供支持的其他下载内容。 若要开发驱动程序，请使用可从[下载 Windows 驱动程序工具包 (WDK)](download-the-wdk.md) 下载的最新公共版 Windows 驱动程序工具包 (WDK) 和工具。
...
我们在特定的 Visual Studio 版本中提供驱动程序开发支持。 若要开发特定 Windows 版本的驱动程序，必须使用下表中列出（并提供下载链接）的某个 Visual Studio 版本。
...
> [!IMPORTANT]
> 如果先安装了适用于 Windows 10 版本 1703 的 WDK，然后又在同一个系统中安装了适用于 Windows 10 版本 1607 的 WDK，则可能会删除较早 WDK 版本的某些文件。 若要还原这些文件，请执行以下操作：
...
企业版 WDK (EWDK) 是一种独立的自包含命令行环境，用于生成驱动程序和基本的 Win32 测试应用程序。 其中包括 Visual Studio 生成工具、SDK 和 WDK。 此环境不包含 Visual Studio 中可用的所有功能，例如集成开发环境 (IDE)。
