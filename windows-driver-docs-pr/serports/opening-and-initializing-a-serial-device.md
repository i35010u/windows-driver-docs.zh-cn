...
- 设备在打开时处于未定义状态。 在使用设备之前，客户端应将设备初始化为已知状态。 用户模式客户端必须使用 Microsoft Windows SDK 中 Windows 基础服务支持的通信功能。 内核模式客户端可以使用 IOCTL\_串行\_集\_Xxx 和 IOCTL\_串行\_内部\_Xxx 请求。 有关详细信息，请参阅[ntddser](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntddser/)标头。
