...
> 2)  从2012年10月1日开始的开放 Web 基础最终规范协议版本1.0 （"OWF 1.0"），网址为 http://www.openwebfoundation.org/legal/the-owf-1-0-agreements/owfa-1-0 。 
...
| 保留 | 3 | 37 | 必须为 0。 | 
| 基址 | 12 | 40|<p>使用 ACPI 一般地址结构描述的串行端口寄存器集的基址。</p>0 = 禁用控制台重定向 <p>**注意：** <p>COM1 （0x3F8）是：</p><ul><li>整数格式： 0x 01 08 00 00 00000000000003F8</li><li>在内存中查看：0x01080000F803000000000000</li></ul><p>COM2 （Ox2F8）是：<ul><li>整数格式： 0x 01 08 00 00 00000000000002F8</li><li>在内存中查看：0x01080000F802000000000000 </li></ul>|
| 中断类型 | 1 | 52 | UART 使用的中断类型：<ul><li>位 [0]： PC 兼容双 8259 IRQ 中断</li><li>位 [1]： i/o APIC 中断（全局系统中断）</li><li>位 [2]： i/o SAPIC 中断（全局系统中断）</li><li>Bit [3]： ARMH GIC 中断（全局系统中断）</li><li>Bit [4： 7]： Reserved （必须设置为0）</li></ul> 其中 <ul><li>0 = 不支持</li><li>1 = 支持</li></ul> 同时具有双8259和 i/o APIC 或 i/o SAPIC 的平台必须设置 IRQ 位（位 [0]）和对应的全局系统中断位。 （例如，支持8259和 SAPIC 的系统会是5）|
| IRQ | 1 | 53 | UART 使用的计算机兼容 IRQ： <ul><li>2-7、9-12、14-15 = 分别为有效的 Irq</li><li>0-1、8、13、16-255 = 保留</li></ul> 仅当设置了中断类型字段的位 [0] 时才有效。 | 
| 全局系统中断 | 4 | 54 | UART 使用的全局系统中断（GSIV）。 <p>如果位 [0] 是 "中断类型" 字段中设置的唯一位，则无效。 </p>|
