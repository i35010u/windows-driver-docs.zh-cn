...
description: 串行端口控制台重定向表，用于指示串行端口或非旧 UART 接口是否可用于 Microsoft® Windows®紧急管理服务（EMS）。
ms.assetid: cbcd2e71-881a-44f6-b3d8-5d2aadb02b27
keywords:
- SPCR
- EMS UART
ms.date: 07/23/2018
ms.openlocfilehash: a36af1925289df4e9cc3614635b7bf1b28224cfc
ms.sourcegitcommit: d30691c8276f7dddd3f8333e84744ddeea1e1020
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/19/2019
ms.locfileid: "75210903"
---
# <a name="serial-port-console-redirection-table--spcr"></a>串行端口控制台重定向表（SPCR）

本文档定义串行端口控制台重定向表的内容。 此表用于指示串行端口或非旧 UART 接口是否可用于 Microsoft® Windows®紧急管理服务（EMS）。
...
> 1)  Microsoft 的社区承诺，<https://www.microsoft.com/openspecifications/en/us/programs/community-promise/default.aspx>上提供;或
> 2)  从2012年10月1日开始的开放 Web 基础最终规范协议版本1.0 （"OWF 1.0"），http://www.openwebfoundation.org/legal/the-owf-1-0-agreements/owfa-1-0提供。 
...
|    创建者修订 | 4 | 32 | 创建该表的实用工具的修订版本。 对于 DSDT、RSDT、SSDT 和 PSDT 表，这是 ASL 编译器的修订版本。 | 
| 接口类型 | 1 | 36 | 指示注册接口的类型： <p> 对于修订版本1： </p> <ul><li>0 = 完整16550接口 </li><li>1 = 完整16450接口（还必须接受写入 16550 FCR register） </li><li>2-255 = 保留 </li></ul><p>对于修订版2： </p>请参阅[DBG2 规范](https://go.microsoft.com/fwlink/p/?LinkId=234837)的表3中的串行端口子类型 </p>| 
| 保留 | 3 | 37 | 必须为 0。 | 
| 基址 | 12 | 40|<p>使用 ACPI 一般地址结构描述的串行端口寄存器集的基址。</p>0 = 禁用控制台重定向 <p>**注意：** <p>-COM1 （0x3F8）为： </p><p>整数格式： 0x 01 08 00 00 00000000000003F8</p><p>在内存中查看：0x01080000F803000000000000</p><p>COM2 （Ox2F8）是： </p><p>整数格式： 0x 01 08 00 00 00000000000002F8</p><p>在内存中查看：0x01080000F802000000000000 </p>|
| 中断类型 | 1 | 52 | UART 使用的中断类型。 <p>位 [0] 计算机兼容双 8259 IRQ 中断</p><p>位 [1] i/o APIC 中断（全局系统中断）</p><p>位 [2] i/o SAPIC 中断（全局系统中断）</p> 效率 <p>Bit [3] ARMH GIC 中断（全局系统中断）</p><p>位 [4： 7] 保留（必须为0）</p><p>0 = 不支持</p><p>1 = 支持</p><p>同时具有双8259和 i/o APIC 或 i/o SAPIC 的平台必须设置 IRQ 位（位 [0]）和对应的全局系统中断位。）。 例如 支持8259和 SAPIC 的系统为5。 </p>|
|效率|1|53 | UART 使用的与计算机兼容的 IRQ。<p>2-7、9-12、14-15 = 分别为有效的 Irq</p><p>0-1、8、13、16-255 = 保留</p><p>仅当设置了中断类型字段的位 [0] 时才有效。</p> | 
|全局系统中断|4| 54| UART 使用的全局系统中断（GSIV）。 如果位 [0] 是 "中断类型" 字段中设置的唯一位，则无效。|
...
| PCI 标志 | 4 | 71 | PCI 兼容性标志位掩码。  默认情况下应为零。<ul><li>位 [0] = 操作系统不应抑制 PNP 设备枚举或禁用此设备的电源管理。 如果不是 PCI 设备，则必须为0</li><li>Bits 1-31 = RESERVED，必须为零。</li></ul>| 
